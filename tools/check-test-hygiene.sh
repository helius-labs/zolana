...
#
# The shielded-pool packages auto-discover their targets: every `tests/*.rs` is
# one binary named after the file. That removes the check this section used to
# carry -- comparing `[[test]]` declarations against the tree -- because there
# are no declarations left to drift from it, and a file added under `tests/`
# cannot fail to become a target.
#
# One surface still needs it. A file in a *subdirectory* of `tests/` is not
# auto-discovered, so it only runs if a target pulls it in as a module. Today
# that is `shielded-pool-localnet/tests/localnet_photon/`, whose leaves are
# reached through `#[path]` from the suite root.
...
# and a citation naming something that no longer exists reads as a green
# checkmark against an invariant nothing tests.
#
# Only that -- do the citations resolve -- is checked. The README's tallies and
# the prose of a retired entry were checked here too; both are bookkeeping about
# the bookkeeping, and a wrong count misleads nobody about whether an invariant
# is tested.
