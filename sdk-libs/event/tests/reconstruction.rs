...
use zolana_event::reconstruction::{
    general_event_from_site, reconstruct_merge_event, reconstruct_transact_event, ReconstructError,
};
...
fn merge_inputs(nullifiers: &[[u8; 32]], first_input_queue_seq: u64) -> Vec<Input> {
    nullifiers
        .iter()
        .enumerate()
        .map(|(offset, nullifier)| Input {
            tree: MERGE_INPUT_TREE,
            input_queue_seq: first_input_queue_seq
...
fn every_transact_source_uses_the_same_flat_instruction_decoder() {
    ...
    for source_tag in [
        ...
        assert_eq!(rebuilt, expected, "tag {source_tag}");
    }
}
...
fn ring_merge_reconstruction_uses_the_embedded_merge_payload() {
    ...
    assert_eq!(
        rebuilt,
        GeneralEvent {
            inputs: merge_inputs(&instruction.merge.nullifiers, 50),
...
fn direct_merge_reconstruction_retains_the_event_view_tag() {
    ...
    assert_eq!(
        rebuilt,
        GeneralEvent {
            inputs: merge_inputs(&instruction.nullifiers, 50),
            outputs: vec![OutputUtxo {
                view_tag: [44; 32],
                utxo_hash: instruction.output_utxo_hash,
                data: Vec::new(),
            }],
            messages: Vec::new(),
            tx_viewing_pk: [0; 33],
...
fn an_owner_tag_past_the_account_list_is_rejected() {
    ...
    data.outputs
        .get_mut(1)
        .expect("fixture has an account-tagged output")
        .owner_tag = OwnerTag::Account(200);
    let event = TransactEvent {
        first_input_queue_seq: 0,
        first_output_leaf_index: 0,
    };
...
fn source_tag_must_match_the_parent_instruction() {
    ...
    *parent.first_mut().expect("parent carries a tag byte") = tag::RING_TRANSACT;
    let event = TransactEvent {
        first_input_queue_seq: 0,
        first_output_leaf_index: 0,
    };
