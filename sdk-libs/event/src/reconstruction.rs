...
//! `zolana-interface`'s instruction types; the event crate owns it because the result
...
pub fn reconstruct_transact_event(
    ...
    reconstruct_transact_from_payload(source_instruction_tag, payload, parent_accounts, event)
}

fn reconstruct_transact_from_payload(
    source_instruction_tag: u8,
    payload: &[u8],
    parent_accounts: &[[u8; 32]],
    event: &TransactEvent,
        ...
        let asset = match transfer.mint_account_position() {
            ...
            Some(within) => {
                let index = offset
                    .checked_add(within)
                    .ok_or(ReconstructError::IndexOverflow)?;
                Some(*account_at(parent_accounts, index)?)
            }
        };
...
pub fn reconstruct_merge_event(
    ...
    let payload = parent_payload(source_instruction_tag, parent_data)?;
    reconstruct_merge_from_payload(source_instruction_tag, payload, parent_accounts, event)
}

fn reconstruct_merge_from_payload(
    source_instruction_tag: u8,
    payload: &[u8],
    parent_accounts: &[[u8; 32]],
    event: &MergeEvent,
...
pub fn general_event_from_site(
    ...
    let parent_payload = parent_payload(source_instruction_tag, parent_data)?;
    ...
    match kind {
        EventKind::Transact => {
            ...
            reconstruct_transact_from_payload(
                source_instruction_tag,
                parent_payload,
                parent_accounts,
                &event,
        ...
        EventKind::Merge => {
            let event = MergeEvent::try_from_slice(body)
                .map_err(|_| ReconstructError::InvalidEventPayload)?;
            reconstruct_merge_from_payload(
                source_instruction_tag,
                parent_payload,
                parent_accounts,
                &event,
