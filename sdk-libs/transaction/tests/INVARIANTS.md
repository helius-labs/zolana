...
| INV-TX-ENC-06 | post / C | Owner signers exclude dummies, P256 owners and the payer, deduplicate owner identities, and preserve first occurrence; `program_signers` follow them, skipping the payer and repeats. `signer_pk_hashes` is payer-first, zero-padded to width, and returns `UnsupportedShape` if the run does not fit. | [x] `commitments.rs::signers_are_payer_first_deduplicated_by_identity_and_exclude_dummies_and_p256`, `commitments.rs::program_signers_follow_the_input_owners_without_duplicates`. |
