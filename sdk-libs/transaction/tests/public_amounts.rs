...
fn spl_transfer(mint: Address, is_deposit: bool, amount: u64, seed: u8) -> SettlementTransfer {
...
fn sol_transfer(is_deposit: bool, amount: u64, seed: u8) -> SettlementTransfer {
...
fn proof_inputs(interface_transfers: Vec<SettlementTransfer>) -> SppProofInputs {
...
fn four_active_assets_are_rejected() {
    let transfers = vec![
        sol_transfer(true, 1, 1),
        spl_transfer(Address::new_from_array([31; 32]), true, 2, 2),
        spl_transfer(Address::new_from_array([32; 32]), true, 3, 3),
        spl_transfer(Address::new_from_array([33; 32]), true, 4, 4),
    ];
...
fn cancelled_asset_still_counts_toward_the_slot_limit() {
    ...
    let transfers = vec![
        spl_transfer(cancelled, true, 9, 1),
        spl_transfer(first, true, 1, 2),
        spl_transfer(cancelled, false, 9, 3),
        spl_transfer(second, true, 2, 4),
        sol_transfer(false, 3, 5),
    ];
...
fn same_asset_sum_overflow_is_rejected() {
    ...
    let inputs = proof_inputs(vec![
        spl_transfer(mint, true, u64::MAX, 1),
...
fn transient_sum_above_u64_is_rejected_before_later_cancellation() {
    ...
    let inputs = proof_inputs(vec![
        spl_transfer(mint, true, u64::MAX, 1),
        spl_transfer(mint, true, u64::MAX, 2),
        spl_transfer(mint, false, u64::MAX, 3),
    ]);
...
#[test]
fn raw_external_data_rejects_zero_and_encoding_count_overflow() {
    let base = || ExternalData::new([0; 33], [0; 16], vec![], vec![], vec![]);
    assert_eq!(
        base().with_interface_transfers(vec![sol_transfer(true, 0, 1)]),
        Err(TransactionError::ZeroInterfaceTransferAmount)
    );
    assert!(base()
        .with_interface_transfers(vec![
            sol_transfer(true, 1, 1);
            zolana_interface::MAX_INTERFACE_TRANSFERS
        ])
        .is_ok());
    assert_eq!(
        base().with_interface_transfers(vec![
            sol_transfer(true, 1, 1);
            zolana_interface::MAX_INTERFACE_TRANSFERS + 1
        ]),
        Err(TransactionError::TooManyInterfaceTransfers {
            got: zolana_interface::MAX_INTERFACE_TRANSFERS + 1,
            max: zolana_interface::MAX_INTERFACE_TRANSFERS,
        })
    );
    assert_eq!(
        base().with_interface_transfers(vec![spl_transfer(SOL_MINT, true, 1, 1)]),
