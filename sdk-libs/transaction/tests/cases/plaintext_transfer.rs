...
pub(crate) fn owner_tag(kp: &ShieldedKeypair) -> [u8; 32] {
    kp.signing_pubkey().as_p256().unwrap().x()
}

pub(crate) fn build(world: &mut TransactionWorld, sender: String, r1: String, r2: String) {
...
pub(crate) fn round_trips(world: &mut TransactionWorld) {
...
pub(crate) fn sequential_blindings(world: &mut TransactionWorld) {
    let outputs = world
        .plaintext_transfer
        .clone()
        .unwrap()
...
pub(crate) fn sender_indexed(world: &mut TransactionWorld, name: String) {
    let indexed = world
        .plaintext_transfer
        .clone()
        .unwrap()
...
pub(crate) fn recipient_indexed(world: &mut TransactionWorld, idx: usize, name: String) {
    let indexed = world
        .plaintext_transfer
        .clone()
        .unwrap()
...
pub(crate) fn output_amounts(world: &mut TransactionWorld, a: u64, b: u64, c: u64, d: u64) {
    let outputs = world
        .plaintext_transfer
        .clone()
        .unwrap()
...
pub(crate) fn rejects_bad_discriminator(world: &mut TransactionWorld) {
...
pub(crate) fn sender_data_without_output(world: &mut TransactionWorld, name: String) {
    let utxos = TransferPlaintextUtxos {
        type_prefix: TRANSFER_PLAINTEXT,
        blinding_seed: SEED,
        sender: Some(TransferPlaintextSender {
