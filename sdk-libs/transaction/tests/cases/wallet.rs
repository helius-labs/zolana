...
pub(crate) fn bootstrap_transfer(
    world: &mut TransactionWorld,
    amount: u64,
    sender: String,
    recipient: String,
...
pub(crate) fn spending_transfer(
    world: &mut TransactionWorld,
    amount: u64,
    sender: String,
    recipient: String,
...
pub(crate) fn recorded_split(world: &mut TransactionWorld, owner: String, parts: u8) {
    let assets = AssetRegistry::default();
...
pub(crate) fn sync_fresh_wallet(world: &mut TransactionWorld, name: String) {
...
pub(crate) fn unspent_sol_balance(world: &mut TransactionWorld, amount: u64) {
    let wallet = world.wallet.as_ref().expect("wallet not synced");
...
pub(crate) fn wallet_counts(world: &mut TransactionWorld, tx_count: u64, request_count: u64) {
    let wallet = world.wallet.as_ref().expect("wallet not synced");
    let entry = wallet
        .viewing_key_history
        .last()
...
pub(crate) fn knows_sender(world: &mut TransactionWorld, name: String, index: u64) {
...
pub(crate) fn knows_recipient(world: &mut TransactionWorld, name: String, index: u64) {
    let pubkey = world.kp(&name).viewing_pubkey();
    let wallet = world.wallet.as_ref().expect("wallet not synced");
    let entry = wallet
        .viewing_key_history
...
pub(crate) fn private_tx_count(world: &mut TransactionWorld, count: usize) {
    let wallet = world.wallet.as_ref().expect("wallet not synced");
    assert_eq!(wallet.private_transactions().len(), count);
}

pub(crate) fn inbound_from(world: &mut TransactionWorld, amount: u64, sender: String) {
    let wallet = world.wallet.as_ref().expect("wallet not synced");
...
pub(crate) fn outbound_to(world: &mut TransactionWorld, amount: u64, recipient: String) {
    let wallet = world.wallet.as_ref().expect("wallet not synced");
...
pub(crate) fn split_recorded(world: &mut TransactionWorld, amount: u64) {
    let wallet = world.wallet.as_ref().expect("wallet not synced");
    let found = wallet.private_transactions().iter().any(|tx| {
