...
/// Prove and submit an already signed shielded transaction.
///
/// Output-indexing waits intentionally remain a caller policy. This action ends
/// once the chain RPC accepts the transaction and returns its signature.
...
fn validate_withdrawal(
    ...
    match (
        ...
        // SPL's `recipient` account is not committed in ExternalData. Only the
        // token account and vault can be coupled to the signed transaction here.
...
/// Prove and submit an already prepared consolidation.
...
pub fn submit_merge_transaction<R: Rpc + ?Sized, I: Rpc + ?Sized>(
    ...
    let owner = request.funding.pubkey();
...
fn validate_merge_submission<R: Rpc + ?Sized>(
    ...
    if prepared.user_viewing_pk != address.viewing_pubkey {
        return Err(ClientError::SubmissionMismatch(format!(
            "merge viewing key does not match funding owner {owner}'s registry record"
        )));
    }
    if nullifier_key.pubkey()? != address.nullifier_pubkey {
        return Err(ClientError::SubmissionMismatch(format!(
            "merge nullifier key does not match funding owner {owner}'s registry record"
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    fn proof_error(result: Result<Vec<SpendProof>, ClientError>) -> ClientError {
        match result {
            Ok(_) => panic!("expected proof validation to fail"),
            Err(err) => err,
        }
    }
    ...
    fn private_submission_requires_matching_sol_withdrawal() {
        ...
        validate_private_submission(&signed, &funding, Some(&matching))
            .expect("matching SOL settlement");
        assert_submission_mismatch(validate_private_submission(&signed, &funding, None));

        let wrong_recipient = TransactWithdrawal::Sol(TransactSolWithdrawal {
            recipient: Pubkey::new_unique(),
        });
        assert_submission_mismatch(validate_private_submission(
            &signed,
            &funding,
            Some(&wrong_recipient),
        ));

        let mut noncanonical_external = signed.external_data.clone();
        noncanonical_external.user_spl_token = Address::new_unique();
        let noncanonical = signed_transaction(&funding, noncanonical_external);
        assert_submission_mismatch(validate_private_submission(
            &noncanonical,
            &funding,
            Some(&matching),
    ...
    #[test]
    fn private_submission_requires_matching_canonical_spl_withdrawal() {
        let funding = Keypair::new();
        ...
        validate_private_submission(&signed, &funding, Some(&matching))
            .expect("matching SPL settlement");

        let wrong_authority = TransactWithdrawal::Spl(TransactSplWithdrawal {
        ...
        let mut noncanonical_external = signed.external_data.clone();
        noncanonical_external.user_sol_account = Address::new_unique();
        let noncanonical = signed_transaction(&funding, noncanonical_external);
        assert_submission_mismatch(validate_private_submission(
            &noncanonical,
            &funding,
    ...
    #[test]
    fn merge_submission_accepts_registered_owner_material() {
        let owner = Pubkey::new_unique();
        let keypair = ShieldedKeypair::new().unwrap();
        let rpc = registry_rpc(owner, &keypair);
    ...
    #[test]
    fn merge_submission_rejects_unregistered_signing_key() {
        let owner = Pubkey::new_unique();
        let registered = ShieldedKeypair::new().unwrap();
        let unrelated = ShieldedKeypair::new().unwrap();
        let rpc = registry_rpc(owner, &registered);
        let mut prepared = prepared_merge(&registered);
        prepared.signing_pubkey = unrelated.signing_pubkey();
    ...
    #[test]
    fn merge_submission_rejects_unregistered_viewing_key() {
        let owner = Pubkey::new_unique();
        let registered = ShieldedKeypair::new().unwrap();
        let unrelated = ShieldedKeypair::new().unwrap();
        let rpc = registry_rpc(owner, &registered);
        let mut prepared = prepared_merge(&registered);
        prepared.user_viewing_pk = unrelated.viewing_pubkey();
    ...
    #[test]
    fn merge_submission_rejects_unregistered_nullifier_key() {
        let owner = Pubkey::new_unique();
        let registered = ShieldedKeypair::new().unwrap();
        let unrelated = ShieldedKeypair::new().unwrap();
        let rpc = registry_rpc(owner, &registered);
        let prepared = prepared_merge(&registered);
    ...
    #[test]
    fn spend_proofs_accept_matching_order_and_tree() {
    ...
    #[test]
    fn spend_proofs_reject_reordered_state_leaves() {
        let tree = Pubkey::new_unique();
        let tree_address = Address::new_from_array(tree.to_bytes());
        let commitments = [commitment(2, 1), commitment(5, 2)];
        let mut rpc = proof_rpc(tree_address, &commitments);
        rpc.state.swap(0, 1);

        let err = proof_error(spend_proofs(&rpc, tree, &commitments));

        assert!(err.to_string().contains("state proof for input 2"));
        assert!(err.to_string().contains("with leaf"));
    }

    #[test]
    fn spend_proofs_reject_reordered_nullifier_leaves() {
        let tree = Pubkey::new_unique();
        let tree_address = Address::new_from_array(tree.to_bytes());
        let commitments = [commitment(2, 1), commitment(5, 2)];
        let mut rpc = proof_rpc(tree_address, &commitments);
        rpc.nullifier.swap(0, 1);

        let err = proof_error(spend_proofs(&rpc, tree, &commitments));

        assert!(err.to_string().contains("nullifier proof for input 2"));
        assert!(err.to_string().contains("with leaf"));
    }

    #[test]
    fn spend_proofs_reject_state_proof_from_another_tree() {
        let tree = Pubkey::new_unique();
        let tree_address = Address::new_from_array(tree.to_bytes());
        let commitments = [commitment(3, 1)];
        let mut rpc = proof_rpc(tree_address, &commitments);
        rpc.state[0].merkle_context.tree = Address::new_unique();

        let err = proof_error(spend_proofs(&rpc, tree, &commitments));

        assert!(err.to_string().contains("state proof for input 3"));
        assert!(err.to_string().contains("from tree"));
    }

    #[test]
    fn spend_proofs_reject_nullifier_proof_from_another_tree() {
        let tree = Pubkey::new_unique();
        let tree_address = Address::new_from_array(tree.to_bytes());
        let commitments = [commitment(3, 1)];
        let mut rpc = proof_rpc(tree_address, &commitments);
        rpc.nullifier[0].merkle_context.tree = Address::new_unique();

        let err = proof_error(spend_proofs(&rpc, tree, &commitments));

        assert!(err.to_string().contains("nullifier proof for input 3"));
        assert!(err.to_string().contains("from tree"));
    }
}
