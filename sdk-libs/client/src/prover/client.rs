...
mod tests {
    ...
    fn poll_async_failed_status_errors() {
        ...
        assert_paths(&requests, ["/prove?wait=true", "/prove/status?job_id=job-failed"]);
    ...
    fn poll_async_rejects_malformed_status_body() {
        ...
        assert_paths(&requests, ["/prove?wait=true", "/prove/status?job_id=job-bad-json"]);
    ...
    fn poll_async_client_error_status_fails_fast() {
        ...
        assert_paths(&requests, ["/prove?wait=true", "/prove/status?job_id=missing-job"]);
        let message = err.to_string();
