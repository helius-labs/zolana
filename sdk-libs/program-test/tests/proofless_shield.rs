...
const INVALID_TRANSACT_SHAPE: u32 = 10;
const INVALID_SETTLEMENT_ACCOUNTS: u32 = 13;
const TREE_PAUSED: u32 = 17;
...
fn sol_deposit_succeeds_and_event_is_faithful() {
    ...
    let mut data = PoolTestRig::sol_shield_data(750_000_000, [7u8; 32]);
...
fn send_raw(
...
) -> Result<(), light_program_test::RigError> {
    ...
    rig.svm
        .send_transaction(tx)
        .map(|_| ())
        .map_err(|e| light_program_test::RigError::Litesvm(format!("send_transaction: {e:?}")))
}
