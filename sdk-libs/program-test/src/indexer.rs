...
use thiserror::Error;
use zolana_event::proofless_output;
use zolana_hasher::Poseidon;
use zolana_interface::event::GeneralEvent;
use zolana_interface::{output_data::encode_encrypted_ring_deposit_output, state::STATE_HEIGHT};
...
impl TestIndexer {
    ...
    pub fn record_ring_deposit(
        &mut self,
        event: &crate::RingDepositOutput,
    ) -> Result<&IndexedUtxo, IndexerError> {
        let record_index = self.utxos.len();
        self.append_output_slot(
            event.leaf_index,
            event.view_tag,
            event.utxo_hash,
            IndexedPayload::Encrypted(encode_encrypted_ring_deposit_output(event.output.clone())),
        )?;
        Ok(&self.utxos[record_index])
    }
...
fn proofless_utxo_hash(event: &crate::DepositOutput) -> Result<[u8; 32], TransactionError> {
    ...
    .with_ring(
        output.ring_data_hash.unwrap_or([0u8; 32]),
        &output.ring_program_id.map(Address::new_from_array),
    )?
    .hash()
}
