...
use zolana_transaction::{
    ...
    P256Signature, SppProofOutputUtxo, TransactionError, UtxoSerialization,
};
...
/// owner. Every slot is a real output, padding included, so every position
/// carries a ciphertext its owner can decrypt.
pub type EncryptedTransfer = EncryptedEnvelope<Vec<MessageData>>;
    ...
    T: SyncWalletAuthority + Send + Sync + ?Sized,
{
    ...
    async fn encrypt_confidential_transfer(
        ...
        SyncWalletAuthority::encrypt_confidential_transfer(self, first_nullifier, outputs)
    }
...
fn encrypt_confidential_transfer_with<K: ViewingKeyTrait>(
    ...
    let slots = outputs
        ...
        .map(|(slot, output)| {
            let owner = output
                .owner_address
                .ok_or(TransactionError::OutputWithoutOwner { slot_index: slot })?;
            let slot_index = u32::try_from(slot).map_err(|_| TransactionError::TooManyOutputs)?;
            ...
            if output.ring_program_id.is_some() {
                let zolana_event::OutputDataEncoding::Encrypted(mut blob) =
                    borsh::from_slice(&message.data)
...
impl SyncWalletAuthority for ClientEd25519WalletAuthority {
    ...
    fn encrypt_confidential_transfer(
        ...
        encrypt_confidential_transfer_with(&self.viewing_key, first_nullifier, outputs)
    }
    ...
    for KeypairWalletAuthority<'_, K>
{
    ...
    fn encrypt_confidential_transfer(
        ...
        encrypt_confidential_transfer_with(self.keypair, first_nullifier, outputs)
    }
...
impl SyncWalletAuthority for ShieldedKeypair {
    ...
    fn encrypt_confidential_transfer(
        ...
        encrypt_confidential_transfer_with(self, first_nullifier, outputs)
    }
