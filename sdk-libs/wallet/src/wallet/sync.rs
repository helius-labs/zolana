...
/// Where one decoded UTXO was published.
///
/// Gathered once per slot because a stored [`WalletUtxo`] records all of it
/// and none of it can be recovered afterwards -- least of all `tree_id`, which
/// the commitment folds in.
#[derive(Clone, Copy)]
struct SlotSite<'a> {
    tx: &'a ShieldedTransaction,
    output_context: &'a OutputContext,
    tree_id: u16,
    slot_index: u32,
}

impl SyncCtx<'_> {
    /// The site of the slot at `slot_index` in `tx`.
    fn slot_site<'a>(
        &mut self,
        tx: &'a ShieldedTransaction,
        slot_index: usize,
    ) -> Option<SlotSite<'a>> {
        let slot = tx.output_slots.get(slot_index)?;
        let slot_index = u32::try_from(slot_index).ok()?;
        Some(SlotSite {
            tx,
            output_context: &slot.output_context,
    ...
    fn store(
        ...
        let utxo_hash = site.output_context.hash;
        if self
            .utxos
            .iter()
            .any(|stored| stored.utxo_hash == utxo_hash)
        {
            return Ok(false);
        }
        let nullifier = utxo.nullifier(&utxo_hash, self.nullifier_key)?;
    ...
    /// Store a UTXO whose slot is found by commitment rather than by position:
    /// the sender rails (split, anonymous-sender change) decode one bundle and
    /// then look up which slot each recovered UTXO landed in. The commitment is
    /// recomputed per candidate slot, because it folds that slot's tree id in.
    fn store_in_tx(
        &mut self,
        utxo: Utxo,
        ...
        let mut site = None;
        // Slots of one transaction share a tree in practice, so the commitment
        // is hashed once and reused until the id actually changes.
        let mut hashed: Option<(u16, [u8; 32])> = None;
        for (position, slot) in tx.output_slots.iter().enumerate() {
            let slot_tree_id = slot.output_context.tree_id;
            let Ok(slot_index) = u32::try_from(position) else {
                continue;
            };
            let hash = match hashed {
                Some((tree_id, hash)) if tree_id == slot_tree_id => hash,
                _ => {
                    let hash =
                        utxo.hash(&self.nullifier_pk, &[0u8; 32], &[0u8; 32], slot_tree_id)?;
                    hashed = Some((slot_tree_id, hash));
                    hash
                }
            };
            if hash == slot.output_context.hash {
                site = Some(SlotSite {
                    tx,
                    output_context: &slot.output_context,
                    tree_id: slot_tree_id,
                    slot_index,
                });
                break;
            }
        ...
        self.store(utxo, site, None, None)
    }
    ...
    fn spent_amounts(&self, nullifiers: &[[u8; 32]]) -> HashMap<Address, u64> {
        ...
        {
            let entry = by_asset.entry(input_utxo.utxo.asset.asset).or_insert(0u64);
            *entry = entry.saturating_add(input_utxo.utxo.amount);
        }
        by_asset
    }
    ...
    fn record_outbound_transfer(
        ...
        for utxo in change {
            if let Some(total) = by_asset.get_mut(&utxo.asset.asset) {
    ...
    fn record_self_transfer(&mut self, tx: &ShieldedTransaction, spent: HashMap<Address, u64>) {
    ...
    fn store_recipient_utxos(
        ...
        for utxo in utxos {
            ...
            if hash != site.output_context.hash {
                self.report.undecryptable_candidates += 1;
                continue;
            }
            self.store(utxo, site, data_hash, ring_data_hash)?;
            stored = true;
        }
        Ok(stored)
    }

    /// Remember what a decode could not look up, so the client sync layer can
    /// backfill the registry and retry. `resolve()` is the only source of
    /// `UnknownAsset`, so this is the single seam where a stale asset registry
    /// surfaces during decode.
    fn note_unresolved(&mut self, err: &TransactionError) {
        match err {
            TransactionError::UnknownAsset(asset_id) => {
                self.report.unknown_asset_ids.insert(*asset_id);
            }
    ...
    /// an undecryptable candidate and remembers anything a registry could not
    /// resolve.
    fn note_undecryptable(&mut self, err: &TransactionError) {
        self.note_unresolved(err);
        self.report.undecryptable_candidates += 1;
    }
    ...
    fn record_confidential_send(
        ...
        if recipient_pks.is_empty()
            && change.iter().filter(|utxo| utxo.amount > 0).count() > 1
            && spent.iter().all(|(asset, amount)| {
                change
                    .iter()
        ...
        {
            self.record_self_transfer(tx, spent);
            return Ok(());
        }
    ...
    pub(super) fn decode_slot(
        ...
        let encrypted_slot_index = slot_site.slot_index;
            ...
            OutputDataEncoding::Encrypted(blob) => {
                ...
                match scheme {
                    ...
                    EncryptedScheme::AnonymousRecipient => {
                        ...
                        if self.store_recipient_utxos(utxos.clone(), slot_site, None, None)? {
                            self.processed_slots.insert(site);
    ...
    pub(super) fn scan<F: ProbeFanout>(
        ...
        for entry in self.viewing_key_history.iter_mut() {
            ...
            if let Some(sites) = index.recipient_sites.get(&bootstrap) {
                for site in sites {
                    let outcome = ctx.decode_slot(transactions, key, *site)?;
            ...
            if let Some(sites) = index.recipient_sites.get(&owner_tag) {
                for site in sites {
                    ctx.decode_slot(transactions, key, *site)?;
                }
            }
            if let Some(txs) = index.sender_sites.get(&owner_tag) {
                for &t in txs {
                    ctx.decode_slot(transactions, key, (t, 0))?;
                }
            }
            ...
            for (_, sites) in &sender_hits {
                for &t in sites {
                    let outcome = ctx.decode_slot(transactions, key, (t, 0))?;
            ...
            for (_, sites) in &request_hits {
                for site in sites {
                    if let Some(sender) = ctx.decode_slot(transactions, key, *site)?.sender {
                        known_senders.entry(sender).or_insert(0);
                    }
                }
            }
            ...
            for (t, tx) in transactions.iter().enumerate() {
                ctx.record_confidential_send(tx, t, key, known_recipients)?;
            }
        ...
        // The nullifier set is the only record of a spend; `Wallet::is_spent`
        // reads it directly, so there is no per-UTXO flag to reconcile.
