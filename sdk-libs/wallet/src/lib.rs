...
use zolana_client::{
    CreatePrivateWalletInput, DecryptionMode, GetDepositInstructionInput,
    ...
    SendPrivateTransferResult, SetDecryptionModeInput, SyncReport, WalletError,
};
...
pub struct DemoWalletEnvironment {
    provider: InMemoryPrivacyProvider,
}

impl DemoWalletEnvironment {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn native_client(&self, owner: Address) -> Result<PrivacyClient> {
        Ok(PrivacyClient::new_with_test_provider(
            owner,
            MockHost::new(ShieldedKeypair::new()?)?,
            self.provider.clone(),
        ))
    }
...
impl DemoWallet {
    pub fn new(name: &str, environment: DemoWalletEnvironment) -> Result<Self> {
        Self::with_owner(owner_from_name(name), environment)
    }

    pub fn with_owner(owner: Address, environment: DemoWalletEnvironment) -> Result<Self> {
    ...
    pub fn with_test_keypair(
        ...
        environment: DemoWalletEnvironment,
    ) -> Result<Self> {
