...
use zolana_interface::{
    instruction::{
        encode_instruction, tag, TransactData, PUBLIC_AMOUNT_DEPOSIT, PUBLIC_AMOUNT_NONE,
...
fn parse_pocket_submit_args(default_tx_name: &str, args: &[String]) -> Result<PocketSubmitOptions> {
    ...
    if opts.bundle.is_none() {
        ...
        if default_tx_name == "transfer"
            && (opts.recipient_wallet.is_none() || opts.recipient_state.is_none())
        {
            bail!("transfer direct proving requires --recipient-wallet and --recipient-state");
        }
    }
    Ok(opts)
}
...
fn pocket_submit(command: &str, opts: PocketSubmitOptions) -> Result<()> {
    ...
    let client = pocket_rpc_client(&opts.rpc_url);
    let signature = send_pocket_instructions(&client, &payer, &[ix])?;
...
fn pocket_build_direct_proof(
    ...
    let request_tx = match command {
        "shield" => {
            ...
            let (utxo, nullifier_secret) = pocket_new_utxo(&payer.pubkey(), asset_id, amount);
            output_targets.push(PocketOutputTarget {
                recipient: PocketOutputRecipient::Sender,
                nullifier_secret,
                owner_pubkey: payer.pubkey().to_string(),
            });
        ...
        "unshield" => {
            ...
            let selected = select_pocket_note(&sender_state, Some(asset_id), amount)?;
            let input_amount = pocket_note_amount(&selected.note)?;
            if input_amount != amount {
                bail!(
                    "unshield currently requires an exact note amount: selected {input_amount}, requested {amount}"
                );
            }
            spent_note_id = Some(selected.note.id.clone());
            pocket_request_tx(PocketRequestTxParams {
...
fn pocket_transact_data(tx: &PocketProofTx) -> Result<TransactData> {
    Ok(TransactData {
        ...
        public_spl_asset_id: tx.public_spl_asset_id,
