...
//! only on `zolana-hasher` and `zolana-interface`, and builds for the SBF
//! target, so a program that consumes SPP transactions through CPI can
//! recompute the values the proof derives instead of trusting instruction data.
//!
//! What belongs here: derivations shared between an SPP proof and the program
//! that verifies its effects, such as the blinding seed derivations in
