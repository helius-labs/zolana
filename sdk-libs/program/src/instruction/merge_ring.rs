...
use super::{merge_transact::CacheWriteAccounts, transact::nullifier_pda_accounts};
...
mod tests {
    ...
    fn expected_accounts(builder: &MergeRing, auth_signer: bool) -> Vec<AccountMeta> {
        ...
        if let Some(CacheWriteAccounts { cache, writer }) = builder.cache {
    ...
    fn cpi_instruction_marks_ring_auth_signer() {
        ...
        let builder = MergeRing {
            ...
            cache: Some(CacheWriteAccounts {
                cache: Pubkey::new_unique(),
                writer: Pubkey::new_unique(),
            }),
        };
        ...
        assert_eq!(ix.accounts.len(), 6 + 8 + 2);
        assert_eq!(
