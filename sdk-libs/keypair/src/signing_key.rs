...
impl SigningKey {
    ...
    pub fn from_p256_bytes(bytes: &[u8; 32]) -> Result<Self, Error> {
        let sk = SecretKey::from_slice(bytes).map_err(|_| Error::InvalidSecretKey)?;
        Ok(Self::P256(sk))
    }
    ...
    pub fn secret_bytes(&self) -> Zeroizing<[u8; 32]> {
        let mut out = [0u8; 32];
        match self {
            Self::P256(sk) => out.copy_from_slice(sk.to_bytes().as_slice()),
    ...
    pub fn signing_pubkey(&self) -> PublicKey {
        match self {
            Self::P256(sk) => PublicKey::from_p256(&P256Pubkey::from_p256(&sk.public_key())),
            Self::Ed25519(kp) => {
                let kb = kp.to_bytes();
    ...
    pub fn sign(&self, msg: &[u8]) -> [u8; 64] {
        ...
        match self {
            Self::P256(sk) => {
                let signing = EcdsaSigningKey::from_bytes(&sk.to_bytes())
    ...
    pub fn verify(&self, msg: &[u8], sig: &[u8; 64]) -> bool {
        match self {
            ...
            Self::Ed25519(_) => match self.signing_pubkey().as_ed25519() {
