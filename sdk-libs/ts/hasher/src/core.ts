 ...
 * lets the inlined entry point and the file-loading one share a single
 * instance. Only the loading differs; the digests cannot.
 *
 ...
 * does the work and the rest await it, so an inlined and a file-loaded entry
 * point in one graph converge on one instance rather than two. A failed load is
 * not cached, so a caller can retry.
 */
