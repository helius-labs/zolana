...
function withdrawal(
  ...
  const userTokenAccount = associatedTokenAddress(recipient, asset);
  const splTokenInterface = splAssetVaultAddress(asset);
  return {
...
export async function createTransfer(
  ...
  try {
    ...
    if (registered === undefined) {
      const created = createWithdrawal(params);
      return Object.freeze({
        transaction: created.transaction,
        recipient: {
          kind: "publicWithdrawal" as const,
    ...
    return Object.freeze({
      transaction: new UnsignedPrivateTransaction({
        ...
        summary: `private transaction transfer of ${String(params.amount)} to ${params.recipient}`,
      }),
