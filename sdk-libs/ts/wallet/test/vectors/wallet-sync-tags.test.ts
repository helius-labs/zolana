...
import type { WalletSyncMaterial } from "@zolana/transaction";
...
import type { WalletAuthority } from "../../src/index.js";
import { syncWallet } from "../../src/index.js";
...
function recorder(): Readonly<{
  ...
  return { indexer, tags, shielded, deposits };
}
...
describe("wallet sync tag vectors", () => {
  ...
  for (const item of fixture.cases as readonly Case[]) {
    it(`asks for the same tags as Rust: ${item.id}`, async () => {
      ...
      const { indexer, tags, shielded, deposits } = recorder();
