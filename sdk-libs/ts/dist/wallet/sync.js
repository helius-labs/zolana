...
import { DEFAULT_INDEXER_POLL_CONFIG, } from "../client/retry.js";
...
import {} from "../transaction/wallet/state.js";
...
export async function backfillAssetRegistry(wallet, registryRpc, context) {
    let accounts;
    try {
        accounts = await runKitRpc("getProgramAccounts", context, (abortSignal) => registryRpc.solanaRpc
            ...
            filters: [
                { dataSize: 48n },
                {
                    memcmp: {
                        offset: 0n,
                        encoding: "base64",
                        bytes: base64Decoder.decode(Uint8Array.of(StateDiscriminator.splAssetRegistry)),
                    },
                },
            ],
        })
    ...
    for (const { account } of accounts) {
        if (account.owner !== SHIELDED_POOL_PROGRAM_ID)
            continue;
        try {
            const registry = decodeSplAssetRegistry(new Uint8Array(base64Encoder.encode(account.data[0])));
...
function sameIdentity(material, wallet) {
    return (bytesKey(material.identity.signingPublicKey.toBytes()) ===
 ...
 * mismatch is noticed.
 */
function walletQueryTags(wallet, material) {
...
function walletQueryNullifiers(wallet) {
...
function walletHasUnknownMint(wallet) {
 ...
 * can still read.
 */
function isDecodablePayload(payload) {
    try {
...
function compareBySlotThenSignature(left, right) {
 ...
 */
function shieldedTransactionKey(transaction) {
    return [
        transaction.txSignature,
        ...transaction.outputSlots.map(({ outputContext }) => `${outputContext.tree}:${String(outputContext.leafIndex)}:${bytesKey(outputContext.hash)}`),
        ...transaction.nullifiers.map(bytesKey),
    ].join("|");
}
function checkedNextCursor(method, cursor, seen) {
    if (cursor === undefined)
        return undefined;
 ...
 */
function compareDeposits(left, right) {
...
async function collectShieldedTransactions(input, context) {
    let cursor;
    const seenCursors = new Set();
    do {
        const response = await input.indexer.getShieldedTransactionsByTags({ tags: input.chunk, limit: input.pageLimit, ...(cursor === undefined ? {} : { cursor }) }, input.rpcConfig, context);
        for (const transaction of response.transactions) {
            ...
            if (!input.out.has(key))
                input.out.set(key, transaction);
        }
        cursor = checkedNextCursor("getShieldedTransactionsByTags", response.nextCursor, seenCursors);
    } while (cursor !== undefined);
}
async function collectProoflessDeposits(input, context) {
    let cursor;
    const seenCursors = new Set();
    do {
        const response = await input.indexer.getEncryptedUtxosByTags({ tags: input.chunk, limit: input.pageLimit, ...(cursor === undefined ? {} : { cursor }) }, input.rpcConfig, context);
        for (const match of response.matches) {
            ...
            if (input.out.has(key))
                continue;
            if (!isDecodablePayload(match.outputSlot.payload))
                continue;
            input.out.set(key, Object.freeze({
                slot: match.slot,
...
async function collectShieldedTransactionsByNullifiers(input, context) {
    let cursor;
    const seenCursors = new Set();
    do {
        ...
        cursor = checkedNextCursor("getShieldedTransactionsByNullifiers", response.nextCursor, seenCursors);
    } while (cursor !== undefined);
}
export async function syncWallet(input, context) {
    try {
        ...
        for (let offset = 0; offset < tags.length; offset += chunkSize) {
            ...
            await collectShieldedTransactions({ indexer: input.client, chunk, pageLimit, rpcConfig, out: transactions }, context);
            await collectProoflessDeposits({ indexer: input.client, chunk, pageLimit, rpcConfig, out: deposits }, context);
        }
        const queriedNullifiers = new Set();
        const initialNullifiers = walletQueryNullifiers(input.wallet);
        for (const nullifier of initialNullifiers)
            queriedNullifiers.add(bytesKey(nullifier));
        ...
        if (report.unknownAssetIds.length > 0 ||
        ...
        const followUpNullifiers = walletQueryNullifiers(input.wallet).filter((nullifier) => !queriedNullifiers.has(bytesKey(nullifier)));
...
export function getPrivateTokenBalances(wallet) {
    return wallet.balances(true);
}
export function getPrivateTransactions(wallet) {
