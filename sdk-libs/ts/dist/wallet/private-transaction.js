...
import { ConfidentialTransfer } from "../transaction/instructions/transact.js";
...
function sameOptionalHash(left, right) {
...
function sameData(left, right) {
    ...
    return (records.length === other.length &&
...
function sameUtxo(left, right) {
    return (equalBytes(left.owner.toBytes(), right.owner.toBytes()) &&
 ...
 */
function matchingInput(wallet, tree, expected) {
    return wallet
        ._state()
        .utxos.some((entry) => !entry.spent &&
...
/** @internal */
export async function authorizePrivateTransaction(transaction, wallet, authority) {
    ...
    const inputs = unsignedInputs.map(({ entry }) => new ProofInputUtxo({
        utxo: entry.utxo,
        nullifierKey,
    ...
    if (action.kind === "split") {
        ...
        if (input === undefined)
            throw new WalletError("WALLET_NO_INPUTS");
