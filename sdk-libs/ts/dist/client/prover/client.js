...
import { checkedServiceUrl, composeSignal, requestError, sleep, } from "../internal.js";
import { circuitUtxo } from "./assembly.js";
import { parseProof } from "./proof.js";
const MAX_RESPONSE_BYTES = 1024 * 1024;
const MAX_ATTEMPTS = 3;
const RETRY_DELAY_MS = 2000n;
...
const DEFAULT_ASYNC_POLL_CONFIG = Object.freeze({
...
export class ProverClient {
    ...
    async #send(body, context) {
        const signal = composeSignal(context, "prove");
        try {
            for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
                if (attempt > 1)
                    await sleep(RETRY_DELAY_MS, { signal: signal.signal });
                const request = composeSignal({ signal: signal.signal, timeoutMs: REQUEST_TIMEOUT_MS }, "prove");
                try {
                    let response;
                    try {
                        ...
                        if (signal.signal.aborted)
                            throw requestError("prove", signal);
                        if (attempt < MAX_ATTEMPTS)
                            continue;
                        if (request.timedOut())
                            throw requestError("prove", request);
    ...
    async #poll(jobId, signal) {
        ...
        for (;;) {
            ...
            if (remaining === 0)
                timeout();
            const request = composeSignal({ signal: signal.signal, timeoutMs: Math.min(REQUEST_TIMEOUT_MS, remaining) }, "proveStatus");
            let response;
            try {
                try {
                ...
                try {
                    ...
                    if (!(error instanceof ClientError && error.code === "CLIENT_PROVER_TEXT"))
                        throw error;
                    await waitOrTimeout();
                    continue;
                }
...
function mergeProverRequest(inputs) {
...
function asyncPollConfig(input) {
    if (input === undefined)
        return DEFAULT_ASYNC_POLL_CONFIG;
