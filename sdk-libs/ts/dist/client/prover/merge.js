...
import { bigintToBytes, bytesField, bytesToBigInt, checkedBytes, field, hashChain, poseidon, } from "../internal.js";
import { createDummyTransferInput, createOutput, createRealInput, validateSpendProof, } from "./assembly.js";
const MERGE_INSTRUCTION_TAG = 13;
export async function assembleMerge(prepared, material, indexer, tree, context) {
    try {
...
export function assembleMergeWithProofs(prepared, material, proofs, tree, dummyNullifierProofs = []) {
    try {
...
function assembleMergeUnchecked(prepared, material, proofs, dummyNullifierProofs, tree) {
    validateMergeMaterial(prepared, material);
    ...
    for (const input of prepared.inputs) {
        ...
        const ownerPublicKeyHash = input.utxo.owner.signatureType() === "p256"
            ? 0n
    ...
    const privateTxHash = bigintToBytes(poseidon([
    ...
    const ownerPublicKeyHash = bytesField(prepared.signingPublicKey.ownerPublicKeyField(), "merge owner public key");
    ...
    const publicInputHash = bigintToBytes(hashChain([...commonPublicInputs, ownerPublicKeyHash]));
    const proverInputs = Object.freeze({
        inputs: Object.freeze(inputs),
        output,
        ownerPublicKeyHash: asField(ownerPublicKeyHash),
        userNullifierPublicKey: asField(bytesField(material.nullifierKey.publicKey(), "merge nullifier public key")),
        userNullifierSecret: asField(bytesField(material.nullifierKey.secretBytes(), "merge nullifier secret")),
    ...
    const instructionData = (proof) => Object.freeze({
        expiryUnixTs: prepared.expiryUnixTs,
        proof: copyMergeProof(proof),
        outputUtxoHash: new Uint8Array(outputHash),
        eddsaOwner,
        privateTxHash: new Uint8Array(privateTxHash),
    ...
    return Object.freeze({
        ...
        outputHash: new Uint8Array(outputHash),
        nullifiers: Object.freeze(nullifiers.map((nullifier) => new Uint8Array(nullifier))),
        utxoTreeRootIndexes,
        nullifierTreeRootIndexes,
        privateTxHash: new Uint8Array(privateTxHash),
...
function validateMergeMaterial(prepared, material) {
    if (!(prepared instanceof PreparedMerge))
        throw new ClientError("CLIENT_INVALID_MERGE");
    if (!(material.signingPublicKey instanceof ShieldedPublicKey) ||
...
function asField(value) {
    return field(value, "merge field");
}
