...
import { Reader, Writer, copyBytes, encodeBase58, fail, sha256, unsignedBigint, } from "../internal.js";
function encoded(value, write, size) {
...
function byteVector(writer, value, name) {
    writer.u16(value.length, `${name}.length`).bytes(value);
}
function writeDepositData(writer, value) {
...
export function encodeDepositInstructionData(value) {
    return encoded(value, writeDepositData);
}
export function encodeAddressTreeParams(value) {
...
function writeInput(writer, value) {
    writer
...
function writeOwnerTag(writer, value) {
    switch (value.kind) {
        case "inline":
...
function writeCircuit(writer, value) {
...
function writeInterfaceTransfer(writer, value) {
    const tag = value.kind === "solDeposit"
        ? 0
...
function writeOutput(writer, value) {
...
function writeTransactData(writer, value) {
    ...
    for (const transfer of value.interfaceTransfers)
        writeInterfaceTransfer(writer, transfer);
    writer
...
export function encodeTransactInstructionData(value) {
    return encoded(value, writeTransactData);
}
function writeMergeData(writer, value) {
...
export function encodeMergeTransactInstructionData(value) {
    return encoded(value, writeMergeData, 492);
}
export function mergeExternalDataHash(input) {
    const expiry = unsignedBigint(input.expiryUnixTs, (1n << 64n) - 1n, "expiryUnixTs");
    const writer = new Writer()
        .u8(input.instructionTag, "instructionTag")
        .bytes(Uint8Array.from({ length: 8 }, (_, index) => Number((expiry >> BigInt((7 - index) * 8)) & 255n)))
        .bytes(input.outputUtxoHash, 32, "outputUtxoHash");
    const digest = sha256(writer.finish());
    digest[0] = 0;
    return digest;
}
function decodeAccount(bytes, size, discriminator, decode) {
...
function readAddress(reader, name) {
    return encodeBase58(reader.bytes(32, name));
}
export function decodeProtocolConfigAccount(bytes) {
    return decodeAccount(bytes, 132, StateDiscriminator.protocolConfig, (reader) => ({
        ...
        splInterfaceCreationIsPermissionless: reader.nonzeroBool("splInterfaceCreationIsPermissionless"),
    }));
}
export function decodeSplAssetCounterAccount(bytes) {
...
export function decodeSplAssetRegistryAccount(bytes) {
...
export function decodeZoneConfigAccount(bytes) {
