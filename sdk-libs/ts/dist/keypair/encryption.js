...
export function ecdhX(secret, counterparty) {
    try {
...
function deriveKeyNonce(dh, ephemeralPublicKey, recipientPublicKey, salt, slotIndex) {
    const ikm = concatBytes(dh, ephemeralPublicKey.toBytes(), recipientPublicKey.toBytes());
    const info = concatBytes(encoder.encode(HPKE_PREFIX), encoder.encode(ENC_INFO_TRANSFER), salt, u32be(slotIndex));
    try {
...
export function applyTransferCipher(secret, counterparty, ephemeralPublicKey, recipientPublicKey, input, salt, slotIndex) {
