...
/**
 * Length of the serialized transaction: the compact-u16 signature count, the
 * signatures, then the message.
 */
export function transactionSize(transaction: Transaction): number {
  const count = transaction.signatures.length;
  const prefix = count < 0x80 ? 1 : count < 0x4000 ? 2 : 3;
  return prefix + count * SIGNATURE_LENGTH + transaction.messageBytes.length;
}
