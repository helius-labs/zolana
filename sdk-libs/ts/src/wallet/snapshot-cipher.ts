 ...
 * 32 bytes of key material the caller obtained instead. Consumes
 * `consumeSecret`, wiping it on both success and failure; pass a copy if
 * the caller needs to retain its buffer.
 */
export async function walletSnapshotKey(consumeSecret: Uint8Array): Promise<CryptoKey> {
  if (consumeSecret.length !== 32) {
    consumeSecret.fill(0);
    throw new WalletError("WALLET_SNAPSHOT", { details: { field: "secret" } });
  }
  const ikmBytes = new Uint8Array(consumeSecret);
  try {
