...
import type { IndexedDepositClient, Prover, RingKeyRegistryReader } from "../client/ports.js";
...
export async function buildRingDepositTransaction(
  ...
  try {
    return await withProofDataRetry(
      ...
      (attempt) => buildRingDepositOnce(input, attempt),
      context,
    );
  } catch (cause) {
...
async function buildRingDepositOnce(
  input: RingDepositTransactionParams,
  context?: RequestContext,
): Promise<Transaction> {
