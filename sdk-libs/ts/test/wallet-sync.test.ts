...
describe("wallet sync", () => {
  ...
  it("aborts an exhausted memo round budget without committing wallet state", async () => {
    const { keypair, keys, split, merge } = mergeAfterSplit();
    const wallet = new Wallet({ identity: keypair.shieldedAddress() });
    const before = serializeWallet(wallet);
    // Fault injection at the memo boundary: a correct holder cannot prevent
    // KeyMemo from caching, but a broken memo must still terminate safely.
    const pending = vi.spyOn(KeyMemo.prototype, "pending").mockReturnValue(true);
    const resolve = vi.spyOn(KeyMemo.prototype, "resolve").mockResolvedValue();
    const destroy = vi.spyOn(KeyMemo.prototype, "destroy");
    try {
      await expect(
        decryptWithKeys(keys, { wallet, transactions: [split, merge] }),
      ).rejects.toMatchObject({ code: "TRANSACTION_KEYS_UNRESOLVED" });
