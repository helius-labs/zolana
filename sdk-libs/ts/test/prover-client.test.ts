...
describe("compressed ring prover contracts", () => {
  it.each(["compressed", "delegate"] as const)(
    ...
    async (kind) => {
      const fetch = vi.fn<typeof globalThis.fetch>(async () => Response.json(STANDARD_PROOF));
      ...
      await expect(prove()).resolves.toEqual(parseProof(STANDARD_PROOF));
  ...
  it("serializes the key registration like Rust `RegisterKeyProofRequest::body`", async () => {
    ...
    const fetch = vi.fn<typeof globalThis.fetch>(async () => Response.json(STANDARD_PROOF));
    const prover = new ProverClient({ url: "https://prover.example", fetch });
    await expect(prover.proveCustomRingRegisterKey(request)).resolves.toEqual(
      parseProof(STANDARD_PROOF),
    );
    fetch.mockResolvedValueOnce(Response.json({ ar: [] }));
