...
export function sha256(input: Uint8Array): Uint8Array {
  ...
  for (let index = 0; index < 8; index += 1) {
    message[message.length - 1 - index] = Number((bitLength >> BigInt(index * 8)) & 0xffn);
  }
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      const previous15 = readWord(words, index - 15);
      const previous2 = readWord(words, index - 2);
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h + sum1 + choice + readWord(ROUND_CONSTANTS, index) + readWord(words, index)) >>> 0;
  ...
  for (let index = 0; index < state.length; index += 1) {
    digestView.setUint32(index * 4, readWord(state, index), false);
  }
  return digest;
}
