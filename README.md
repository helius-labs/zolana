...
- `forester.yml` — compile/smoke (no infra) and bounded e2e + compressible (Redis + zolana CLI + Anza CLI)
...
(adds Go, Anza CLI, surfpool, Photon, zolana, and local SBF artifacts on top of setup-rust).
...
- `just build-zolana-cli`, `just install-surfpool`, `just install-photon`,
  `just build-prover-server`, and `just build-light-programs` for forester e2e / compressible tests
