...
pub(crate) fn restart_localnet() {
    ...
    // The transact flow reads the sender's user-registry record, so the
    ...
    // The CPI-forwarding fixture program signs its `CPI_SIGNER_PDA` when forwarding
    // a transact to SPP, so it must live in the same validator.
    let cpi_program_id = Pubkey::new_from_array(CPI_TEST_PROGRAM_ID).to_string();
    let cpi_program_so = format!("{root}/target/deploy/cpi_test_program.so");
