...
use crate::{actor::Actor, CpiLifecycleWorld};
...
impl CpiLifecycleWorld {
    ...
    /// eddsa rail, so the validator verifies the vanilla Groth16 proof.
...
fn fresh_pool(_world: &mut CpiLifecycleWorld) {
    // `CpiLifecycleWorld::new` already restarted the validator + Photon, started the
    // (persistent) prover, loaded the CPI fixture, and created the protocol config
    // and a tree. This step exists so features can name the precondition.
}

#[given(expr = "{word} with shielded solana keypair")]
fn shielded_solana_keypair(world: &mut CpiLifecycleWorld, name: String) {
