...
`test-squads`, `test-vk-registry`, and the tiers above them run per pull
request. The aggregate and nullifier-fold tiers prove against keys that are
neither published nor pinned by the lockfile, so they run locally and on the
manual `aggregate tiers` workflow, which samples the keys on the runner.
...
- The aggregate keys are unpublished and not in the lockfile, so `just
  ...
  constants they pin before it builds the programs. Setting
  `ZOLANA_AGGREGATE_NO_AUTOGEN=1` turns that into a failure naming the missing
  keys. The tier is behind the `aggregate` cargo feature and the
  `aggregate_keys` Go build tag, so no other recipe builds it.
...
- A test recipe must leave the tree clean, and `just check-tree-clean` gates
  ...
  them. `tools/ensure-generated-keys.sh` reverts that, and a cold run keeps its
  rewrite because the constant must match the key just sampled.
