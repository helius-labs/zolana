...
/// Test fixture pairing an interface transfer with the settlement addresses committed
...
/// `solana_owner_identity(resolved_owner_tag)`, one per output position. These
/// test fixtures use inline tags; account-backed tags require an account list
/// and are rejected here instead of being guessed.
pub fn output_owner_pk_hashes(outputs: &[TransactOutput]) -> Result<Vec<[u8; 32]>> {
    outputs
        ...
        .map(|output| {
            ...
            solana_owner_identity(&owner_tag).map_err(|e| anyhow!("owner identity: {e:?}"))
        })
        .collect()
}
...
pub fn external_data_hash_with_addresses(
    ...
    let (_, external_data_prefix) = TransactIxDataRef::parse_with_external_data_prefix(&bytes)
        .map_err(|e| anyhow!("parse instruction data: {e:?}"))?;
...
pub fn external_data_hash(
    ...
    addresses.extend(committed_owner_addresses(transact_ix_data)?);
...
pub fn committed_owner_addresses(ix: &TransactIxData) -> Result<Vec<[u8; 32]>> {
