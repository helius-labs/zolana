...
pub struct TransferProverInputsArgs {
    ...
    /// The transaction's single private random value. The circuit re-derives
    /// the output blinding seed and the private transaction blinding from it,
    /// so it must be the secret the outputs were blinded with
    /// ([`TEST_TX_SECRET`] for [`derive_test_output_blindings`]).
    pub tx_secret: [u8; 32],
...
/// The transaction secret every hand-built program-test fixture uses.
...
///
/// It must be non-zero: the prover rejects a zero secret
/// (`spp: txSecret must not be zero`,
/// `prover/server/prover/transfer_eddsa_only/marshal.go:157`), because a zero
/// secret makes the output blinding seed and the private transaction blinding
/// public functions of the first nullifier. The leading zero byte keeps the
/// value below the BN254 scalar modulus.
pub const TEST_TX_SECRET: [u8; 32] = [
...
/// `Poseidon(TXOS, first_nullifier, TEST_TX_SECRET)`, exactly as the circuit
/// recomputes it from the witness `tx_secret`.
...
/// `Poseidon(TXPB, first_nullifier, TEST_TX_SECRET)`.
...
/// transaction secret; fixtures use [`TEST_TX_SECRET`] for reproducibility
