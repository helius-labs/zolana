...
fn cached_merge_spend_e2e_benchmark() -> Result<()> {
    let config = zolana_test_utils::benchmark::BenchmarkConfig::from_env()?;
    for run in 1..=config.runs {
        let mut harness = LifecycleHarness::new()?;
        zolana_test_utils::benchmark::restart_prover()?;
        let mut warmup_ms = 0;
        if config.warm_keys {
            warmup_ms = harness.cached_merge_spend_benchmark(&config, run, "warmup", 0)?;
            harness = LifecycleHarness::new()?;
        }
        harness.cached_merge_spend_benchmark(&config, run, "measured", warmup_ms)?;
    }
    Ok(())
}
