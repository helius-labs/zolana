...
fn nullifier_test_forester_batches_queued_nullifiers_with_photon_indexer() -> TestResult {
    ...
    let mut spendable_utxos = VecDeque::new();
    ...
    for deposit_index in 0..2 {
        ...
        spendable_utxos.push_back(spendable_utxo);
    }
    ...
    for i in 0..queue_tx_count {
        ...
        let first_utxo = spendable_utxos
            .pop_front()
            .ok_or_else(|| anyhow!("missing first spendable UTXO for queue tx {i}"))?;
        let second_utxo = spendable_utxos
            .pop_front()
            .ok_or_else(|| anyhow!("missing second spendable UTXO for queue tx {i}"))?;
        queued_nullifiers.push(first_utxo.nullifier);
        queued_nullifiers.push(second_utxo.nullifier);

        let first_state_proof = wait_for_merkle_proof(&indexer, tree_address, first_utxo.hash)?;
        let second_state_proof = wait_for_merkle_proof(&indexer, tree_address, second_utxo.hash)?;
        let first_nullifier_proof =
            wait_for_non_inclusion_proof(&indexer, tree_address, first_utxo.nullifier)?;
        let second_nullifier_proof =
            wait_for_non_inclusion_proof(&indexer, tree_address, second_utxo.nullifier)?;
        assert_eq!(
        ...
        let mut transfer = ConfidentialTransfer::new(
            sender_address,
            vec![
                SppProofInputUtxo::new(first_utxo.utxo.clone(), &sender),
                SppProofInputUtxo::new(second_utxo.utxo.clone(), &sender),
            ],
            payer_address,
        );
        ...
        assert_eq!(commitments[0].nullifier, first_utxo.nullifier);
        assert_eq!(commitments[1].nullifier, second_utxo.nullifier);
        ...
        assert_eq!(
            change_utxo.hash, indexed.output_slots[1].output_context.hash,
            "decrypted SOL change UTXO should match output commitment"
        );
        assert_eq!(
            recipient_utxo.hash, indexed.output_slots[2].output_context.hash,
            "decrypted recipient UTXO should match output commitment"
        );
        spendable_utxos.push_back(change_utxo);
        spendable_utxos.push_back(recipient_utxo);
    }
