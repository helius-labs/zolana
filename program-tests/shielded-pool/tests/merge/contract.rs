...
fn merge_ix_data_at_input_count(input_count: usize) -> MergeTransactIxData {
    ...
    data.nullifiers.truncate(input_count);
    for extra in data.nullifiers.len()..input_count {
        data.nullifiers.push(fe(extra as u64 + 1));
    }
    data.utxo_tree_root_index.resize(input_count, 0);
    data.nullifier_tree_root_index.resize(input_count, 0);
    data
}
