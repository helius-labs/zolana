...
fn send_batch(
    ...
    label: &str,
) -> Result<(solana_signature::Signature, u64)> {
    let signature = fixture.rpc.create_and_send_transaction(
        instructions,
        fixture.owner.pubkey(),
        &[&fixture.owner],
        direct::COMPUTE_BUDGET,
    )?;
    let cu = assert_transaction_compute_units(&fixture.rpc, &signature, label, 1_400_000)?;
    Ok((signature, cu))
}
...
fn submit_batches(
    ...
    for batch in packed {
        ...
        let (signature, cu) = send_batch(fixture, &batch, "packed spend")?;
        submitted.signature = signature;
        submitted.total_cu += cu;
        submitted.last_cu = cu;
    }
    Ok(submitted)
}
...
fn direct_spend_e2e_benchmark() -> Result<()> {
    ...
    let mut preparation_cu = 0;
    let mut preparation_ms = 0;
    let mut preparation_transactions = 0;
    ...
    if prepared {
        ...
        let submitted = submit_batches(&mut fixture, batches)?;
        preparation_cu = submitted.total_cu;
        preparation_transactions = submitted.transactions;
    ...
    let indexed_transactions = fixture
        .indexer
        .get_shielded_transactions_by_signature(signature, None)?;
    let transactions = indexed_transactions
        .transactions
        .into_iter()
    ...
    println!(
        ...
        serde_json::json!({
            ...
            "preparation_cu": preparation_cu, "final_transaction_cu": submitted.last_cu, "total_cu": preparation_cu + submitted.total_cu,
            "transactions": preparation_transactions + submitted.transactions, "send_transactions": submitted.transactions,
        })
    );
    Ok(())
}
