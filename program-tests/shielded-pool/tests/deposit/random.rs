...
use crate::mollusk::{deposit_fixture, setup_mollusk};
...
proptest! {
    ...
    fn arbitrary_account_free_instruction_bytes_are_deterministic(
        ...
        for data in cases {
            let instruction = MolluskInstruction {
                program_id,
                accounts: Vec::new(),
                data,
            };
    ...
    #[test]
    fn valid_deposit_instruction_and_account_mutations_are_deterministic_and_atomic(
        ...
        for (kind, index, value) in mutations {
            ...
            match kind {
                ...
                6 => {
                    ...
                    if mollusk_solana_pubkey::Pubkey::new_from_array(wrong_owner)
                    ...
                    accounts.first_mut().expect("tree account").1.owner =
                        mollusk_solana_pubkey::Pubkey::new_from_array(wrong_owner);
                }
                7 => {
            ...
            if first.raw_result.is_err() {
                prop_assert_eq!(&first.resulting_accounts, &accounts);
            }
        }
    }
}
