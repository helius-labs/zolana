...
proptest! {
    ...
    /// Long mixed data/control-plane sequences prove atomic rollback and all
    /// conservation/nullifier invariants after every attempted transition.
    #[test]
    fn mixed_protocol_sequences_are_atomic(actions in prop::collection::vec(mixed_action(), 24..180)) {
        ...
        for (nonce, raw) in actions.iter().enumerate() {
            ...
            if result.is_err() {
                prop_assert_eq!(&backend.state, &before, "rejected action mutated state: {:?}", action);
            }
