...
fn register_spl_interface(world: &mut PoolWorld) {
    let mint = world.rig().create_mint().expect("create_mint");
...
fn assert_interface(world: &mut PoolWorld, registry_index: u64, vault_index: u64) {
    let mint = world.mint();
    ...
    assert_create_spl_interface(rig, &registry, &vault, &mint, registry_index, vault_index);
}

#[when(expr = "the authority registers the same SPL interface again")]
fn register_same_interface(world: &mut PoolWorld) {
...
fn register_second_interface(world: &mut PoolWorld) {
    let mint_b = world.rig().create_mint().expect("create_mint");
    let authority = world.authority().insecure_clone();
...
fn register_interface_non_authority(world: &mut PoolWorld) {
    let mint = world.rig().create_mint().expect("create_mint");
...
fn spl_depositor(world: &mut PoolWorld, balance: u64) {
...
fn spl_shield(world: &mut PoolWorld, amount: u64) {
    let tree = world.tree().pubkey();
    let mint = world.mint();
    let user_token = world.user_token();
    let vault = world.rig().spl_asset_vault_pda(&mint);
    let mut recipient =
    ...
    let vault_before = world.rig().token_balance(&vault).expect("vault balance");
...
fn spl_shield_foreign_token(world: &mut PoolWorld) {
    let tree = world.tree().pubkey();
    let mint = world.mint();
    let depositor = world.depositor().insecure_clone();
    ...
    world
        ...
        .expect("mint_to");
    let accounts = spl_accounts(world.rig(), &tree, &depositor.pubkey(), &other_token, &mint);
    let err = world
        .rig()
...
fn spl_shield_non_canonical_vault(world: &mut PoolWorld) {
    let tree = world.tree().pubkey();
    let mint = world.mint();
    let user_token = world.user_token();
    let depositor = world.depositor().insecure_clone();
    ...
    let decoy_vault = world
        ...
        .expect("decoy vault");
    let mut accounts = spl_accounts(world.rig(), &tree, &depositor.pubkey(), &user_token, &mint);
...
fn spl_shield_mint_mismatch(world: &mut PoolWorld) {
    let tree = world.tree().pubkey();
    let mint_a = world.mint();
    let depositor = world.depositor().insecure_clone();
    let mint_b = world.rig().create_mint().expect("mint B");
    let token_b = world
        .rig()
    ...
    world
        ...
        .expect("mint_to");
    let accounts = spl_accounts(world.rig(), &tree, &depositor.pubkey(), &token_b, &mint_a);
    let err = world
        .rig()
        .proofless_shield_with_accounts(
            accounts,
...
fn spl_shield_unaffordable(world: &mut PoolWorld, amount: u64) {
    let tree = world.tree().pubkey();
    let mint = world.mint();
    let user_token = world.user_token();
    let depositor = world.depositor().insecure_clone();
...
fn rejected_token_insufficient(world: &mut PoolWorld) {
