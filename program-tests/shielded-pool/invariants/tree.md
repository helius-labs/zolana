...
- [x] **INV-CLOSE-PDA-08: only the tree, the recipient, and the closed PDAs change**
  ...
  - Also covered by: `close_smart_account_batches_at_the_4096_byte_boundary` in the same file (real Squads execution with one member signature, the 128-account-lock feature, and a 64 KiB heap request; 108 closes serialize to 4,075 bytes, 109 to 4,109 bytes, so only 108 fit the assumed 4,096-byte wire limit. Both execute in LiteSVM, return all PDA rent, pay the exact reimbursement, and exhaust an exactly funded fee balance).
  - Kind: frame
