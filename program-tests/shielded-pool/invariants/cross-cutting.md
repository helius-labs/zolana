...
- [x] **INV-XC-03: every unknown tag is rejected**
  ...
  - Location: `programs/shielded-pool/src/lib.rs` (`fn process_instruction`), `program-libs/interface/src/instruction/tag.rs` (`impl TryFrom<u8> for InstructionTag`)
  - Error: `ProgramError::InvalidInstructionData`
  - Severity: Medium
...
- [x] **INV-XC-17: the owner tag encoding is bound, and an account-tagged owner's resolved address is appended**
...
- [x] **INV-XC-32: retired wire formats fail closed at decode, new wire formats are fixed-width**
  - Covered by: `program-libs/interface/src/instruction/instruction_data/transact.rs` unit `rejects_unknown_owner_tag_discriminant` (the retired `OwnerTag::P256SigningKey` discriminant 2 fails both decoders), and `program-libs/interface/tests/transact_ref.rs` (fixed-width `InputUtxo` / selector decode)
  - Kind: precondition
  - Affects: Transact, RingTransact, RingAuthorityTransact
