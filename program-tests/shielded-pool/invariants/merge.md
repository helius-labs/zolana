...
- [x] **INV-MERGE-06: a supported merge shape is enforced at parse time**
  - Covered by: `program-tests/shielded-pool/tests/merge/contract.rs` `merge_rejects_a_wrong_input_count_shape`; `program-libs/interface/src/instruction/instruction_data/merge_transact.rs` `every_supported_shape_has_the_contracted_encoded_length`, `rejects_wrong_shape`, `rejects_disagreeing_vector_lengths`
  - Kind: precondition
