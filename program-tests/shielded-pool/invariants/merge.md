...
- [x] **INV-MERGE-19: merge collects the forester fee for 8 queued nullifiers**
  - Covered by: `program-tests/spp-test-validator/tests/lifecycle.rs` `actor_owned_merge_covers_every_supported_input_count` and `eddsa_merge_covers_every_supported_input_count` via the shared merge action, which asserts the payer loses and the input tree gains exactly `MERGE_INPUT_COUNT` (8) × the 20-lamport per-element fee (`spp-test-validator/tests/actions/merge.rs:224-238`)
  - Kind: postcondition
