...
- [x] **INV-TRANSACT-42: the tree's insertion fee is collected from the payer per queued input and credited to the fee balance**
  ...
  - Location: `programs/shielded-pool/src/instructions/transact/tree.rs` (`fn apply_input_tree`, `credit_insertion_fee`), `transact/processor.rs` (`create_nullifier_pdas`), `nullifier_pda/create.rs` (`fn collect_forester_fee` transfers the whole fee once before any PDA is created, `fn create_nullifier_pdas` then assigns and resizes each PDA and tops up its rent from the tree), `program-libs/tree/src/fees.rs` (`fn TreeAccount::credit_insertion_fee`)
...
- [x] **INV-TRANSACT-33: dummy-slot proofs are locked out once the tree crosses the capacity threshold**
  ...
  - Location: `programs/shielded-pool/src/instructions/transact/tree.rs` (`fn apply_input_trees`), `program-libs/tree/src/lib.rs` (`fn dummy_input_headroom`); merge uses the same check in `programs/shielded-pool/src/instructions/merge/processor.rs`
  - Error: `ShieldedPoolError::TransactProofVerificationFailed = 7008`
...
- [x] **INV-RING-TRANSACT-02: ring_config must be a valid SPP-owned RingConfig**
  ...
  - Kind: precondition
  - Statement: the `ring_config` account must be owned by the shielded-pool program, have `data_len` exactly `RingConfig::SIZE` (69), and discriminator byte exactly 4; any violation returns Err.
