...
//!
//! The cache replaces state inclusion, so the spent UTXOs are never appended to
//! the tree: the fixture seats their commitments directly, the way a merge
//! would, and the spend publishes no UTXO root for the group. Everything else is
//! unchanged, so a cached spend still proves ownership and nullifier
//! non-inclusion.
...
use zolana_interface::{
    instruction::{
        instruction_data::{
            transact::{CachedInputs, CircuitId},
            CreateCacheData,
        },
        CreateCache, Transact,
    },
    pda,
    shape::Shape,
    state::{
        cache::{cached_input_fields, CacheAccount, CACHE_CAPACITY},
        discriminator::CACHE,
    },
    N_PUBLIC_SLOTS,
};
...
impl CachedSpendFixture {
    pub fn build(self, rpc: &mut ZolanaProgramTest, tree: Pubkey, tree_id: u16) -> CachedSpend {
        ...
        let [bitmap_field, cache_tree_id, cache_chain] = cached_inputs;
        prover_inputs.cache_input_bitmap = BigUint::from_bytes_be(&bitmap_field);
        prover_inputs.cache_tree_id = BigUint::from_bytes_be(&cache_tree_id);
        prover_inputs.cache_input_hash_chain = BigUint::from_bytes_be(&cache_chain);
    ...
    fn seat_cache(
        ...
        for (slot, commitment) in state.commitments.iter_mut().zip(commitments) {
            *slot = *commitment;
        }
...
/// Selects every one of `n_inputs` inputs. `1 << 64` is undefined, so the full
/// word is spelled out rather than shifted into.
fn full_bitmap(n_inputs: usize) -> u64 {
    if n_inputs >= u64::BITS as usize {
        u64::MAX
    } else {
        (1u64 << n_inputs) - 1
    }
