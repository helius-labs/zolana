...
impl RealMergeProof {
    pub fn build(self, pool: &mut Pool) -> RealMerge {
        ...
        // shielded address. A fixed nullifier secret keeps the run
        // deterministic; the blinding is derived on-chain from the leaf index
        // and read back from the indexer.
