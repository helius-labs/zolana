...
/// The same arrangement for the right fold, which the cached-commitment chain
...
fn right_hash_chain_4_folds_the_documented_examples() {
    let zero = [0u8; 32];
    let e: [[u8; 32]; 5] = core::array::from_fn(|index| field(index as u32 + 1));
    let [a, b, c, d, f] = &e;

    assert_eq!(
        create_right_hash_chain_4_from_slice(&e[..4]).unwrap(),
        Poseidon::hashv(&[a, b, c, d]).unwrap(),
        "four elements fold in one call, like the left chain"
    );
    assert_eq!(
        create_right_hash_chain_4_from_slice(&e).unwrap(),
        Poseidon::hashv(&[a, &zero, &zero, &Poseidon::hashv(&[b, c, d, f]).unwrap()]).unwrap(),
        "the short group is leftmost and its element stays left-aligned"
    );
    assert_eq!(
        create_right_hash_chain_4_from_slice(&e[..2]).unwrap(),
        Poseidon::hashv(&[a, &zero, &zero, b]).unwrap()
    );
    assert_eq!(
        create_right_hash_chain_4_from_slice(&e[..3]).unwrap(),
...
fn seeding_with_a_folded_zero_suffix_skips_its_groups() {
    ...
    for populated in 0..=8usize {
        ...
        for skipped in 0..=5usize {
            ...
            let split = inputs.len() - suffix_len;
            if inputs[split..].iter().any(|element| element != &zero) {
                continue;
            }
            let seed = create_right_hash_chain_4_from_slice(&inputs[split..]).unwrap();
            assert_eq!(
                create_right_hash_chain_4_from_seed(&inputs[..split], seed).unwrap(),
                create_right_hash_chain_4_from_slice(&inputs).unwrap(),
...
/// The fold direction is the whole point of the change, so pin exactly where
/// the two chains agree and where they part. They coincide only where the
/// convention forces them to: nothing to fold (0, 1) and the single full call
/// that both spell `Poseidon(e0, e1, e2, e3)` (4). At every other length the
/// padding sits at a different end and the digests differ, which is what makes
/// a proof built against the old convention fail rather than silently verify.
#[test]
