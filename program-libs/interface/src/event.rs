//! Events the shielded pool emits through the `EMIT_EVENT` self-CPI.
//!
//! An event instruction is `[EMIT_EVENT, EventKind, body]`. Deposits log a whole
//! [`GeneralEvent`]; `transact` and `merge` log only the positions execution
//! assigns and leave the rest to be rebuilt from the parent instruction.

use borsh::{BorshDeserialize, BorshSerialize};

use crate::instruction::{tag, MessageData};
...
///
/// Layout is `[EMIT_EVENT, kind, body]`. The body is little-endian, which is
/// exactly what Borsh writes for these all fixed-size structs, so the two
