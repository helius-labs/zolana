...
//! the stateless prepare syscalls from the compile-time VK constants. Its PDA
...
/// Prepared-blob backend byte; must match the blob headers the prepare
...
/// Prepared wire-blob size; must equal the fork's
...
/// G2 sources of a VK in the canonical registry order:
/// beta, gamma, delta, then for BSB22 keys commitment.g2 and
...
/// contents instead of relying on constant-promotion pointer identity. A
/// non-catalog key resolves to `None` and the caller fails closed.
