...
use wincode::{containers, io::Reader, len::FixIntLen, ReadError, SchemaRead, SchemaWrite};
...
impl InterfaceTransfer {
    ...
    pub const fn user_account_position(self) -> usize {
        match self {
            Self::SolDeposit { .. } | Self::SolWithdrawal { .. } => 1,
            Self::SplDeposit { .. } | Self::SplWithdrawal { .. } => 3,
        }
    }

    pub const fn spl_interface_account_position(self) -> Option<usize> {
        match self {
            Self::SolDeposit { .. } | Self::SolWithdrawal { .. } => None,
            Self::SplDeposit { .. } => Some(1),
            Self::SplWithdrawal { .. } => Some(2),
        }
    }
    ...
    pub const fn validate_amount(self) -> Result<(), ShieldedPoolError> {
        if self.amount() == 0 {
...
pub fn validate_interface_transfers(
    ...
    transfers
        .iter()
        .try_for_each(|transfer| transfer.validate_amount())
}
...
/// The fields of [`TransactIxData`] that `external_data_hash` commits to, in
/// their serialized order (spec: SPP `transact`). A wincode struct serializes
/// as its fields back to back, so this encodes to exactly the prefix of the
/// flat instruction data that the program hashes in place.
#[derive(Clone, Debug, PartialEq, Eq, SchemaRead, SchemaWrite)]
pub struct TransactExternalData {
    ...
    /// this transaction; copied verbatim into the reconstructed `GeneralEvent`.
    pub tx_viewing_pk: [u8; 33],
    /// Per-transaction encryption salt shared by every output ciphertext;
    /// copied into the reconstructed `GeneralEvent`.
    pub salt: [u8; 16],
...
///
/// The fields through `messages` are [`TransactExternalData`] and form the
/// contiguous prefix covered by `external_data_hash`; the borrowed view
/// [`TransactIxDataRef`] groups them under `external_data`.
#[derive(Clone, Debug, PartialEq, Eq, SchemaRead, SchemaWrite)]
...
/// Borrowed view of [`TransactExternalData`]. Byte arrays and payloads alias
/// the instruction buffer; only the small element vectors are read owned.
#[derive(Clone, Debug, PartialEq, Eq, SchemaRead)]
pub struct TransactExternalDataRef<'a> {
    pub expiry_unix_ts: u64,
    ...
    #[wincode(with = "containers::Vec<MessageDataRef<'a>, FixIntLen<u8>>")]
    pub messages: Vec<MessageDataRef<'a>>,
}

/// Borrowed view of [`TransactIxData`]. `external_data` is the prefix that
/// `external_data_hash` commits to; the remaining fields follow it on the wire.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct TransactIxDataRef<'a> {
    pub external_data: TransactExternalDataRef<'a>,
    pub private_tx_hash: &'a [u8; 32],
    pub circuit: CircuitId,
    pub proof: TransactProofRef<'a>,
    pub inputs: Vec<InputUtxoRef<'a>>,
}

#[derive(SchemaRead)]
struct TransactTailRef<'a> {
    private_tx_hash: &'a [u8; 32],
    circuit: CircuitId,
    proof: TransactProofRef<'a>,
    #[wincode(with = "containers::Vec<InputUtxoRef<'a>, FixIntLen<u8>>")]
    inputs: Vec<InputUtxoRef<'a>>,
}

impl<'a> TransactIxDataRef<'a> {
    /// Parse the instruction and return the exact serialized prefix covered by
    /// `external_data_hash`: the bytes `external_data` was read from.
    pub fn parse_with_external_data_prefix(data: &'a [u8]) -> Result<(Self, &'a [u8]), ReadError> {
        let mut cursor = data;
        let external_data =
            <TransactExternalDataRef<'a> as SchemaRead<'a, RefConfig>>::get(cursor.by_ref())?;
        let external_data_prefix = data
            .len()
        ...
        let tail = <TransactTailRef<'a> as SchemaRead<'a, RefConfig>>::get(cursor.by_ref())?;
        if !cursor.is_empty() {
            return Err(ReadError::TrailingBytes);
        }
        let parsed = Self {
            external_data,
            private_tx_hash: tail.private_tx_hash,
            circuit: tail.circuit,
            proof: tail.proof,
            inputs: tail.inputs,
        };
...
mod tests {
    ...
    fn assert_ref_matches_owned(view: &TransactIxDataRef, owned: &TransactIxData) {
        ...
        assert_eq!(external.expiry_unix_ts, owned.expiry_unix_ts);
        assert_eq!(view.private_tx_hash, &owned.private_tx_hash);
        assert_eq!(view.circuit, owned.circuit);
        assert_eq!(external.tx_viewing_pk, &owned.tx_viewing_pk);
        assert_eq!(external.salt, &owned.salt);
        ...
        for (got, want) in view.inputs.iter().zip(&owned.inputs) {
        ...
        assert_eq!(external.interface_transfers, owned.interface_transfers);
        assert_eq!(external.data_hash, owned.data_hash.as_ref());
        assert_eq!(external.ring_data_hash, owned.ring_data_hash.as_ref());
        assert_eq!(external.outputs.len(), owned.outputs.len());
        for (got, want) in external.outputs.iter().zip(&owned.outputs) {
        ...
        assert_eq!(external.messages.len(), owned.messages.len());
        for (got, want) in external.messages.iter().zip(&owned.messages) {
    ...
    #[test]
    fn external_data_struct_serializes_to_the_parsed_prefix() {
        let owned = ix_data(proof());
        let bytes = owned.serialize().expect("serialize");
        let (view, prefix) =
            TransactIxDataRef::parse_with_external_data_prefix(&bytes).expect("parse");
        let external = TransactExternalData {
            expiry_unix_ts: owned.expiry_unix_ts,
            tx_viewing_pk: owned.tx_viewing_pk,
            salt: owned.salt,
            interface_transfers: owned.interface_transfers.clone(),
            data_hash: owned.data_hash,
            ring_data_hash: owned.ring_data_hash,
            outputs: owned.outputs.clone(),
            messages: owned.messages.clone(),
        };
        assert_eq!(external.serialize().expect("serialize prefix"), prefix);
        assert_eq!(
            bytes.get(prefix.len()..prefix.len() + 32),
            Some(view.private_tx_hash.as_slice())
        );
    }
