...
pub use zolana_event::{OutputData, OutputUtxo};
...
/// resolves to the transaction-level shared P256 signing key. Wire tags are
...
pub struct TransactIxData {
    ...
    #[wincode(with = "containers::Vec<OutputData, FixIntLen<u8>>")]
...
/// (`TransactOutput::data`, `OutputData::data`) the owned structs write with
...
/// Borrowed view of a [`zolana_event::OutputData`]; `data` aliases the buffer.
#[derive(Clone, Copy, Debug, PartialEq, Eq, SchemaRead)]
...
pub enum FetchTagError {
    /// `Account(index)` referenced a raw account slot that does not exist.
    AccountMissing(u8),
    /// `P256SigningKey` was used but the transaction carries no
    /// `p256_signing_pk_x`.
...
) -> Result<[u8; 32], FetchTagError> {
    match tag {
        OwnerTag::Inline(bytes) => Ok(*bytes),
        OwnerTag::Account(index) => {
            account_address(*index).ok_or(FetchTagError::AccountMissing(*index))
        }
        OwnerTag::P256SigningKey => p256_signing_pk_x
            .copied()
            .ok_or(FetchTagError::MissingP256SigningKey),
    }
}
...
impl TransactOutput {
    ...
    pub fn resolved(
        ...
        p256_signing_pk_x: Option<&[u8; 32]>,
        account_address: impl Fn(u8) -> Option<[u8; 32]>,
    ) -> Result<ResolvedOutput<'_>, FetchTagError> {
        Ok(ResolvedOutput {
...
impl<'a> TransactOutputRef<'a> {
    ...
    pub fn resolved(
        ...
        p256_signing_pk_x: Option<&[u8; 32]>,
        account_address: impl Fn(u8) -> Option<[u8; 32]>,
    ) -> Result<ResolvedOutput<'a>, FetchTagError> {
        Ok(ResolvedOutput {
            utxo_hash: self.utxo_hash,
            owner_tag: fetch_tag(&self.owner_tag, p256_signing_pk_x, account_address)?,
            data: self.data,
...
/// `view_tag`/`data` byte accessors shared by the owned [`OutputData`] and the
...
impl OutputDataBytes for OutputData {
    fn view_tag(&self) -> &[u8; 32] {
        &self.view_tag
    }
    fn data(&self) -> &[u8] {
...
mod tests {
    ...
    fn fetch_tag_resolves_every_variant() {
        ...
        assert_eq!(
            ...
            Err(FetchTagError::AccountMissing(5))
        );
        assert_eq!(
            fetch_tag(&OwnerTag::P256SigningKey, None, accounts),
            Err(FetchTagError::MissingP256SigningKey)
        );
    }

    fn hash_of(outputs: &[ResolvedOutput], messages: &[OutputData]) -> [u8; 32] {
        ExternalDataHash {
