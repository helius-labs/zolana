...
/// Borrowed view of [`MergeTransactIxData`]. Proof points and nullifiers alias
/// the instruction buffer; the small index vectors are read owned.
...
pub struct MergeTransactIxDataRef<'a> {
    ...
    #[wincode(with = "containers::Vec<&'a [u8; 32], FixIntLen<u8>>")]
    pub nullifiers: Vec<&'a [u8; 32]>,
    #[wincode(with = "containers::Vec<u16, FixIntLen<u8>>")]
    pub utxo_tree_root_index: Vec<u16>,
    #[wincode(with = "containers::Vec<u16, FixIntLen<u8>>")]
    pub nullifier_tree_root_index: Vec<u16>,
...
impl<'a> MergeTransactIxDataRef<'a> {
    ...
    pub(crate) fn validate_shape(&self) -> Result<(), ReadError> {
        ...
        {
            return Err(ReadError::Custom("unsupported merge shape"));
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn round_trips_owned_and_ref() {
        ...
        assert_eq!(view.nullifiers, owned.nullifiers.iter().collect::<Vec<_>>());
        assert_eq!(
