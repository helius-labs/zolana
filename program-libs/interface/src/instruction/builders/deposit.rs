...
impl AssetDeposit {
    fn validate_fields(&self, entry_index: usize) -> Result<(), DepositBuildError> {
        ...
        validate_canonical_field(&self.blinding, entry_index, "blinding")?;
...
mod tests {
    ...
    fn rejects_every_non_canonical_deposit_field() {
        ...
        let mut blinding = entry(DepositAsset::Sol, 1);
        blinding.blinding = BN254_SCALAR_MODULUS_BE;
        assert_eq!(
            deposit(vec![blinding]).instruction(),
            Err(DepositBuildError::NonCanonicalField {
                entry_index: 0,
                field: "blinding",
            })
