...
fn accessors_and_behavior_match_variants() {
    ...
    assert_eq!(p256_cached.cache_access(), Some(CACHE));
    ...
    for (rail, twin) in [
        ...
        assert!(rail.cache_access().is_none());
    }
    assert!(CircuitId::RingAuthority(2, 2, 3).cache_access().is_none());
