//! Pins the record math to the canonical UTXO types.
...
fn record_commitments_match_the_canonical_utxo_types() {
    let owner = RecordsOwner::new(TEST_PDA.as_array()).unwrap();
    ...
    assert_eq!(
        owner.address(RecordKind::Block, &member).unwrap(),
    ...
    let output = ProofInputUtxo::new(expected_owner_hash, &SOL_MINT, 0, &record.blinding())
        .unwrap()
        .with_data_hash(record.data_hash(&address).unwrap());
    let utxo_hash = record.utxo_hash(&owner, &address).unwrap();
    assert_eq!(utxo_hash, output.hash().unwrap());
    assert_eq!(
        record_nullifier(&utxo_hash, &record.blinding()).unwrap(),
        nullifier_key
            .nullifier(&utxo_hash, &record.blinding())
            .unwrap()
    );
}
