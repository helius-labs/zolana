...
fn test_queue_rejects_insert_at_tree_capacity() {
    ...
    let tree =
        init_tree_account_data::<200>(&mut account_data, 200, 1, height, TreeType::AddressV2, None)
            .unwrap();
...
fn test_queue_insert_advances_queue_index_only() {
    ...
    let rng = &mut rand::rngs::StdRng::from_seed([0u8; 32]);
    let tree = init_tree_account_data::<5>(&mut account_data, 5, 1, 40, TreeType::AddressV2, None)
        .unwrap();
