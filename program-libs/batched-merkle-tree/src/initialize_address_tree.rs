...
fn test_init_indexed_tree_init_roots() {
    ...
    let mut nullifier_data = vec![0u8; get_address_merkle_tree_account_size_from_params(params)];
    ...
    .unwrap();
    assert_eq!(
        *nullifier_account.root_history.get(0).unwrap(),
        NULLIFIER_TREE_INIT_ROOT_40
    );
    assert_eq!(nullifier_account.next_index, 1);

    // Address tree keeps the default address sentinel root.
    let mut address_data = vec![0u8; get_address_merkle_tree_account_size_from_params(params)];
    ...
    .unwrap();
    assert_eq!(
        *address_account.root_history.get(0).unwrap(),
