...
/// Deprecated path node layout kept only to preserve historical
/// [`MerkleTreeEvent`] wire compatibility.
#[deprecated(note = "legacy event layout reserved for wire compatibility")]
#[derive(AnchorDeserialize, AnchorSerialize, Debug, Eq, PartialEq)]
pub struct ChangelogPathNode {
    pub node: [u8; 32],
    pub index: u32,
}

/// Deprecated changelog layout kept only to reserve the original
/// [`MerkleTreeEvent`] variant tag 0.
#[deprecated(note = "legacy event layout reserved for wire compatibility")]
...
pub struct ChangelogEvent {
    ...
    pub id: [u8; 32],
    /// Merkle paths.
    pub paths: Vec<Vec<ChangelogPathNode>>,
