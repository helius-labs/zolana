...
use crate::{
    access::AccessMetadata, errors::MerkleTreeMetadataError, rollover::RolloverMetadata, QueueType,
};
...
impl anchor_lang::IdlBuild for QueueMetadata {}

impl borsh::BorshSerialize for QueueMetadata {
    fn serialize<W: borsh::io::Write>(&self, writer: &mut W) -> borsh::io::Result<()> {
        borsh::BorshSerialize::serialize(&self.access_metadata, writer)?;
        borsh::BorshSerialize::serialize(&self.rollover_metadata, writer)?;
        crate::serialize_address(&self.associated_merkle_tree, writer)?;
        crate::serialize_address(&self.next_queue, writer)?;
        borsh::BorshSerialize::serialize(&self.queue_type, writer)
    }
}

impl borsh::BorshDeserialize for QueueMetadata {
    fn deserialize_reader<R: borsh::io::Read>(reader: &mut R) -> borsh::io::Result<Self> {
        Ok(Self {
            access_metadata: borsh::BorshDeserialize::deserialize_reader(reader)?,
            rollover_metadata: borsh::BorshDeserialize::deserialize_reader(reader)?,
            associated_merkle_tree: crate::deserialize_address(reader)?,
            next_queue: crate::deserialize_address(reader)?,
            queue_type: borsh::BorshDeserialize::deserialize_reader(reader)?,
        })
    }
}
...
mod tests {
    ...
    fn test_check_queue_type_valid() {
        let valid_queue_type = QueueType::InputStateV2;
    ...
    fn test_check_queue_type_invalid() {
        ...
        let expected_queue_type = QueueType::AddressV2;
        assert!(matches!(
    ...
    fn test_rollover_method_valid() {
        ...
        let mut queue_metadata =
            create_queue_metadata(associated_merkle_tree, QueueType::InputStateV2);
