...
fn verify_nullifier_update_proof(
    ...
    let public_input_hash = queue_hash_chain(&[
        old_root,
        new_root,
        leaves_hash_chain,
...
fn queue_hash_chain(values: &[[u8; 32]]) -> Result<[u8; 32], ProgramError> {
    if values.is_empty() {
        return Ok([0u8; 32]);
    }
    // Light's batched queue proofs commit to leaves with a left-folded
    // Poseidon hash chain. The SPP transaction proof uses a right fold, but
    // this handler must bind to the exact hash_chain_stores value consumed
    // from the queue batch.
    let mut h = values[0];
    for value in &values[1..] {
        h = Poseidon::hashv(&[h.as_slice(), value.as_slice()])
            .map_err(|_| ShieldedPoolError::InvalidNullifierBatchUpdate)?;
    }
    Ok(h)
}
