...
use crate::instructions::{
    merge::cache::{parse_cache_accounts, MergeCacheAccounts},
...
/// self-CPI), then one writable nullifier PDA per input and, when the merge
/// writes a cache, the writable cache followed by its writer (signer). The
/// `ring_config` must sign, be unpaused, and be a valid
...
pub struct MergeRingAccounts<'a> {
    ...
    pub cache: Option<MergeCacheAccounts<'a>>,
...
impl<'a> MergeRingAccounts<'a> {
    pub fn validate_and_parse(
        ...
        let cache = parse_cache_accounts(iter, cache_slot)?;
        Ok(Self {
            cache,
            input_tree,
            output_tree,
