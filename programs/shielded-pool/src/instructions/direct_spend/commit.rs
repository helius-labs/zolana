...
use zolana_interface::{
    ...
    verifying_keys::{direct_payment_512_2, spend_balance_16_2},
};
...
pub fn process_commit(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    ...
    let Payload::Payment { statement, proof } = load_payload(payment, owner.address().as_array())?
    else {
    ...
    let mut values = Vec::with_capacity(MAX_CERTIFICATES);
    {
        ...
        match &statement.inputs {
            ...
            PaymentInputs::Notes {
                ...
                verify(&proof, &fields, &direct_payment_512_2::VERIFYINGKEY)?;
