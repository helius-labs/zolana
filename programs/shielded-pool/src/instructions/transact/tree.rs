...
use zolana_interface::{
    ...
    instruction::instruction_data::transact::{InputUtxoRef, TransactIxDataRef},
...
pub(crate) fn apply_input_tree(
    ...
    // Only the first queue index is kept: the queue counter is monotone and
    // this loop walks one tree in order, so every later index is `first + i`.
...
pub(crate) struct OutputTreeWrite {
    pub first_output_leaf_index: u64,
    /// Raw id of the output tree; every output is hashed under it, so the
    /// proof commits to it as a public input.
...
pub(crate) fn apply_output_tree(
    ...
    ix: &TransactIxDataRef<'_>,
    slot: u64,
) -> Result<OutputTreeWrite, ProgramError> {
