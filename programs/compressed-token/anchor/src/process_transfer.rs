...
>(
    ...
    for input_token_data in input_token_data_with_context.iter() {
        ...
        if signer_is_delegate.is_some() {
            if let Some(delegate_index) = input_token_data.delegate_index {
                let delegate_account = remaining_accounts[delegate_index as usize].key();
                if *signer != delegate_account {
                    msg!(
                        "signer {:?} != delegate in remaining accounts {:?}",
                        signer,
                        delegate_account
                    );
                    msg!("delegate index {:?}", delegate_index as usize);
    ...
    pub fn create_inputs_and_remaining_accounts(
        ...
        if let Some(delegate) = delegate {
            ...
            for account in input_token_data.iter() {
                if let Some(account_delegate) = account.delegate {
                    let account_delegate_pubkey = account_delegate.into();
                    if delegate != account_delegate_pubkey {
                        println!("delegate: {:?}", delegate);
                        println!("account.delegate: {:?}", account_delegate);
