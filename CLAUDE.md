...
only a four-shape subset, which the Go key-path resolver does not model, so it
will happily name a key file for a shape the program rejects.
...
### Proving keys and the prover binary must be built together

A proving key embeds the compiled constraint system, so a key generated by a
stale `light-prover` disagrees with the circuit the running server builds a
witness for. The failure is opaque -- gnark reports `invalid witness size, got
N, expected N+1` -- and it looks like a client bug rather than a stale artifact.

Always `go build -o light-prover .` immediately before `setup-transfer` or
`setup-merge`, and
regenerate the verifying keys from those same key files in the same pass. If
`regenerate_all_vkeys.sh` reports changes to verifying keys you did not intend
to touch, the local `proving-keys` directory is out of date with the circuit
source; do not commit those changes, because the committed verifying keys match
the *distributed* proving keys, not whatever is on disk.

Note also that `groth16.Setup` is randomized: two runs over the same circuit
produce different, mutually incompatible key pairs. Locally generated keys are
for local testing only; distribution goes through
