...
rings
- policy rings over the shielded pool, one nested Cargo workspace each
...
- `rings/squads`: the Squads policy ring, a nested workspace that depends on the
  root crates by path and settles through SPP by CPI. `tools/check-test-hygiene.sh`
  compares every external dependency the ring consumes against the root pin,
...
rings/squads/
  program/             -- on-chain ring program, verifies the ring proof and CPIs SPP
  interface/           -- tags, builders, instruction data, state, verifying keys
  sdk/                 -- ring crypto, proposal building, prover glue
  client/              -- ring client
  integration-tests/   -- LiteSVM and localnet suites, load the built ring .so
```
...
`rings/squads` is a nested workspace, so it needs its own build and test
recipes. `just fmt`, `just fmt-check`, `just check-all`, and `just clippy`
already cover it in a second invocation.

```bash
just build-programs-squads   # rings/squads/target/deploy/zolana_squads_program.so
just test-squads             # unit and LiteSVM suites, no ring proving keys
```

The proof-backed ring suites need the ring proving keys, which are unpublished.
Generate them with `prover/server/scripts/generate_keys_squads.sh`, which also
rewrites the committed verifying-key constants they pin, then rebuild the ring
