...
`E2E_BENCH_OVERLAP=1` starts proving and uploads the immutable statement concurrently. Each buffer transaction is confirmed before the next offset is written. Once proving completes, `SpendUpload::finish` checks the original statement, payload length, variant and circuit capacity before appending the proof and commitment. The final transaction consumes the completed buffer. Set the flag to `0` to measure the sequential schedule.
...
Settings matched: native dev profile, GOMAXPROCS=18, four prover permits, client concurrency 4, overlap enabled, 25 ms confirmed-RPC polling and 500 ms indexer polling. Raw logs, artifact manifest, timing rows and summary are in `target/admission-bench/final-smoke`. No source changes followed these successful runs.
