...
The [benchmark runner](../tools/bench-direct-spend.md) records binary hashes, per-payment prover logs, individual timing rows and summaries. All 11 artifact hashes still matched after the matrix. The PR #320 comparison source is saved locally on `experiment/merge/10x-settlement` at `aab26b696`; the GKR implementation and this report are on `experiment/merge/second-gkr`. The [service notes](../prover/server/benchmarks/gkr-service.md) record key generation and prover experiments. Keys are local development artifacts; the new shapes need production key generation and release integration before deployment.
