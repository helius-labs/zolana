...
`owner_pk_field` hashes the x-coordinate alone, omitting the SEC1 parity byte, so a P256 owner field has the same shape as an Ed25519 one. A spender needs the full point to build the witness and holds it already; the owner identity does not have to encode it.

`pk_field` keeps the parity byte as an extra Poseidon layer. Its only use is a registered viewing key, in `merge_transact`.
...
1. `owner_hash` is a fixed target. Its second half, `nullifier_pk = Poseidon(nullifier_secret)`, is a private witness the prover chooses, so an attacker already holds that half of the preimage. Reusing another owner's `owner_hash` means hitting one specific Poseidon output: a preimage at roughly 2⁻²⁵⁴, not a birthday collision at 2⁻¹²⁷.
...
The two forms prove the same statement. The confidential variant pins the `p256_signing_pk` public input to the key the proof recomputes from the witnessed point, so the equality test selects the same inputs as the zero test and the owner key hash reaching `owner_hash` is identical. On the Solana-only confidential rail the recomputed key is `0`, which makes the equality test a zero test.
...
| Nullifier non-inclusion | Each input nullifier must NOT exist in the nullifier tree at its corresponding `nullifier_tree_roots[i]` before the transaction. |
| Nullifiers | Public nullifier per input equals the input's [nullifier](#nullifier). |
| Input cleanliness — `data_hash` | For each non-dummy input UTXO: `data_hash = 0`. UTXOs with `utxo_data` set are not mergeable; the owner consumes them through an owner-signed `transact` flow. Applies to both `merge_transact` and `merge_zone`. |
...
The three `bool` fields are stored as `u8`. `create_protocol_config` writes the
raw instruction byte without a range check and readers test for non-zero, so a
flag byte of `7` is well formed and reads as set. A conforming decoder treats
any non-zero byte as `true`.
...
covering every mutable field, and the program assigns the one field the variant
names:
...
Solana executes atomically. The seven fields have no cross-field validation:
each authority gates a different instruction and each check reads only its own
field, so a sequence of single-field updates reaches the same states as a full
write. `create_protocol_config` is the only full write.

Rotating `protocol_authority` additionally requires the **incoming** authority
to sign, as a third account. The instruction is rejected unless that signer
equals the address being written, so the authority cannot be rotated to an
...
branch by how many accounts remain after the fixed prefix, and rejects any
account beyond the last one it consumes, so neither table may be padded.
...
| 2 | depositor | x | x | funds the deposit; the signer authorizes any attached `utxo_data` |
...
| 5 | registry |   |   | the mint's `Asset registry` PDA; supplies the deposited asset |
| 6 | token_program |   |   | SPL Token or Token-22 |
| 7 | program |   |   | SPP's own account, for the [`emit_event`](#instructions) self-CPI in check 7 |

**Instruction data**

The program deserializes with `wincode::deserialize_exact`, so a payload of any
other shape is rejected whole rather than partially accepted.
...
**Discovery.** SPP does not read `view_tag`; it copies the value into the
event's output slot. Photon persists it per output and
[`get_shielded_transactions_by_tags`](#get_shielded_transactions_by_tags)
filters on that column, which is how a wallet finds an incoming deposit. Any
other value still produces a spendable UTXO, and no error is returned: the
recipient's wallet does not find it. `view_tag` is the only field carrying the
recipient's signing pubkey; the payload's `owner` holds the `owner_hash`.
...
2. `depositor` signs, and `amount` is non-zero.
...
[`deposit`](#deposit) sets with `zone_config` inserted after the depositor. The
trailing `program` account is required here too.
...
Deserialized with `wincode::deserialize_exact`, as in [`deposit`](#deposit).
...
A field whose domain the protocol does not bound below `2^53` **also** accepts a decimal string, which holds the full width of its Rust declaration. Decoders accept both forms; encoders may keep writing numbers. These fields take the union:
...
Any other integer is a plain JSON number and a string is refused for it. A leaf index, and the low and high element indices of a non-inclusion proof, are capped by the tree height; `tree_type` and `root_index` are 16-bit; a page `limit` is 1 through 1000. For a field this section does not name, apply the same test: a monotonic queue sequence takes the union, a bounded index does not.

An encoder that cannot write a value as a safe JSON number reports an error instead, so a payload does not carry a truncated integer.
...
2. **Default-zone sync, anonymous-zone sync, and merge sync run as independent parallel branches.**
    ...
    3. **Merge sync — for each viewing key `k` in parallel.**
        1. Fetch loop in batches of 10 000 until first empty batch: derive `wallet.get_merge_view_tag(n)` for `n in [i, i+10_000)`; call `indexer.get_shielded_transactions_by_tags(tags)`.
        2. **Decrypt and store.** Decrypt merged-output ciphertexts with `k`; store UTXOs along with the transaction's `nullifiers`. Track `max(observed n)`; the `merge_count` is `max(observed n) + 1`. A wallet that never had a policy-zone merge yields an empty first batch.
...
5. **Set wallet state**: `Utxos`, `known_senders`, `known_recipients`, per-key counters `tx_count`, `request_count` (= `max(observed index) + 1`), `merge_count` (= `max(observed n) + 1`) from step 2, `last_synced = current_timestamp()`.
