...
| Private transaction hash | `private_tx_hash` as defined in [SPP Proof](#spp-proof---solana-privacy-zk-proof). It covers every input, the output, and the external-data hash (which binds `expiry_unix_ts`), so the proof cannot be replayed with different state. |
