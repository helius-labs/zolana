...
**Coin type.** `TSPP_COIN_TYPE = 1392955331'` (placeholder), derived as `SHA-256("luminous.TSPP.v1")[0..4]` as a big-endian `u32` masked to 31 bits (`& 0x7FFF_FFFF`). Not finalized or SLIP-0044-registered; changing it re-derives every identity, so identities derived under it now are disposable until it is finalized for production.
...
**Recovery.** Each source matches a rail. A P-256 wallet recovers from its BIP-39 phrase (the first source). A Solana-only (ed25519) wallet has no separate phrase, and SPP does **not** prescribe a Solana derivation path: the invariant is to recover the **exact 32-byte Solana Ed25519 secret**, using whatever scheme that Solana wallet used (Solana tooling supports several — e.g. `m/44'/501'/account'/change'`, a bare `m/44'/501'`, or the legacy first-32-bytes-of-seed mode). The ed25519 bridge (the second source) then reconstructs the shielded identity from that secret. So a Solana mnemonic recovers the shielded wallet *through* the Solana key; matching the original Solana wallet's derivation is the caller's responsibility.
