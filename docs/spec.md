...
/// the preimage of the output's owner public input `hash_bytes_33(0x53 || fetch_tag)`.
enum OwnerTag {
...
struct MergeTransactIxData {
    ...
    /// length prefix; same length as `nullifiers`. Every entry must equal
    /// entry 0.
