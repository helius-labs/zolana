...
**In-circuit truncation and non-inclusion.** The tree stores truncated values, so the non-inclusion check at `nullifier_tree_roots[i]` runs on truncated values: the circuit MUST compute `truncate_248` itself. Truncating on-chain instead is rejected — it drops the in-circuit check, and nothing else catches a double spend in time. Outputs enter the UTXO tree at `transact` time; nullifiers enter the tree later (queue → forester). Queue insertion only checks bloom filters of batches not yet in the tree; it never checks the tree itself. Invariant (MUST preserve): every inserted nullifier is blocked by a live bloom filter or is in every live cached root; the in-circuit check is the part that rejects against those roots. Both gates are normative; neither MUST be removed as an optimization. Also liveness: a duplicate that gets past the check breaks the batch append proof, the forester cannot skip it, and the batch is stuck.

**Canonical decomposition (circuit soundness).** The decomposition MUST be canonical (`< p`): `Num2BitsStrict` or the stack's equivalent, else a constrained comparison against `p − 1`. Otherwise `x + p` gives different low 248 bits — two valid nullifiers for one UTXO, a double spend. A 248-bit range check is no substitute; `x + p` is also in range. There is no gadget to copy from `light-protocol`: it truncates outside the circuit (Keccak, first byte zeroed) and only range-checks. Applies to every in-circuit truncation: [SPP Proof](#spp-proof---solana-privacy-zk-proof) and [Merge Proof](#merge-proof---merge-zk-proof) nullifiers. CI MUST run a shared negative vector (`x + p` decomposition fails) and positive vector (canonical `x` passes) for every circuit that computes nullifiers.
