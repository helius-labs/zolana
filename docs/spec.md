...
| `nullifier_secret` | owner wallet's symmetric secret (see [Nullifier Key](#nullifier-key)); recomputes the input's `nullifier_pk` and feeds the [nullifier](#nullifier) formula. Inputs of the same wallet carry equal values. |
