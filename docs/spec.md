...
**Policy sources:** the reference ring program compiles its rule table into the binary and pins `Policy.hash` over a per-kind source map at `create_policy`. Each record kind the table references names the records owner serving it, the ring's own records PDA or a curator ring's, so one curated list (an OFAC blocklist) serves many rings from one write. The map enters the policy hash as eight positional `(kind, owner_hash)` slots and the transfer circuit resolves every pool entry's owner from it. `set_policy_source` lets the ring authority re-point one kind only while the stored hash is reproducible from the deployed table, a rebuilt table stays fail closed. All sources live in the transfer's records tree. The encoding is pinned by `program-libs/ring-policy` and the cross-language vectors in `custom-rings/sdk/tests/go_policy_vectors.rs`.
