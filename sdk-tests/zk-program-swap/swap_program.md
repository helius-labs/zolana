...
The taker learns the order and may decline to take it, but it cannot take the escrowed funds: the
...
sequenceDiagram
    ...
    Swap->>SPP: CPI transact -> change + escrow UTXO (swap utxo_data) + marker message
    Note over SPP: spend maker source UTXO, append change + escrow UTXO <br> source_asset_id public, source_amount + order terms private in utxo_data <br> escrow ciphertext encrypted to taker viewing pubkey; marker message tagged to taker
    Note over Taker,SPP: taker wallet sync finds the marker message (its tag), <br> decrypts the matching escrow slot -> escrow hash preimage (terms + blinding)
    Note over Maker: maker can decrypt the escrow slot via the tx viewing key it holds

    Note over Taker: 2a. Take, before expiry (Taker holds the escrow hash preimage)
    Taker->>Swap: take_verifiable_encryption (take proof + SPP transact)
    Swap->>SPP: CPI transact: escrow + Taker destination UTXO -> source UTXO to Taker + destination UTXO to maker
    Note over SPP: destination ciphertext checked by the take proof, decryptable via the escrow blinding; escrow consumed
...
| `MarkerData` | Borsh | `{ escrow_utxo_hash: [u8;32], maker_pubkey: [u8;32] }`, the plaintext `make` writes into the transact's single marker message. `escrow_utxo_hash` locates the escrow slot; `maker_pubkey` is the `make` signer's Solana pubkey, which the taker resolves to the maker's registered shielded address via the user registry. See [make](#make). |
| `Order terms` | — | The fields committed in the escrow UTXO's `utxo_data` (record tag `0x02`), hashed into the escrow `utxo_hash` via `data_hash`: `destination_asset_id`, `destination_amount`, `maker_address`, `expiry`, `taker_pk_fe`, `take_mode`. See [Order Terms](#order-terms). |
...
- **Private:** `price`, `source_amount`, `destination_amount`, and the aggregate volume per asset.
  These live only inside confidential UTXOs and the escrow `utxo_data`.
...
The swap program owns no accounts: the order and its funds live in the escrow UTXO, a leaf in the
...
The escrow UTXO is the order. `make` writes the
...
order_terms = (
    ...
    take_mode,       // which take instruction may settle this escrow: 0 = take (derived), 1 = take_verifiable_encryption
)
data_hash = Poseidon(order_terms)        // enters the escrow utxo_hash directly
```

`take_mode` selects the take instruction that can settle the escrow: each take circuit reconstructs
`data_hash` with its own hardcoded `take_mode`, so settling with the wrong take yields a mismatched
escrow hash and the proof fails. `cancel` takes `take_mode` as an unconstrained witness and refunds
either kind.
...
Knowledge of the escrow hash preimage, the order terms plus the escrow `blinding`, is the complete
spend capability: the nullifier includes the `blinding` and the circuits need it to recompute the
escrow `utxo_hash`. The preimage is delivered in the make
transaction (see
[make](#make)): the escrow output's recipient ciphertext contains the order terms other
...
signer, and verifies the resolution by recomputing the escrow `utxo_hash`. Moving the escrow also
requires the program: SPP spends a PDA-owned UTXO only when the swap program
produces the escrow-authority signer via `invoke_signed`, which it does only through `take` or
...
circuits leave the escrow owner a free witness; SPP enforces the PDA ownership at spend time, when
the escrow input's owner must match the escrow-authority signer. The program derives the PDA via
`find_program_address`, checks it is present among the forwarded SPP accounts, and flips it to a
signer inside the SPP CPI to authorize the escrow spend (the client sets the escrow input's
`eddsa_signer_index = 2`, the PDA's position in the forwarded SPP slice
`[payer, tree, escrow_authority, spp_program]`); the PDA is a bare address and signs only inside
the CPI.
...
the escrow blinding it already holds: `take_verifiable_encryption` proves a ciphertext keyed from
it, and `take` fixes the destination blinding to `Poseidon(escrow_blinding, DOMAIN)`.
...
|---|-------------|-----|-------------|---------------|-------------------|----------------|
| 1 | [make](#make) | 2 | Verify the make proof and CPI SPP `transact` to escrow the source funds into the order's escrow UTXO (swap `utxo_data`) and write the taker's marker message. | — | SPP trees (CPI) | Maker signs (fee payer) |
| 2 | [take](#take) | 3 | Verify the take proof (standard Groth16) and CPI SPP `transact`: spend escrow + a matching destination UTXO, pay destination to the maker (blinding derived from the escrow blinding, standard output ciphertext) and source to the taker. | escrow_authority | SPP trees (CPI) | Any fee payer signs; the escrow spend is authorized by the program's escrow-authority PDA signer; the maker payout is constrained to the committed `maker_address` |
| 3 | [take_verifiable_encryption](#take_verifiable_encryption) | 5 | Verify the take proof and CPI SPP `transact`: spend escrow + the taker's destination UTXO, pay destination (with a proof-checked ciphertext) to the maker and source to the taker. | escrow_authority | SPP trees (CPI) | Taker signs (fee payer); the escrow spend is authorized by the program's escrow-authority PDA signer; the circuit checks the taker-side input's owner against the committed `taker_pk_fe` |
| 4 | [cancel](#cancel) | 4 | Verify the cancel proof and CPI SPP `transact`: after expiry, spend escrow back to `maker_address`. | escrow_authority | SPP trees (CPI) | Maker signs; the proof checks the signer against the committed `maker_owner_hash`; the program's escrow-authority PDA authorizes the escrow spend |
...
(seeds `[b"escrow_authority"]`), with the [order terms](#order-terms) in its `utxo_data` (which,
with the PDA owner, makes SPP spend it only through a swap circuit). The transact is 1-in/2-out
(the maker's source UTXO in; a change UTXO to the maker and the escrow UTXO out) plus a marker
...
recomputing the escrow `utxo_hash`. The program requires exactly one transact message, with empty
`data`, and writes a plaintext [`MarkerData`](#glossary) `{ escrow_utxo_hash,
maker_pubkey }` into it (the escrow hash read from transact output index 1, the pubkey from the
signer), so ordinary wallet sync finds the trade and
can locate the matching escrow slot to decrypt. The message is committed in `private_tx_hash` via
...
The proof checks the escrow output against the order rules (see the [make
...
struct MakeIxData {
    ...
    proof: MakeProof,
    /// SPP transact (1-in/2-out): maker source UTXO -> change + escrow UTXO;
...
[`take_verifiable_encryption`](#take_verifiable_encryption): 2-in/2-out, escrow + a
...
blinding to `Poseidon(escrow_blinding, DOMAIN)`, so the maker reconstructs and spends the bought
output from the
escrow blinding and order terms alone, and that output includes an ordinary recipient ciphertext for
wallet discovery rather than a verifiable one. The circuit checks only the payout and the blinding
derivation, so any holder of the escrow hash preimage and a matching destination UTXO may
take; the maker payout still flows only to the committed `maker_address`, and the escrow's
`take_mode` forbids settling a `take_verifiable_encryption` escrow this way.
...
sign: authorization rests on the escrow-authority PDA signer alone.
...
struct TakeIxData {
    ...
    proof: TakeProof,
    /// SPP transact (2-in/2-out): escrow + destination UTXO -> source to the taker,
...
keyed from the escrow blinding, so the maker can decrypt and spend the bought funds; the taker's
source output is sender-encrypted. The taker
holds the escrow hash preimage (recovered from the escrow ciphertext) and its own destination UTXO
...
can take. The swap program supplies the escrow-authority PDA signer for the escrow spend, reads
...
3. `tree_accounts` — SPP trees the transact touches; writable.
4. `escrow_authority` — escrow-authority PDA (seeds `[b"escrow_authority"]`); read-only, non-signer.
...
struct TakeVerifiableEncryptionIxData {
    ...
    /// SPP transact (2-in/2-out): escrow + taker destination UTXO -> source to taker,
...
The swap program supplies the escrow-authority PDA signer
...
5. `escrow_authority` — escrow-authority PDA (seeds `[b"escrow_authority"]`); read-only, non-signer.
   The program flips it to a signer inside the SPP CPI to authorize the escrow spend (see
   [Order Terms](#order-terms)).
...
proof inside `transact`. Each circuit witnesses the SPP transaction's UTXO hash preimages (the escrow
...
Proves the escrow output commits the order terms. Matches the 1-in/2-out transact
(source UTXO in; change + escrow out), padded to the SPP `(2, 2)` proving shape. The marker message
...
- **Private inputs:** the order terms (`destination_asset`, `destination_amount`, the maker's
  ...
  escrow and change output UTXOs. The escrow owner is a free witness
  (the escrow-authority PDA, which SPP enforces at spend time).
- **Constraints:**
  - The `private_tx_hash` recomputation mirrors the padded transact exactly: `chain([source_input,
    0])` over inputs, `chain([change, escrow])` over outputs, `chain([0, 0])` over
  ...
  - The escrow output committed in `private_tx_hash` has `data_hash = Poseidon(order terms)`
    with `maker_address` hashed in as a field element, zone fields 0, and a nonzero amount, so the
    public SPP escrow output commits the terms.
  - The change output is constrained to the escrow's asset and to `maker_owner_hash`, with empty
    data.
...
against the committed terms, matching the 2-in/2-out transact (escrow + destination UTXO in;
...
- **Private inputs:** the escrow UTXO hash preimage (order terms, `source_amount`,
  `escrow_blinding`), the maker's compressed viewing pubkey (feeding `maker_address`), the taker's
  destination-side input UTXO, and the two output UTXO hash preimages.
- **Constraints:**
  - The public `expiry` equals the committed order `expiry`; the escrow `data_hash` uses the `take`
    constant for `take_mode`, so only a `take`-mode escrow reconstructs.
  ...
  - The `destination_output` blinding is fixed to the low 248 bits (31 bytes) of
    `Poseidon(escrow_blinding, DOMAIN)`, so the maker recovers and spends it from the escrow
    blinding alone.
  - `destination_amount > 0` (64-bit range-checked); the escrow amount is nonzero.
...
payout against the committed terms. Matches the 2-in/2-out transact (escrow + taker destination
...
- **Private inputs:** the take circuit's core (the escrow UTXO hash preimage incl. `utxo_data` =
  order terms, `source_amount`, `escrow_blinding`; the taker's destination-side input; the hash
  preimages of the two output UTXOs) plus the taker's nullifier pubkey. The escrow
  owner is the escrow-authority PDA constant and the preimage includes the `escrow_blinding`.
- **Constraints:**
  - The taker-side input's owner equals `Poseidon(taker_pk_fe, taker_nullifier_pk)`; SPP requires
    ...
    public `expiry` equals the committed order `expiry`; the escrow `data_hash` uses the
  ...
  - The `destination_output` ciphertext is AES-256-CTR over `destination_amount ||
    ...
    `Poseidon(escrow_blinding, KDF_DOMAIN)`, integrity via `ciphertext_hash`; this reuses the SPP
    [merge-proof](../../docs/spec.md#merge-proof---merge-zk-proof) symmetric scheme, the source of
    the BSB22 commitment. Holders of the escrow hash preimage, the maker and the taker, can
...
- **Private inputs:** the escrow UTXO hash preimage (incl. `utxo_data` = order terms,
  `source_amount`, `escrow_blinding`), the `source_output` hash preimage, and the maker's
  `(maker_owner_pk_field,
  maker_nullifier_pk)`, the preimage of the committed `maker_owner_hash`. The escrow owner is the
  escrow-authority PDA constant and the escrow preimage
  includes the `escrow_blinding`.
- **Constraints:**
  - The public `expiry` equals the committed order `expiry`; `take_mode` is an unconstrained
    witness, so either escrow kind refunds.
