...
use crate::{error::SwapError, vk_registry_specs::VK_REGISTRY_SPECS};

/// VKs in `VK_REGISTRY_SPECS` order.
static VK_CATALOG: [&groth16_solana::groth16::Groth16Verifyingkey<'static>; 4] = [
    &crate::verifying_keys::cancel::VERIFYINGKEY,
    &crate::verifying_keys::make::VERIFYINGKEY,
...
pub fn process_init_vk_registry_ix(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    ...
    let vk = *VK_CATALOG
        .get(vk_index)
        .ok_or(SwapError::InvalidVkRegistryIndex)?;
    ...
    if !address_eq(registry.address(), &Address::from(spec.address)) {
        return Err(SwapError::InvalidVkRegistryAccount.into());
    }
    ...
    if registry.data_len() == 0 {
        ...
        header
            ...
            .map_err(|_| SwapError::VkRegistryAlreadyInitialized)?;
        return Ok(());
    }
    ...
    for (index, source) in sources.iter().take(spec.g2_count as usize).enumerate() {
        ...
        .map_err(|_| SwapError::VkRegistryInitFailed)?;
    }
    let target =
...
fn check_unfinalized(registry: &AccountView, spec: &VkRegistrySpec) -> ProgramResult {
    ...
    header
        .check(spec.g2_count)
        .map_err(|_| SwapError::InvalidVkRegistryAccount)?;
    if header.is_finalized() {
        return Err(SwapError::VkRegistryAlreadyInitialized.into());
    }
    Ok(())
}
    ...
    let data = registry.try_borrow()?;
    {
        let header: &VkRegistryHeader = bytemuck::from_bytes(
            data.get(..VkRegistryHeader::SIZE)
                .ok_or(SwapError::InvalidVkRegistryAccount)?,
        );
        header
            .check(spec.g2_count)
            .map_err(|_| SwapError::InvalidVkRegistryAccount)?;
        if !header.is_finalized() {
            return Err(SwapError::VkRegistryNotReady.into());
        }
    }
