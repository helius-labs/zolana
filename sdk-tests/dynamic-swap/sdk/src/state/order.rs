...
impl EscrowUtxo {
    ...
    /// Takes the tree because the commitment and the nullifier both fold it in.
...
/// An input owned by the escrow or pool authority, whose commitment and
/// nullifier both use the public zero-secret nullifier key.
pub(crate) fn authority_input_utxo(
    utxo: Utxo,
    data_hash: [u8; 32],
    tree_id: u16,
    leaf_index: u64,
) -> Result<SppProofInputUtxo> {
    let key = escrow_nullifier_key();
    let nullifier_pubkey = key.pubkey().map_err(err)?;
    let utxo_hash = utxo
        .hash(&nullifier_pubkey, &data_hash, &[0; 32], tree_id)
        .map_err(err)?;
    let nullifier = key.nullifier(&utxo_hash, &utxo.blinding).map_err(err)?;
