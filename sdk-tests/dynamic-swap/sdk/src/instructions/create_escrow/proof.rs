...
pub struct EscrowOpenProofInputParams {
    ...
    /// Raw id of the tree the order and taker-change outputs are appended to;
    /// it is the second element of every output's commitment.
...
impl EscrowOpenProofInputParams {
    pub fn to_proof_inputs(&self) -> Result<EscrowOpenProofInputs> {
        ...
        let taker_change =
            ProofInputUtxo::try_from((&self.taker_change, self.output_tree_id)).map_err(err)?;
        ...
        let change_owner = check_output_utxo(
            "taker_change",
            &self.taker_change,
            &self.source_in.utxo.asset.asset,
            expected_change,
        )?;
