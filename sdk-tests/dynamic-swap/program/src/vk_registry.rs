...
//! shielded pool's `instructions/vk_registry.rs`, with permissionless
//! step-machine creation, address-commitment authentication, prepared-operand
//! verify.
...
use crate::{error::DynamicSwapError, vk_registry_specs::VK_REGISTRY_SPECS};
...
pub fn process_init_vk_registry_ix(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    ...
    let vk = *VK_CATALOG
        .get(vk_index)
        .ok_or(DynamicSwapError::InvalidVkRegistryIndex)?;
    ...
    if !address_eq(registry.address(), &Address::from(spec.address)) {
        return Err(DynamicSwapError::InvalidVkRegistryAccount.into());
    }
    ...
    if registry.data_len() == 0 {
        ...
        header
            ...
            .map_err(|_| DynamicSwapError::VkRegistryAlreadyInitialized)?;
        return Ok(());
    }
    ...
    for (index, source) in sources.iter().take(spec.g2_count as usize).enumerate() {
        ...
        .map_err(|_| DynamicSwapError::VkRegistryInitFailed)?;
    }
    let target =
...
fn check_unfinalized(registry: &AccountView, spec: &VkRegistrySpec) -> ProgramResult {
    ...
    header
        .check(spec.g2_count)
        .map_err(|_| DynamicSwapError::InvalidVkRegistryAccount)?;
    if header.is_finalized() {
        return Err(DynamicSwapError::VkRegistryAlreadyInitialized.into());
    }
    Ok(())
}
    ...
    let data = registry.try_borrow()?;
    {
        let header: &VkRegistryHeader = bytemuck::from_bytes(
            data.get(..VkRegistryHeader::SIZE)
                .ok_or(DynamicSwapError::InvalidVkRegistryAccount)?,
        );
        header
            .check(spec.g2_count)
            .map_err(|_| DynamicSwapError::InvalidVkRegistryAccount)?;
        if !header.is_finalized() {
            return Err(DynamicSwapError::VkRegistryNotReady.into());
        }
    }
