...
fn create_escrow_underwater_then_refund() -> Result<()> {
    ...
    let escrow = {
        ...
        // Photon indexes a confirmed transaction asynchronously, so block until
        // create_escrow is queryable under the escrow authority's view tag. The
        // settler below recovers the order note from a single scan of that tag
        // and fails outright if the indexer has not caught up yet.
