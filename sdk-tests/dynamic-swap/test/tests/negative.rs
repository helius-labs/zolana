...
use zolana_transaction::{
    instructions::transact::{asset_field, SppProofOutputUtxo},
...
/// The `create_pair` terms the rejection cases vary.
#[derive(Clone, Copy)]
struct PairTerms {
    price: u64,
    expiry_slots: u64,
...
impl PairTerms {
    ...
    // Returns the pair PDA (or the RPC error, which the rejection cases assert
    // on).
    fn create(self, env: &TestEnv, authority_solana: &dyn Signer) -> Result<Pubkey> {
        let pair = pair_pda(
...
// (kept in a single `#[test]` so the binary boots one localnet):
...
fn zero_price_and_authority_checks() -> Result<()> {
    ...
    let err = PairTerms {
        ...
        price_tolerance: PRICE_TOLERANCE + 1,
        ..valid_terms
    }
    .create(&env, authority_solana)
    .err()
    ...
    let err = PairTerms {
        maker_encryption_pubkey: bad_encryption_pubkey,
        ..valid_terms
    }
    .create(&env, authority_solana)
    .err()
    ...
    let pair = valid_terms.create(&env, authority_solana)?;
    ...
    let err = env
        ...
        .create_and_send_transaction(
            &[withdraw_ix],
            authority_solana.pubkey(),
            &[authority_solana as &dyn Signer, &intruder],
            ComputeBudgetConfig::for_instruction_count(1),
        )
        .err()
...
fn client_side_rejections(
    ...
    .into();
    let order_out =
        SppProofOutputUtxo::new(env.source_mint(), over_cap_amount, env.user.address()?)
            .map_err(|e| anyhow!("order_out: {e:?}"))?;
    let taker_change = SppProofOutputUtxo::new(env.source_mint(), 0, env.user.address()?)
