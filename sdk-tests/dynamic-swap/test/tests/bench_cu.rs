...
use dynamic_swap_sdk::{
    ...
    instructions::{
        ...
        settle::{cancel_blinding_seed, settle_blinding_seed, Settle, SettleProofInputParams},
    ...
    state::{
        escrow_authority_identity, pool_authority_identity, EscrowUtxo, IndexedPoolNote, PoolUtxo,
    },
    Groth16ProofBytes,
};
...
use zolana_keypair::{random_blinding, NullifierKey, ShieldedKeypair, ShieldedPda, SigningKey};
...
fn bench_cu_dynamic_swap() {
    ...
    let mut bench = CuBenchmark::new(ReadmeConfig {
        ...
        description: "Compute unit profiling for the dynamic-swap create_pair/update_price/\
             ...
             compute-budget limit ix and may not exceed 1232 bytes, and as the transaction v1 \
             these instructions are actually sent as, which states its compute ceilings in the \
...
fn escrow_bench_world() -> EscrowBenchWorld {
    ...
    let source_asset = Mint::new(Address::new_from_array([2u8; 32]), SOURCE_ASSET_ID);
    let destination_asset = Mint::new(Address::new_from_array([3u8; 32]), DESTINATION_ASSET_ID);
    let pair = pair_pda(
...
impl EscrowBenchWorld {
    fn pair_state(&self, available_liquidity: u64, open_reservations: u64) -> Pair {
        Pair {
            ...
            available_liquidity,
            open_reservations,
            source_asset: asset_field(&self.source_asset.asset).expect("source asset field"),
            destination_asset: asset_field(&self.destination_asset.asset)
    ...
    /// The order UTXO as leaf 0 of the fixture tree.
    fn order_in(&self) -> (SppProofInputUtxo, [u8; 32]) {
        let order_in = self
            ...
            .expect("order_in");
        let order_in_hash = order_in.hash();
...
fn bench_create_escrow(
    ...
    let source_in: SppProofInputUtxo = zolana_test_utils::utxo::wallet(
        source_utxo,
        &world.user_keypair.nullifier_key,
        BENCH_TREE_ID,
        0,
        None,
        None,
    )
    .expect("indexed source input")
    .into();

    // SPP derives both output blindings from the transaction's seed and the
    // source input's nullifier.
    let blinding_seed = random_blinding();
    let (private_tx_blinding, blindings) =
        transaction_blindings(&source_in.nullifier, &blinding_seed, 2)
            .expect("transaction blindings");
    let [order_blinding, change_blinding]: [_; 2] =
        blindings.try_into().expect("two output blindings");

    let escrow_authority_address = world
        .escrow_owner
        .shielded_address()
        .expect("escrow authority address");
    let order_out = EscrowUtxo {
        blinding: order_blinding,
        ..world.escrow_utxo.clone()
    }
    ...
    let mut taker_change = SppProofOutputUtxo::new(
        world.source_asset,
        FUNDING_AMOUNT - ORDER_AMOUNT,
        user_address,
    )
    .expect("taker_change");
    taker_change.blinding = change_blinding;
    ...
    // The escrow authority owns the data-bearing order output, so it joins the
    // owner signers the proof binds.
    let spp_proof_inputs = SppProofInputs {
        input_utxos,
        output_utxos: encoded.output_utxos,
        external_data,
        payer: world.user_solana.pubkey(),
        blinding_seed,
    ...
    let proof_inputs = EscrowOpenProofInputParams {
        ...
        source_asset: asset_field(&world.source_asset.asset).expect("source asset field"),
...
fn bench_settle(
    ...
    let pool_in = pool_note
        .to_input_utxo(&pool_address, BENCH_TREE_ID)
        .expect("pool_in");

    // The settle seed derives from the order opening, so the taker recovers
    // its payout without a ciphertext; SPP derives every output blinding from
    // it and the order's nullifier.
    let blinding_seed =
        settle_blinding_seed(&order_in.utxo.blinding).expect("settle blinding seed");
    let (private_tx_blinding, blindings) =
        transaction_blindings(&order_in.nullifier, &blinding_seed, 3)
            .expect("transaction blindings");
    let [recipient_blinding, change_blinding, receipt_blinding]: [_; 3] =
        blindings.try_into().expect("three output blindings");
    ...
    let spp_proof_inputs = SppProofInputs {
        ...
        output_utxos: encoded.output_utxos,
        external_data,
        payer: world.authority_solana.pubkey(),
        blinding_seed,
        output_tree_id: BENCH_TREE_ID,
    };
    ...
    let (transact, spp_dur) = prove_transact_timed(
        ...
        &spend_proofs,
        &prover,
        &[world.escrow_owner.as_ref(), world.pool_owner.as_ref()],
    );
    ...
    let proof_inputs = SettleProofInputParams {
        order_in,
        pool_in,
        pool_booked_in: pool_note.note.booked,
        recipient_out,
        pool_change,
        maker_receipt,
        execution_price: PRICE - 1,
        ...
        destination_asset: asset_field(&world.destination_asset.asset)
            .expect("destination asset field"),
...
fn bench_cancel(
    ...
    // The refund seed derives from the order opening, so the taker recovers
    // its refund without a ciphertext.
    let blinding_seed =
        cancel_blinding_seed(&order_in.utxo.blinding).expect("cancel blinding seed");
    let (private_tx_blinding, blindings) =
...
fn bench_withdraw_liquidity(
    ...
    let mint = world.destination_asset.asset;
    ...
    let pool_in = pool_in_note
        .to_input_utxo(&pool_address, BENCH_TREE_ID, 0)
        .expect("pool_in");
    // SPP derives the change blinding from the transaction's seed and the pool
    // input's nullifier.
    let blinding_seed = random_blinding();
    let (private_tx_blinding, blindings) =
        transaction_blindings(&pool_in.nullifier, &blinding_seed, 1)
            .expect("transaction blindings");
    let pool_out_note = PoolUtxo {
        asset: world.destination_asset,
        amount: FUNDING_AMOUNT - amount,
        booked: FUNDING_AMOUNT - amount,
        blinding: blindings.into_iter().next().expect("pool change blinding"),
    };
    ...
    let viewing_key = get_transaction_viewing_key(&world.authority_keypair, &input_utxos)
        .expect("transaction viewing key");
    let encoded =
        encrypt_transaction_data(std::slice::from_ref(&pool_out), &viewing_key, BENCH_TREE_ID)
            .expect("encode outputs");
    let external_data = ExternalData::new(
        *viewing_key.pubkey().as_bytes(),
        encoded.salt,
...
fn bench_rebalance_liquidity(
    ...
    let spp_proof_inputs = prepared
        .spp_proof_inputs(&world.authority_keypair, world.authority_solana.pubkey())
    ...
    // Only the real input is a tree leaf; each dummy input gets a nullifier
    // non-inclusion proof from `prove_transact_timed`.
    let commitments = spp_proof_inputs
        .input_utxo_hashes()
        .expect("input commitments");
    let leaves: Vec<[u8; 32]> = commitments.iter().map(|input| input.utxo_hash).collect();
