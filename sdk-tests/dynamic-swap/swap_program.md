...
- Consolidation is the only place surplus becomes public liquidity again: `consolidate_liquidity` merges pool notes and lets the maker declare a public `credit`, adding exactly that much of the merged notes' surplus to `liquidity_bound`. Timing and granularity are maker-chosen; a zero credit is a pure merge.
...
- Deposit notes are public (amount, owner, blinding, and `booked = amount` are instruction data; the program computes the commitment itself, no proof). Pool confidentiality comes from settle, withdraw, and consolidation change notes with maker-held private blindings and confidential `booked` values. The only encrypted handoff is the order UTXO data at `create_escrow`.
...
| 8 | cancel | 8 | Refunds one escrow after expiry and closes it. One IN1_OUT1 `escrow_cancel` proof spends the order UTXO back to the confidential recipient committed in its data hash: the full `order_amount` in the source asset, with the refund blinding derived from the order blinding. Applies `liquidity_bound += max_order_size` and `open_reservations -= 1`. `rent_recipient` must be the escrow's `owner`. | — | pair account, user escrow account (closed) | Permissionless: only a holder of the order UTXO data (the taker, or the maker via the handoff) can build the proof, and the destination is fixed by the proof |

## Liquidity Accounting

Per-note invariant: `0 <= booked <= amount`. Global invariant:
`sum(booked) >= liquidity_bound + open_reservations * max_order_size`, which
together give `sum(pool UTXO amounts) >= liquidity_bound + open_reservations *
max_order_size`.

| Instruction | liquidity_bound | open_reservations | actual pool total |
|-------------|-----------------|-------------------|-------------------|
| deposit_liquidity | += amount | — | += amount |
| withdraw_liquidity | -= amount (reject if amount > bound) | — | -= amount |
| consolidate_liquidity | += credit | — | — |
...
`owed` leaves the note, and consolidation raises booked by the declared public
`credit`, capped by the merged notes' surplus (`amount_out - booked_1 -
booked_2`). The credit is bounded by surplus that is provably present and not
yet counted, so no value is counted twice; the other transitions only lower
booked or match a public delta, so the bound stays a lower bound on the pool
total. Settle does not change the bound, so the public record of the maker's
liquidity is deposits, withdrawals, and the credits the maker publishes.
...
```
                          ...
                          +==================+
                                   ...
                                   |  rejected if liquidity_bound < max_order_size
                                   |  liquidity_bound -= max_order_size
                                   |  open_reservations += 1
...
3. larger consolidation shapes (INk_OUT1) if pool fragmentation outgrows chained IN2_OUT1 merges
