...
func (c *Circuit) Define(api frontend.API) error {
	...
	privateTxHash := gnarksdk.PrivateTxHash(
		api,
		[]frontend.Variable{sourceInHash},
		[]frontend.Variable{orderOutHash, takerChangeHash},
		c.ExternalDataHash,
		c.PrivateTxBlinding,
	)
...
func (c *Circuit) checkOrderOutputUtxo(api frontend.API) frontend.Variable {
	...
	api.AssertIsEqual(c.OrderOut.DataHash, gnarksdk.Poseidon(api, c.SourceIn.Owner, c.MinPrice))
...
func (c *Circuit) checkTakerChangeOutputUtxo(api frontend.API) frontend.Variable {
	c.TakerChange.AssertDefaultRing(api)
