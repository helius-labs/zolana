...
use timelock_escrow_prover::{
    CircuitId, EscrowProofInputs, FundingProofInput, ProgramUtxoProofInputs,
};
...
mod shared;
use shared::{
    creator, ensure_keys, escrow, escrow_params, funding, generated_vk, keypair, verifies, FUNDING,
    TREE_ID, UNLOCK,
};
...
fn escrow_prove_verify() {
    ...
    let creator = creator();
    let inputs = escrow(&creator, funding(&creator, FUNDING), 250)
        .to_proof_inputs()
...
fn escrow_rejects_zero_amount() {
    ...
    assert!(escrow(&creator, funding(&creator, FUNDING), 0)
        .to_proof_inputs()
        .expect("escrow proof inputs")
        .prove()
        .is_err());
...
fn escrow_zero_change_proves() {
    ensure_keys(CircuitId::Escrow);
    let creator = creator();
    let inputs = escrow(&creator, funding(&creator, FUNDING), FUNDING)
        .to_proof_inputs()
        .expect("escrow proof inputs");
    let proof = inputs.prove().expect("prove failed");
...
fn escrow_outputs_are_derived_from_the_transaction_blinding_seed() {
    ensure_keys(CircuitId::Escrow);
    let creator = creator();
    let mut inputs = escrow(&creator, funding(&creator, FUNDING), 250)
        .to_proof_inputs()
        .expect("escrow proof inputs");
...
fn with_source(
    ...
    source: ProgramUtxoProofInputs<FundingProofInput>,
) -> EscrowProofInputs {
    ...
    inputs.public.private_tx_hash = PrivateTxHash::new(
        &[inputs.source.utxo.hash().expect("source hash"), [0u8; 32]],
        &[
...
#[test]
fn escrow_spends_only_the_creator_s_funding() {
    ensure_keys(CircuitId::Escrow);
    let creator = creator();
    let transaction = escrow(&creator, funding(&creator, FUNDING), 250);
    let own_funding = transaction
        .funding()
        .proof_inputs()
        .expect("funding inputs");
    let locked_escrow = ProgramUtxoProofInputs {
        utxo: transaction
            .escrow_utxo()
            .proof_inputs()
            .expect("escrow inputs")
            .utxo,
        state: own_funding.state,
    };
    let other_funding = funding(&keypair(6), FUNDING)
        .proof_inputs()
        .expect("other funding inputs");

    assert_eq!(
        (
            with_source(&transaction, own_funding).prove().is_ok(),
            with_source(&transaction, locked_escrow).prove().is_err(),
            with_source(&transaction, other_funding).prove().is_err(),
            escrow_params(&creator, funding(&keypair(6), FUNDING), 250)
                .build(&creator)
                .err()
                .map(|e| e.to_string()),
        ),
        (
            ...
            true,
            true,
            Some("the funding belongs to another creator".to_string())
        )
    );
}
