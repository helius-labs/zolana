...
impl<S: ProgramState> NewProgramUtxo<S> {
    ...
    pub fn deposit(&self, asset: DepositAsset) -> Result<AssetDeposit> {
        let settles_the_asset = match &asset {
            DepositAsset::Sol => self.asset == Mint::SOL,
            DepositAsset::Spl(spl) => spl.mint == self.asset.asset,
        };
        if !settles_the_asset {
            bail!("the deposit settles another asset than the program utxo holds");
        }
        Ok(AssetDeposit {
            asset,
            view_tag: self.viewer.x(),
            owner: self.owner.owner_hash()?,
            amount: self.amount,
            utxo_data: Some(UtxoData {
                data_hash: self.state.data_hash()?,
                data: self.state.utxo_data(),
            }),
            memo: None,
        })
    }
