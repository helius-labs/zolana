use zk_program_sdk::{
    circuit::{
        constant, poseidon, zero, CircuitVar, ConstraintSystem, DataHash, DataUtxo, Field,
        TokenUtxo, Utxo,
    },
...
fn a_data_utxo_follows_its_lifecycle() {
    ...
    let spent = Utxo::try_from(&plain_input(12, 7, 17).with_data_hash(counter_hash)).unwrap();
    let mut tokens =
        TokenUtxo::new_mut([Utxo::try_from(&plain_input(11, 50, 1)).unwrap()]).unwrap();
    let funded =
    ...
    assert_eq!(
        (
            ...
            error(DataUtxo::new_burn(
                &Utxo::try_from(&plain_input(12, 7, 17)).unwrap(),
                counter(9)
            )),
        ),
        (
    ...
    assert_eq!(
        (
            ...
            error(TokenUtxo::new_mut([
                Utxo::try_from(&plain_input(11, 300, 1)).unwrap(),
                Utxo::try_from(&plain_input(12, 1, 3)).unwrap(),
            ])),
...
fn a_token_utxo_enforces_its_rules_in_r1cs() {
    ...
    let dummy_with_value = {
        ...
        let token = TokenUtxo::new_mut([
            Utxo::try_from(&plain_input(11, 300, 1))
                .unwrap()
                .instantiate(&allocator)
                .unwrap(),
