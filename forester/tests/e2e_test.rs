...
const INVOKE_CPI_WITH_READ_ONLY_DISCRIMINATOR: [u8; 8] = [86, 47, 163, 166, 21, 223, 92, 8];
...
async fn e2e_test() {
    ...
    let mut config = ForesterConfig {
        ...
        general_config: GeneralConfig {
            ...
            skip_v1_state_trees: !is_v1_state_test_enabled(),
            skip_v2_state_trees: !is_v2_state_test_enabled(),
            skip_v1_address_trees: !is_v1_address_test_enabled(),
            skip_v2_address_trees: !is_v2_address_test_enabled(),
            tree_ids: vec![],
    ...
    let iterations = get_env_usize("FORESTER_E2E_ITERATIONS", 100);
    ...
    let slot_advance_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            let target = current_slot + get_env_u64("FORESTER_E2E_SLOT_WARP_STEP", 50);
            if advance_rpc.warp_to_slot(target).await.is_err() {
                break;
            }
            tokio::time::sleep(Duration::from_millis(get_env_u64(
                "FORESTER_E2E_SLOT_WARP_INTERVAL_MS",
                2000,
            )))
            .await;
        }
    ...
    let expected_minimum_processed_items =
        get_env_usize("FORESTER_E2E_EXPECTED_MIN_PROCESSED_ITEMS", test_iterations);

    let total_processed_items = wait_for_work_report(
        &mut work_report_receiver,
        &state_tree_params,
    ...
    // Shut down the local services before assertions so a failing check does not
    // leave the validator, indexer, or forester background tasks running.
