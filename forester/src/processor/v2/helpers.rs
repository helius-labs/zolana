...
const MAX_QUEUE_ELEMENTS_PER_REQUEST: u64 = 500;
const PAGE_SIZE_BATCHES: u64 = 5;
const ADDRESS_PAGE_SIZE_BATCHES: u64 = 5;

fn queue_page_size_elements(zkp_batch_size: u64, page_batches: u64) -> crate::Result<u64> {
    if zkp_batch_size == 0 {
        return Err(anyhow!("zkp_batch_size cannot be zero"));
    }
    if zkp_batch_size > MAX_QUEUE_ELEMENTS_PER_REQUEST {
        return Err(anyhow!(
            "zkp_batch_size {} exceeds Photon queue request limit {}",
            zkp_batch_size,
            MAX_QUEUE_ELEMENTS_PER_REQUEST
        ));
    }

    let max_batches_per_request = MAX_QUEUE_ELEMENTS_PER_REQUEST / zkp_batch_size;
    let page_batches = page_batches.min(max_batches_per_request).max(1);
    Ok(page_batches * zkp_batch_size)
}
...
pub async fn fetch_paginated_batches<R: Rpc>(
    ...
    let page_size_elements = queue_page_size_elements(zkp_batch_size, PAGE_SIZE_BATCHES)?;
...
pub async fn fetch_streaming_address_batches<R: Rpc + 'static>(
    ...
    let page_size_elements = queue_page_size_elements(zkp_batch_size, ADDRESS_PAGE_SIZE_BATCHES)?;
    let num_pages = total_elements.div_ceil(page_size_elements) as usize;
