...
impl<R: Rpc + Indexer> EpochManager<R> {
    ...
    async fn register_for_epoch_with_retry(
        ...
        for attempt in 0..max_retries {
            let slot = rpc.get_slot().await.map_err(ForesterError::Rpc)?;
            let latest_register_epoch = self
                .protocol_config
                .get_latest_register_epoch(slot)
                .map_err(|error| {
                    ForesterError::Registration(RegistrationError::RegistrationFailed {
                        epoch,
                        error: format!(
                            "failed to determine registerable epoch at slot {slot}: {error:?}"
                        ),
                    })
                })?;
            if latest_register_epoch != epoch {
                info!(
                    event = "registration_epoch_moved",
                    run_id = %self.run_id,
                    epoch,
                    latest_register_epoch,
                    slot,
                    "Skipping registration because this epoch is no longer registerable"
                );
                return Err(ForesterError::Registration(
                    RegistrationError::RegistrationFailed {
                        epoch,
                        error: format!(
                            "slot {slot} is in registration epoch {latest_register_epoch}"
                        ),
                    },
                ));
            }
