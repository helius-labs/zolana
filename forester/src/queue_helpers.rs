...
pub fn parse_state_v2_queue_info(
    ...
    let input_total_zkp_batches = input_parsed.batch_infos.len() as u64
        * input_parsed
            .batch_size
            .checked_div(input_parsed.zkp_batch_size)
            .unwrap_or(0);
    let output_total_zkp_batches = output_parsed.batch_infos.len() as u64
        * output_parsed
            .batch_size
            .checked_div(output_parsed.zkp_batch_size)
            .unwrap_or(0);
...
pub fn parse_address_v2_queue_info(merkle_tree: &BatchedMerkleTreeAccount) -> V2QueueInfo {
    ...
    let input_total_zkp_batches = parsed.batch_infos.len() as u64
        * parsed
            .batch_size
            .checked_div(parsed.zkp_batch_size)
            .unwrap_or(0);
...
pub async fn print_state_v2_input_queue_info<R: Rpc>(
    ...
    if let Some(mut account) = rpc.get_account(*merkle_tree_pubkey).await? {
        ...
        let pending_ops = total_unprocessed
            .checked_div(parsed.zkp_batch_size)
            .unwrap_or(0);
...
pub async fn get_state_v2_output_queue_info<R: Rpc>(
    ...
    if let Some(mut account) = rpc.get_account(*queue_pubkey).await? {
        ...
        let output_total_zkp_batches = parsed.batch_infos.len() as u64
            * parsed
                .batch_size
                .checked_div(parsed.zkp_batch_size)
                .unwrap_or(0);
