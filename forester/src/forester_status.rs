...
pub struct ForesterStatus {
    ...
    /// Total number of trees (active + rolled over)
    pub total_trees: usize,
    /// Number of active (non-rolled-over) trees
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AggregateQueueStats {
    pub state_v1_total_pending: u64,
    pub state_v2_input_pending: u64,
...
pub async fn get_forester_status_with_options(
    ...
    if !active_epoch_forester_pdas.is_empty() {
            ...
            .filter(|&w| w > 0)
        {
            ...
            if let Some(active_light_slots) =
                epoch_phases.active.length().checked_div(light_slot_length)
            {
                total_light_slots = active_light_slots;

                // Compute current light slot if in active phase
                if slot >= epoch_phases.active.start && slot < epoch_phases.active.end {
                    let current_light_slot_index = (slot - epoch_phases.active.start)
                        .checked_div(light_slot_length)
                        .unwrap_or(0);
    ...
    // Compute aggregate statistics from tree statuses
        ...
        match t.tree_type.as_str() {
            ...
            "StateV2" => {
                if let Some(ref info) = t.v2_queue_info {
                    aggregate_queue_stats.state_v2_input_pending +=
...
async fn fetch_registry_accounts_filtered<R: Rpc>(
    ...
    forester_epoch_pdas.sort_by_key(|a| a.epoch);
    epoch_pdas.sort_by_key(|a| a.epoch);
    ...
    if let Some(_current_epoch_pda) = current_epoch_pda_entry {
        if active_epoch_foresters.is_empty() {
            ...
            if total_epoch_weight > 0 {
                ...
                if slot >= epoch_phases.active.start && slot < epoch_phases.active.end {
                    if let Some(current_light_slot_index) = (slot - epoch_phases.active.start)
                        .checked_div(protocol_config.config.slot_length)
                    {
                        let start_solana_slot_of_current_light_slot = epoch_phases.active.start
                            + current_light_slot_index * protocol_config.config.slot_length;
                        let end_solana_slot_of_current_light_slot =
                ...
                let num_light_slots = match epoch_phases
                    .active
                    .length()
                    .checked_div(protocol_config.config.slot_length)
                {
                    Some(num_light_slots) => num_light_slots,
                    None => {
                        println!("error: protocol config slot_length is zero; cannot calculate light slots");
                        0
                    }
