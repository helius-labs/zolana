...
//! Ported from the shielded-pool `instructions/shared.rs`, with mismatches mapped
//! to [`SquadsZoneError`]. The syscall-bound derivations are cfg-gated for the
//! Solana target so the host build (default features, no entrypoint) compiles.
...
/// program: `create_program_address([seed, bump], program_id)`. Maps a mismatch
/// to [`SquadsZoneError::InvalidZoneAuth`].
