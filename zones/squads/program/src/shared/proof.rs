...
/// Fixed-capacity builder for a public-input chain. Overflow is an error, never
/// a dropped element: a shorter chain hashes to a value the circuit never
/// constrained.
...
impl<const N: usize> Chain<N> {
    pub fn push(&mut self, element: [u8; 32]) -> Result<(), ProgramError> {
        let slot = self
            .elements
            .get_mut(self.len)
...
/// The high and low 128-bit halves of a 32-byte big-endian value.
fn split_16(value: &[u8; 32]) -> ([u8; 16], [u8; 16]) {
    let mut high = [0u8; 16];
    let mut low = [0u8; 16];
...
fn right_align_16(bytes: &[u8; 16]) -> [u8; 32] {
    let mut out = [0u8; 32];
...
/// A single multi-input Poseidon over `inputs`, matching the Go circuits'
/// `gadget.PoseidonHash` (state width `t = inputs.len() + 1`,
/// prover/server/circuits/gadget/poseidon.go:51).
#[inline(never)]
