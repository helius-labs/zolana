//! Offline unit tests for the P256 probe/finalize split and recipients addressed
//! by `owner_pk_field`. None of these contact the prover
...
use zolana_transaction::{Address, Data, SppProofOutputUtxo, SOL_MINT};
...
use crate::prover::{
    ...
    SquadsWithdrawalProbe, WithdrawalDestination,
};
...
/// the tree heights so the local witness assembly accepts it. Only the circuit
/// checks the Merkle contents.
...
fn random_identity() -> SquadsIdentity {
    SquadsIdentity {
        ...
        nullifier_secret: random_blinding_31(),
        viewing_secret: SecretKey::random(&mut OsRng),
    }
}

fn recipient_output(signing_pubkey: PublicKey, nullifier_pubkey: [u8; 32]) -> SppProofOutputUtxo {
    SppProofOutputUtxo {
        asset: SOL_MINT,
        amount: 400,
        blinding: [3u8; 32],
        ring_program_id: Some(Address::default()),
        ring_data_hash: None,
        data_hash: None,
...
/// A recipient `SppProofOutputUtxo` addressed by a precomputed `owner_pk_field` hashes
...
fn recipient_owner_pk_field_hash_matches_pubkey_path() {
    ...
    assert_eq!(
        PublicKey::from_owner_proof_input_hash(owner_pk_field)
            .owner_proof_input_hash()
...
fn build_transfer_probe(identity: &SquadsIdentity) -> SquadsTransferProbe {
    ...
    SquadsTransferProbe {
        ...
        recipient,
        transferred: 400,
        recipient_blinding: random_blinding_31(),
...
fn transfer_probe_deterministic_and_signature_independent() {
    ...
    // Rebuild an identical probe from the first probe's fields to check determinism.
