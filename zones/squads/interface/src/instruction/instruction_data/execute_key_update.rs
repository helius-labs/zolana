...
pub struct ExecuteKeyUpdateIxData {
    ...
    /// The account's nullifier public key. A rotation re-encrypts the same
    /// nullifier secret, so this must equal the stored value. Existing UTXOs
    /// bind it, and a changed key makes them unspendable.
