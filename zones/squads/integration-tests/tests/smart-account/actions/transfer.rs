...
use crate::{
    ...
    harness::SquadsSmartAccountHarness,
...
impl SquadsSmartAccountHarness {
    ...
    pub(crate) fn send_execute_v0_alt(&mut self, ix: Instruction) -> Result<Signature> {
        ...
        let signers: [&Keypair; 4] = [&payer, &co_signer, &member, &member_b];
        self.send_v0_with_alt(wrapped, &signer_pubkeys, &signers)
    }
