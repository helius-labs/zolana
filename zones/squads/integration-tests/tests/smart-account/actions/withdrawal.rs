...
//! backend. The proposal format binds an SPL token destination as well, but the
//! mock crank does not yet assemble SPL settlement accounts, so the SPL async
...
use zolana_squads_sdk::proposal::{proposal_hash, ProposalOperation};
use zolana_test_utils::{spl::create_token_account, test_validator_asserts::to_address};
use zolana_transaction::{Address, SOL_MINT};

/// A fresh 31-byte blinding, the form the proposal and decrypted UTXO carry.
...
use crate::{
    actions::transfer::placeholder_encrypted_utxos,
    fixture::PROPOSAL_WITHDRAWN,
    harness::{SquadsSmartAccountHarness, WithdrawalRecord},
};

fn proposal_expiry() -> i64 {
...
/// The SPL asset id the suite registers (mirrors `harness::FIRST_SPL_ASSET_ID`).
...
impl SquadsSmartAccountHarness {
    ...
    pub(crate) fn withdraw_spl(&mut self, name: &str, withdrawn: u64) -> Result<()> {
        ...
        let vault = pda::spl_interface(&spl.mint);
    ...
    pub(crate) fn create_withdrawal_proposal(&mut self, name: &str) -> Result<()> {
        ...
        let proposal_address = self.queue_proposal(
            hash,
            SquadsAddress::new_from_array(self.proposer_vault.to_bytes()),
