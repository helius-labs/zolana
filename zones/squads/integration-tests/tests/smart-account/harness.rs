...
use zolana_squads_client::{
    authorization::InsecurePermissiveRead, DecryptedUtxo, GetBalancesRequest, SquadsBackend,
};
...
fn ring_auth_pda() -> Pubkey {
    Pubkey::find_program_address(&[RING_AUTH_PDA_SEED], &squads_program_id()).0
}

pub struct SquadsSmartAccountHarness {
    ...
    /// The proposer smart account's vault (account index 0). It is the shielded
    /// UTXO owner and the inner-instruction executor.
    ...
    pub(crate) backend: SquadsBackend<ZolanaIndexer, SolanaRpc, InsecurePermissiveRead>,
}

impl std::fmt::Debug for SquadsSmartAccountHarness {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("SquadsSmartAccountHarness")
    }
}
...
impl SquadsSmartAccountHarness {
    pub fn new() -> Result<Self> {
        ...
        .with_read_authorization(InsecurePermissiveRead);
    ...
    pub(crate) fn ensure_spl_asset(&mut self) -> Result<()> {
        ...
        let ix = CreateSplInterface {
            authority: protocol_vault,
            mint,
            token_program: Pubkey::new_from_array(zolana_interface::SPL_TOKEN_PROGRAM_ID),
        }
        .instruction();
        let sync_ix = execute_sync_ix(&protocol_settings, 0, &[authority.pubkey()], &[ix]);
        send_transaction(
