...
pub(crate) fn restart_localnet() {
    ...
    // Viewing key accounts are created at runtime through the backend (random
    // secrets recoverable via the auditor), so only the smart-account program
    // config is seeded at genesis.
    let account_dir = format!("/tmp/zolana-squads-sa-accounts-{rpc_port}");
    // Start from a clean directory: the genesis load pulls in EVERY json here, so a
    // stale viewing-key fixture from an earlier suite version would seed a second
    // VKA for a runtime owner. The crank resolves a VKA by owner scan, so that
    // duplicate (a different shared key at a non-canonical address) shadows the
    // runtime account and breaks proposal decryption. Only the program config is
    // seeded at genesis; viewing key accounts are created at runtime.
    ...
    let restart_once = || -> bool {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let ledger = format!("/tmp/zolana-squads-sa-ledger-{nanos}");
...
pub(crate) fn send_v0_transaction(
    ...
    loop {
        ...
        // fast with its real cause instead of timing out. Only return once the
        // status reaches the `confirmed` commitment that `get_account` reads at:
        // a merely `processed` status can be observed before the confirmed bank
        // reflects the settlement writes, so a caller reading a fresh withdrawal
        // recipient would otherwise see a stale (empty) account.
