//! Rail-selection unit tests for `request_transact` and
//! `request_transact_probe`. The full P256 probe and finalize flow needs the
//! indexer and prover server and runs in the e2e suite. These tests cover the
//! rail dispatch and error paths, pure client logic reached before any RPC
//! call.
...
fn no_owner_pubkey_selects_smart_account_rail() {
    // The smart-account rail resolves the sender VKA first, so the empty mock
    // surfaces AccountNotFound instead of the P256 owner-signature error.
    let request = RequestTransactRequest {
