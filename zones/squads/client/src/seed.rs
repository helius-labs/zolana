...
fn to_shared_ct(bytes: Vec<u8>) -> Result<SharedKeyCiphertext> {
    bytes
        .as_slice()
        .try_into()
        .map_err(|_| SquadsBackendError::Malformed("shared key ciphertext length".into()))
}

/// Encrypt `viewing_sk_be` to `recipient` under `ephemeral`, matching
/// `KeyEncryptionProofInputs::prove`'s per-recipient ciphertext.
fn encrypt_shared_key(
    ...
    let dh = ViewingKey::from_secret_key(ephemeral_sk.clone()).ecdh(recipient)?;
    let ciphertext = crypto::ecdh_encrypt(&dh, ephemeral_comp, recipient.as_bytes(), viewing_sk_be)
        .map_err(crypto_err)?;
    to_shared_ct(ciphertext)
}

/// Build a viewing key account whose shared viewing secret is recoverable by
/// each recovery key and each auditor key, and whose nullifier secret is
/// encrypted to the shared viewing key. `nullifier_secret` is the 32-byte field
/// element. The account stores its low 31 bytes.
pub fn seed_viewing_key_account(
    ...
    let null_plaintext = nullifier_secret
        .get(1..32)
        .ok_or_else(|| SquadsBackendError::Malformed("nullifier secret length".into()))?;
    let dh_null = ViewingKey::from_secret_key(ephemeral_sk.clone()).ecdh(&shared_viewing_pk)?;
    ...
    let encrypted_nullifier_secret: EncryptedNullifierSecret = nullifier_ciphertext
        .as_slice()
        .try_into()
        .map_err(|_| SquadsBackendError::Malformed("nullifier ciphertext length".into()))?;
