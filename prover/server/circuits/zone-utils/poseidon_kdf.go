...
// Domain separators packed into a field element. Copied from
// confidential-transfers (encryption.rs); rename to TSPP separators before use.
const (
...
// PoseidonKDF is one keyed step of the zone KDF chain: a Poseidon hash with
// KdfDomainSep prepended as the first input. Spec: squads_policy_program.md,
// Zone Proof "Change-blinding derivation".
...
// ForceEqualIfEnabled enforces a == b when enabled is non-zero by
// constraining (a-b)*enabled === 0.
