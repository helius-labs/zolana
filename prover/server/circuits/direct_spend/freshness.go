...
func (f *Freshness) constrain(api frontend.API) error {
	...
	for i, witness := range f.Witnesses {
		...
		root := abstractor.Call(api, gadget.MerkleRootGadget{
			Hash:  gadget.IndexedLeafHash(api, witness.Low, witness.Next),
			Index: api.ToBinary(witness.Index, transaction.NullifierTreeHeight),
			Path:  witness.Path, Height: transaction.NullifierTreeHeight,
		})
