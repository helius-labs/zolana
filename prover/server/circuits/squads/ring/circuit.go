...
type Circuit struct {
	...
	// is structurally real because its nullifier seeds the tx_viewing_sk KDF. On
	// the (1,1) shape the slice is empty, leaving that constraint system unchanged.
...
// NewWithdrawalCircuit builds the withdrawal shape with a single sender change
...
func newCircuit(numInputs, numOutputs int) *Circuit {
	return &Circuit{
		...
		Transaction: squadsutils.Transaction{
			Inputs:  make([]squadsutils.Utxo, numInputs),
			Outputs: make([]squadsutils.Utxo, numOutputs),
		},
...
func (c *Circuit) Define(api frontend.API) error {
	...
	// NumOutputs is a compile-time constant. A withdrawal (1-out) has no recipient,
	// so its recipient amount is 0 and the recipient constraints are omitted entirely.
	...
	senderCiphertextHash, err := c.Sender.Constrain(api, g, c.Transaction, c.InputsDummy, txViewingSk, c.PublicAmount, recipientAmount)
	if err != nil {
		return err
	}

	// The public input chain binds private_tx_hash, public_amount, sender account,
	// sender ciphertext, then (transfer only) tx_viewing_pk + recipient account +
	...
	if hasRecipient {
		// tx_viewing_pk equals tx_viewing_sk · G, derived rather than witnessed, so the
		// keypair is consistent. Bound into the chain so the recipient derives the same
		// ECDH key.
		var txViewingSkBytes [32]frontend.Variable
		copy(txViewingSkBytes[:], squadsutils.FieldToBytesBE(api, txViewingSk, 32))
		txViewingPkComp := p256.CompressPubkey(api, p256.ScalarMulGenerator(api, txViewingSkBytes))
		pkLo, pkHi := squadsutils.Pack33To2FECircuit(api, txViewingPkComp)
	...
	proposalHash := c.Proposal.Constrain(
		api,
		c.Transaction,
		c.InputsDummy,
