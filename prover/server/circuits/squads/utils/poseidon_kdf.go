...
// Domain separators packed into a field element. MUST match the Rust
// encryption.rs key schedule byte for byte.
const (
	DomSepSharedSecret uint32 = 0x43545f53 // "CT_S"
	DomSepSilo         uint32 = 0x43545f49 // "CT_I"
	DomSepKey          uint32 = 0x43545f4b // "CT_K" (key_sep_0; key_sep_1 = +1)
...
//
// The layout matches the Rust.
//   - split = min(31, infoLen)
//   - fe0[0] = infoLen (length byte)
//   - fe0[32-split..32] = info[0..split]
//   - fe1[32-(infoLen-split)..32] = info[split..infoLen]
//
// When infoLen <= 30 the length byte at fe0[0] does not overlap any data
// byte. When infoLen == 31 the length byte occupies the position immediately
// above data, with no gap.
...
// ForceEqualIfEnabled enforces a == b when enabled is non-zero.
func ForceEqualIfEnabled(api frontend.API, a, b, enabled frontend.Variable) {
	api.AssertIsEqual(api.Mul(api.Sub(a, b), enabled), frontend.Variable(0))
}
...
func feToBytesBE(api frontend.API, fe frontend.Variable) [32]frontend.Variable {
	// gnark caps the 256-bit decomposition at the field bit length and pads the
	// top with 0.
	allBits := bits.ToBinary(api, fe, bits.WithNbDigits(256))

	var out [32]frontend.Variable
	for byteIdx := 0; byteIdx < 32; byteIdx++ {
		// Big-endian means byte 0 is the most significant byte (bits 248..255).
		bytePos := 31 - byteIdx
		startBit := bytePos * 8
...
//
// sharedSecret is a single field element, the output of DeriveSharedSecret.
// info holds variable info bytes and only info[0:infoLen] is consumed.
// infoLen is the compile-time length of the active info prefix and must be
// <= 62.
//
...
//
// dh is the 32-byte ECDH x-coordinate.
// encCompressed is the 33-byte compressed ephemeral pubkey.
// rpkCompressed is the 33-byte compressed recipient pubkey.
//
