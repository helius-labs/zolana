...
// ecdhEncrypt is one verifiable encryption to a recipient. ECDH against the
// recipient key under the shared ephemeral, Poseidon key schedule, AES-CTR
// over the plaintext. Returns the Poseidon hash of the ciphertext for the
// public input chain. Integrity comes from the returned hash, not a GCM tag.
func ecdhEncrypt(
	api frontend.API,
	g *aes.AESGadget,
	...
	sharedSecret := squadsutils.DeriveSharedSecret(api, dh, ephPkComp, rpkComp)
