...
type Circuit struct {
	...
	// creation. The on-chain program checks it against the stored
	// keys-and-ciphertexts hash, the circuit only folds it into the chain.
	OldStateHash frontend.Variable

	// ViewingSecretKey is the shared viewing scalar. Its public key is the
	// account's shared_viewing_key and the scalar is the 32-byte plaintext
	...
	// ciphertexts, witnessed full range.
...
func (c *Circuit) Define(api frontend.API) error {
	...
	sharedPkLo, sharedPkHi := squadsutils.Pack33To2FECircuit(api, sharedPkComp)
	...
	ephPkLo, ephPkHi := squadsutils.Pack33To2FECircuit(api, ephPkComp)

	// Public input chain in fixed order, recomputed by the on-chain program from
	...
	for i := range c.RecipientKeys {
		...
		rpkLo, rpkHi := squadsutils.Pack33To2FECircuit(api, rpkComp)
		ctHash := ecdhEncrypt(api, g, ephBytes, ephPkComp, pk, rpkComp, viewingSkBytes[:])
		chain = append(chain, rpkLo, rpkHi, ctHash)
	}

	// The published nullifier_pubkey commits to the secret, and the secret is
	...
	nullPlaintext := squadsutils.FieldToBytesBE(api, c.NullifierSecret, 31)
