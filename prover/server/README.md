...
7. extract-circuit - Transpiles the inclusion and non-inclusion circuits from gnark to Lean
   Flags:  
   1. output *file path* - File to be written to
   2. state-tree-height *n* - State (UTXO) merkle tree height (32)
   3. nullifier-tree-height *n* - Nullifier indexed merkle tree height (40)
   4. batch-size *n* - Number of UTXOs / nullifiers per circuit (8)
...
The inclusion (state tree) and non-inclusion (nullifier tree, full-field
low-leaf bracketing) circuits are extracted to Lean and verified against the
committed proofs in `formal-verification/`. `Circuit.lean` is generated by
`extract-circuit` and committed; CI re-extracts it and fails on drift, then
runs `lake build` to check every theorem.

1. Install [Elan](https://github.com/leanprover/elan).
2. ```
   just verify        # re-extracts Circuit.lean, then builds the Lean project
   ```
   or manually:
   ```
