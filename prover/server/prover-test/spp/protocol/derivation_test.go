...
func TestBlindingSeedDomainsAreAsciiTags(t *testing.T) {
...
// TestBlindingSeedChildrenMatchPinnedVectors fixes the wire-level
// derivation. The Rust client must reproduce these exact values for
// first_nullifier = 7 and blinding_seed = 42; a mismatch means client and circuit
// disagree on private_tx_hash or on every output blinding.
func TestBlindingSeedChildrenMatchPinnedVectors(t *testing.T) {
	for _, tc := range []struct {
		name   string
		derive func(firstNullifier, blindingSeed *big.Int) (*big.Int, error)
		want   string
	}{
		{
...
// TestBlindingSeedChildrenDiffer is what the domain separation buys: one
// seed and one first nullifier produce two unrelated children, so a party
// holding one cannot reach the other.
func TestBlindingSeedChildrenDiffer(t *testing.T) {
	...
	if seed.Cmp(blinding) == 0 {
		t.Fatal("the two children of one blinding seed collided")
	}
}

// TestBlindingSeedChildrenBindFirstNullifier is the uniqueness property:
// a nullifier enters the nullifier tree once, so reusing a blinding seed
// across two accepted transactions still yields different children.
func TestBlindingSeedChildrenBindFirstNullifier(t *testing.T) {
	for _, tc := range []struct {
		name   string
		derive func(firstNullifier, blindingSeed *big.Int) (*big.Int, error)
	}{
...
func TestBlindingSeedChildrenRejectInvalidFieldElements(t *testing.T) {
	...
	if _, err := PrivateTxBlinding(fe(7), nil); err == nil {
		t.Fatal("expected a nil blinding seed to fail")
	}
}
