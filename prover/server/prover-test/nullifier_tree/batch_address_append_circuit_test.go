...
func TestBatchAddressAppendCircuit(t *testing.T) {
	...
	t.Run("Basic operations", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				circuit := nullifiertree.InitBatchAddressTreeAppendCircuit(tc.treeHeight, tc.batchSize)
	...
	t.Run("Invalid cases", func(t *testing.T) {
		testCases := []struct {
			...
			batchSize    uint32
			startIndex   uint64
			modifyParams func(*nullifiertree.BatchAddressAppendParameters)
			wantPanic    bool
		}{
			{
				...
				batchSize:  1,
				startIndex: 0,
				modifyParams: func(p *nullifiertree.BatchAddressAppendParameters) {
