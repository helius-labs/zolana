...
// InnerSystem compiles the append circuit so the outer circuit can be built
...
func InnerSystem(p Params) (constraint.ConstraintSystem, error) {
	...
	return nullifiertree.R1CSBatchAddressAppend(p.TreeHeight, p.BatchSize)
}
...
func R1CSFold(p Params, innerCcs constraint.ConstraintSystem, innerVk groth16.VerifyingKey) (constraint.ConstraintSystem, error) {
	...
	circuit, err := foldcircuit.NewCircuit(int(p.Run), int(p.BatchSize), fixed, innerCcs)
	if err != nil {
