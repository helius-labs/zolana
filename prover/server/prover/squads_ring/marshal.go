...
func (p *RingParameters) toJSON() RingParametersJSON {
	out := RingParametersJSON{
		...
		Proposal: ProposalParamsJSON{
			...
			Asset:        common.FeHex(p.Proposal.Asset),
			Destination:  common.FeHex(p.Proposal.Destination),
...
func (p *RingParameters) updateWithJSON(params RingParametersJSON) error {
	...
	if len(pk) != 65 {
		return fmt.Errorf("recipient viewingPubkey must have 65 elements, got %d", len(pk))
	}
	for i := 0; i < 65; i++ {
	...
	if p.Proposal.Asset, err = common.FeFromHex(params.Proposal.Asset); err != nil {
		return err
	}
	if p.Proposal.Destination, err = common.FeFromHex(params.Proposal.Destination); err != nil {
		return err
	}
