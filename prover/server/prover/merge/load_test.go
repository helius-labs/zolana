...
// tag it MergeCircuitType, and report the shape the file name declares. Skips
// per shape when the (gitignored) key is absent.
func TestReadMergeKeyDispatch(t *testing.T) {
	for _, nInputs := range SupportedNInputs() {
		t.Run(fmt.Sprintf("%d_inputs", nInputs), func(t *testing.T) {
			...
			if ps.NInputs != nInputs || ps.NOutputs != MergeNOutputs {
				t.Fatalf("shape: got %dx%d want %dx%d", ps.NInputs, ps.NOutputs, nInputs, MergeNOutputs)
			}
		})
	}
}
