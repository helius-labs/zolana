...
func TestBuildProofAssignmentRequiresExactShapeArity(t *testing.T) {
	...
	_, _, _, _, err := buildProofAssignment(shape, ProofTransactionRequest{
...
func TestBuildProofAssignmentRejectsMixedNullifierSecrets(t *testing.T) {
	shape := model.Shape{NInputs: 2, NOutputs: 2}
	...
	_, _, _, _, err = buildProofAssignment(shape, tx, signerHash, proofBuildOptions{})
...
func TestBuildProofAssignmentRejectsBadPublicAmountRequests(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: "transfer mode carries public amounts",
		},
		{
