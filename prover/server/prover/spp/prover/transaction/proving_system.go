...
func Compile(shape model.Shape) (constraint.ConstraintSystem, error) {
...
func Setup(shape model.Shape) (*ProofSystem, error) {
...
func (ps *ProofSystem) UnsafeReadFrom(r io.Reader) (int64, error) {
	...
	shape, err := model.NewShape(int(nInputs), int(nOutputs))
	if err != nil {
		return total, err
	}
	ps.Shape = shape
