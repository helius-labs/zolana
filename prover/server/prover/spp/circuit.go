...
type Circuit struct {
	...
	PreNullifiers  []frontend.Variable
...
func (c *Circuit) Define(api frontend.API) error {
	...
	for i := 0; i < c.Shape.NInputs; i++ {
		...
		// v1 Solana-signer ownership branch. TODO(v1): add the P256 branch
		// without gnark commitment public inputs so it remains compatible with
		// the Solana Groth16 verifier.
		api.AssertIsEqual(api.Mul(notDummy, api.Sub(c.InputUtxos[i].Owner, c.SolanaPubkeyHash)), 0)

		nullifier := NullifierHashCircuit(api, inputHash, c.PreNullifiers[i])
		...
		api.AssertIsEqual(api.Mul(c.IsDummyInput[i], c.PreNullifiers[i]), 0)
...
func (c *Circuit) validateShape() error {
	...
	if len(c.PreNullifiers) != c.Shape.NInputs {
		return fmt.Errorf("spp: pre-nullifier count mismatch: got %d want %d", len(c.PreNullifiers), c.Shape.NInputs)
	}
