...
func buildDummyInputShield(t testing.TB, deposit int64) *Circuit {
	...
	assignment := buildCircuitAssignmentFromUtxos(
		t,
		shape,
		[]protocol.Utxo{sampleUtxoWithAssetAndAmount(10, solAssetID, spptest.Fe(50))},
		twoOutputUtxos(sampleUtxoWithAssetAndAmount(100, solAssetID, spptest.Fe(deposit))),
		big.NewInt(deposit),
		big.NewInt(0),
		spptest.Fe(0),
	)
	...
	// nullifier, zero roots, and zero Solana key hash (the exact fields the
	// inertness constraints pin). The remaining witness fields are gated on
	// notDummy and so are ignored.
	in := &assignment.Inputs[0]
	in.IsDummy = spptest.Fe(1)
	in.Utxo.AssetAmount = spptest.Fe(0)
	in.UtxoTreeRoot = spptest.Fe(0)
	in.NullifierRoot = spptest.Fe(0)
	in.SolanaPkHash = spptest.Fe(0)
...
// (spend.go: assertZeroWhen(IsDummy, AssetAmount)). AssetAmount is not a public
...
func TestDummyInputRejectsNonZeroAmount(t *testing.T) {
	...
	assignment.Inputs[0].Utxo.AssetAmount = spptest.Fe(1)
