...
// ProofSystem is the SPP Groth16 proving system for one fixed circuit shape.
...
func Compile(shape Shape) (constraint.ConstraintSystem, error) {
...
func Setup(shape Shape) (*ProofSystem, error) {
...
func (ps *ProofSystem) UnsafeReadFrom(r io.Reader) (int64, error) {
	...
	shape, err := NewShape(int(nInputs), int(nOutputs))
	if err != nil {
		return total, err
	}
	ps.Shape = shape
