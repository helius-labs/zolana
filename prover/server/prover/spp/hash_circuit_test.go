...
func (c *hashParityCircuit) Define(api frontend.API) error {
	...
	preNullifier := PreNullifierCircuit(api, c.Utxo.Blinding, c.NullifierSecret)
	api.AssertIsEqual(preNullifier, c.PreNullifier)

	nullifier := NullifierHashCircuit(api, utxoHash, c.PreNullifier)
...
func TestHashCircuitMatchesNative(t *testing.T) {
	...
	preNullifier := mustPreNullifier(t, utxo.Blinding, nullifierSecret)
