...
func TestIndexedTranscriptVector(t *testing.T) {
	...
	for index := range slots {
		...
		if index < len(vector.Trees) {
			...
			fields[1], err = FeFromHex(tree.UtxoRoot)
			if err != nil {
				t.Fatal(err)
			}
			fields[2], err = FeFromHex(tree.NullifierRoot)
			if err != nil {
				t.Fatal(err)
			}
		}
		slots[index], err = HashFields(fields)
		if err != nil {
			t.Fatal(err)
		}
	}
	tree, err := RightHashChain(slots)
	if err != nil {
		t.Fatal(err)
	}
	public, err := FeFromHexSlice(vector.PublicInputs)
	if err != nil {
		t.Fatal(err)
	}
	...
	hash, err := HashChain4(transcript)
	if err != nil {
		t.Fatal(err)
	}
	if FeHex(hash) != vector.PublicInputHash {
