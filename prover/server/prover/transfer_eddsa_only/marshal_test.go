...
// travels as one secret the circuit expands.
func TestTransferParametersJSONKeys(t *testing.T) {
	...
	for _, key := range []string{"treeSlots", "outputTreeId", "txSecret"} {
	...
	for _, key := range []string{"outputBlindingSeed", "privateTxBlinding"} {
		if _, ok := fields[key]; ok {
			t.Fatalf("%s must not be on the wire; the circuit derives it from txSecret", key)
		}
	}
...
func TestTransferParametersDecodeTreeSlots(t *testing.T) {
	...
	if got.TxSecret.Cmp(want.TxSecret) != 0 {
		t.Fatalf("tx secret: got %s want %s", got.TxSecret, want.TxSecret)
	}
}

// TestTransferParametersRejectTxSecret guards the one field the whole
// transaction's randomness hangs off. A zero or omitted secret gives an observer
...
func TestTransferParametersRejectTxSecret(t *testing.T) {
	data, err := json.Marshal(sampleTransferParams(RingVariant))
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
...
// signal, so a missing TreeSlots, OutputTreeID, or TxSecret fails here.
