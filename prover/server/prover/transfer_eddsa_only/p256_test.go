...
// Solana-only encoder, so it must carry the same tree slots and single tx
// secret and none of the per-input roots or pre-derived blindings.
func TestP256TransferParametersJSONRoundTrip(t *testing.T) {
	...
	if decoded.TxSecret.Cmp(params.TxSecret) != 0 {
		t.Fatalf("tx secret = %v", decoded.TxSecret)
	}
...
// TestP256TransferParametersRejectTxSecret guards the transaction's single
// private random value: the circuit derives the private tx blinding and every
// output blinding from it, so a zero or omitted secret hands an observer both
// and must be refused at parse time.
func TestP256TransferParametersRejectTxSecret(t *testing.T) {
