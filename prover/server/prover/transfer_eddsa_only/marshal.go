...
func (p *TransferParameters) CreateTransferParametersJSON() TransferParametersJSON {
	...
	paramsJson := TransferParametersJSON{
		...
		TxSecret:                     common.FeHex(p.TxSecret),
...
func (p *TransferParameters) UpdateWithJSON(params TransferParametersJSON) error {
	...
	// blinding and every output blinding from this one secret, so a zero or
	// omitted secret hands an observer a known blinding and predictable output
	// blindings. Fail at parse time rather than as a confusing proving error.
	if params.TxSecret == "" {
		return fmt.Errorf("spp: txSecret is required")
	}
	if p.TxSecret, err = common.FeFromHex(params.TxSecret); err != nil {
		return err
	}
	if p.TxSecret.Sign() == 0 {
		return fmt.Errorf("spp: txSecret must not be zero")
	}
