...
// InnerSystem compiles the key-encryption circuit so the outer circuit can be
// built against its constraint system. Setup also needs the inner verifying
// key, which only exists in the inner proving system, so the caller supplies
// that.
func InnerSystem(p Params) (constraint.ConstraintSystem, error) {
	if err := p.Validate(); err != nil {
		return nil, err
	}
	return keyencryption.R1CSKeyEncryption(p.KeysPerLeg)
}
...
func R1CSFold(p Params, innerCcs constraint.ConstraintSystem, innerVk groth16.VerifyingKey) (constraint.ConstraintSystem, error) {
	...
	circuit, err := foldcircuit.NewCircuit(int(p.Legs), int(p.KeysPerLeg), fixed, innerCcs)
	if err != nil {
