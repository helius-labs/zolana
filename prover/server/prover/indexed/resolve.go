...
func (r *Resolver) resolve(ctx context.Context, data []byte) (*Resolved, error) {
	...
	for treeIndex, tree := range request.Trees {
		...
		for index, input := range group.inputs {
			...
			leaf, err := common.HashFields([]*big.Int{low, high})
			if err != nil {
				return nil, err
			}
	...
	for index, slot := range slots {
		hashes[index], err = common.HashFields([]*big.Int{slot.ID, slot.UtxoRoot, slot.NullifierRoot})
		if err != nil {
			return nil, err
		}
	}
	treeHash, err := common.RightHashChain(hashes)
	if err != nil {
		return nil, err
	}
	...
	*prepared.hash, err = common.HashChain4(transcript)
	if err != nil {
		return nil, err
	}
...
func verifyPath(leaf Hash, path []Hash, index uint64, root Hash) error {
	...
	for level, sibling := range path {
		...
		hash, err = common.HashFields(pair)
		if err != nil {
			return err
		}
	}
