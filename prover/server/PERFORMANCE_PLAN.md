...
1. Make Redis dequeue and processing registration atomic, with a renewable
   ...
   permit waiting.
2. Benchmark CPU affinity and Gnark MSM parallelism together with worker count.
...
3. Benchmark GPU batching behind the same bounded execution interface. Track
...
4. Profile a representative traffic mix and test a PGO build against the same
   workload. Keep the profile tied to a circuit mix and Go version. Compare
   throughput and p95 latency, not binary size.
5. Consider server push for queued results only after measuring fallback rate.
