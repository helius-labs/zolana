...
ring_vkey_dir="$vkey_dir"
ring_vk_bin="$tmp_dir/auditor_key_encryption.vkbin"
echo "exporting raw vk: auditor_key_encryption"
./light-prover export-vk --keys-file "$ring_key" --output "$ring_vk_bin" >/dev/null
"$xtask" bsb22-vk "$ring_vk_bin" "$ring_vkey_dir" "auditor_key_encryption.rs"
rustfmt "$ring_vkey_dir/auditor_key_encryption.rs"
