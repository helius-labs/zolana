...
#
# Each timed operation is one server-side proof: witness assembly plus
# groth16.Prove. Circuit compilation and the Groth16 setup run before the timer
# starts and are excluded.
#
# Keys are generated locally by a real setup rather than loaded from the
# committed set, because this branch's circuits changed and no pinned key can
# prove their witnesses. That makes a run slower to start but leaves the timings
# comparable to earlier committed-key rows.
#
# Combinations that fail (for example a circuit whose constraint count no longer
...
#
# Env:
#   ZOLANA_BENCH_ALL_SHAPES=1  bench every supported shape instead of the subset
set -euo pipefail
...
# -v is required: without it, go test hides the per-combination skip and failure
# lines that the results section reports.
status=0
...
trap 'rm -f "$raw_file" "$section_file"' EXIT
{
    ...
    echo
    echo "Locally generated setup, GOMAXPROCS ${procs:-unknown}, shapes: ${shapes}."
    echo
