...
python3 scripts/generate_lockfile.py "$keys_dir" --release custom_ring_policy.key --release custom_ring_base.key --release custom_ring_compressed_policy.key --release custom_ring_compressed_register.key
...
aws s3 sync "$keys_dir/" "s3://$bucket/$lock_prefix/" --exclude '*' --include '*.key' --exclude 'custom_ring_policy.key' --exclude 'custom_ring_base.key' --exclude 'custom_ring_compressed_policy.key' --exclude 'custom_ring_compressed_register.key'
...
Still MANUAL (one reviewed PR, so pk <-> vk <-> lock all move together):
  ...
  4. publish custom_ring_policy.key, custom_ring_base.key,
     custom_ring_compressed_policy.key and custom_ring_compressed_register.key with
