...
python3 scripts/generate_lockfile.py "$keys_dir" --release custom_ring.key --release audit.key
...
aws s3 sync "$keys_dir/" "s3://$bucket/$lock_prefix/" --exclude '*' --include '*.key' --exclude 'custom_ring.key' --exclude 'audit.key'
...
Still MANUAL (one reviewed PR, so pk <-> vk <-> lock all move together):
  ...
  4. publish custom_ring.key and audit.key with
