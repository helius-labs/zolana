...
use zolana_interface::{
    ...
    state::{discriminator, tree, ProtocolConfig, SplAssetCounter, SplAssetRegistry, ZoneConfig},
...
fn builders() -> Value {
    ...
    map.insert(
        ...
        instruction_json(&CreateAssetCounter { authority: owner() }.instruction()),
    );
    map.insert(
    ...
    map.insert(
        ...
        instruction_json(
            &create_tree.instruction_with_nullifier_params(tree::address_tree_params()),
        ),
    );
    map.insert(
    ...
    map.insert(
        ...
        instruction_json(
            &Transact {
                payer: account(30),
                tree: account(20),
                withdrawal: Some(sol_withdrawal),
                data: transact.clone(),
            }
            .instruction(),
        ),
    ...
    map.insert(
        ...
        instruction_json(
            &Transact {
                payer: account(30),
                tree: account(20),
                withdrawal: Some(spl_withdrawal),
                data: transact.clone(),
            }
            .instruction(),
        ),
    ...
    map.insert(
        ...
        instruction_json(
            &Transact {
                payer: account(30),
                tree: account(20),
                withdrawal: Some(spl_withdrawal_no_authority),
                data: transact.clone(),
            }
            .instruction(),
        ),
...
fn decode_acceptance() -> Value {
    ...
    let merge = merge_data();
    let mut wrong_prefix = merge.serialize().expect("serializable");
