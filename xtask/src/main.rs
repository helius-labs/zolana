...
fn generate_vk_registry_consts() {
    ...
    write_registry_spec_file(
        Path::new("program-libs/interface/src/verifying_keys/registry.rs"),
        "use super::registry_spec::VkRegistrySpec;",
        &SHIELDED_POOL_PROGRAM_ID,
        &VK_CATALOG,
    );

    const SDK_IMPORT: &str = "use zolana_interface::verifying_keys::registry_spec::VkRegistrySpec;";
    write_registry_spec_file(
        Path::new("sdk-tests/timelock-escrow/program/src/vk_registry_specs.rs"),
        SDK_IMPORT,
        timelock_escrow_program::ID.as_array(),
        &[
            (
                "escrow",
    ...
    write_registry_spec_file(
        Path::new("sdk-tests/dynamic-swap/program/src/vk_registry_specs.rs"),
        SDK_IMPORT,
        dynamic_swap_program::ID.as_array(),
        &[
            (
                "escrow_open",
    ...
    write_registry_spec_file(
        Path::new("sdk-tests/zk-program-swap/program/src/vk_registry_specs.rs"),
        SDK_IMPORT,
        ...
        &[
            ...
            ("make", &swap_program::verifying_keys::make::VERIFYINGKEY),
            ("take", &swap_program::verifying_keys::take::VERIFYINGKEY),
            (
...
fn write_registry_spec_file(
    path: &Path,
    import: &str,
    consumer: &[u8; 32],
    catalog: &[(&str, &groth16_solana::groth16::Groth16Verifyingkey<'static>)],
) {
    use zolana_interface::verifying_keys::registry_spec::derive_vk_registry_spec;
    ...
    for (name, vk) in catalog {
        let spec = derive_vk_registry_spec(vk, consumer);
        let const_name = format!("{}_REGISTRY", name.to_uppercase());
        out.push_str(&format!(
            "pub const {const_name}: VkRegistrySpec = VkRegistrySpec {{\n    \
             g2_count: {},\n    digest: {:?},\n    address: {:?},\n    bump: {},\n}};\n\n",
            spec.g2_count, spec.digest, spec.address, spec.bump
        ));
