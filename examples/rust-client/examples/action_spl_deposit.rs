//! Shield an SPL token into the pool with the ergonomic `create_deposit` action.
//!
//! Registers a test mint and its shielded-pool interface, mints to a funding
//! token account, then deposits. The deposit infers the mint, vault, and
//! registry from the token account.

use anyhow::Result;
use rust_client_example::{ensure_spl_asset, new_party, setup};
use solana_address::Address;
use zolana_client::{create_deposit, get_private_token_balances, sync_wallet, CreateDeposit};
