...
use zolana_squads_client::{
    seed_viewing_key_account, GetProposalsRequest, ReadAuthorization, SquadsBackend,
    SquadsBackendError, ViewingKeyAccountSeed, OP_TRANSFER, OP_WITHDRAW,
};
...
fn build_backend(
    accounts: Vec<(Address, Account)>,
    auditor: SecretKey,
) -> SquadsBackend<MockRpc, MockRpc, AllowSeededRead> {
    let indexer = MockRpc {
        accounts: Vec::new(),
    };
...
/// A withdrawal proposal is encrypted to the sender. A transfer proposal is
/// encrypted to the recipient. Authenticated decryption plus the
/// operation-bound core classifies them without a zero recipient sentinel.
#[test]
fn reconstructs_and_classifies_withdrawal_and_transfer() {
    ...
    let withdrawal_destination = Address::new_from_array([44u8; 32]);
    let withdrawal_hash = proposal_hash(
    ...
    // The transfer is encrypted to the recipient. The proposal recipient is
    // bound as Poseidon(owner_pk_field, nullifier_pubkey).
    let owner_hash = poseidon(&[
        recipient.owner.to_bytes().as_ref(),
        recipient.vka.nullifier_pubkey.as_ref(),
    ])
    .expect("owner hash");
    let transfer_hash = proposal_hash(
    ...
    assert_eq!(w.recipient, withdrawal_destination);
...
fn reconstruction_rejects_hash_mismatch() {
    ...
    let wrong_hash =
        proposal_hash(ProposalOperation::Withdrawal, 0, &[0u8; 32], &blinding, 999).expect("hash");
    let proposal = Proposal::new(
        sender.owner,
        Address::new_from_array([44u8; 32]),
        Address::default(),
        wrong_hash,
        ct,
        i64::MAX,
