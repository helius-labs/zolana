//! Backend API request and response types. They mirror the `Backend API`
//! section of `docs/squads_policy_program.md`. The backend maps them onto the
//! SDK proving and construction types.
...
pub struct AssetBalance {
    pub asset_id: u64,
    /// `Address::default()` for SOL.
    pub mint: Address,
    /// Total across the asset's UTXOs.
    pub amount: u64,
    /// Empty when `skip_utxos` is set.
...
pub struct ReconstructedProposal {
    ...
    /// For a transfer, the recipient's `owner_pk_field`. For a withdrawal, the
    /// exact public SOL/SPL destination account the proposal approves.
    pub recipient: Address,
    ...
    pub amount: u64,
    /// The withdrawn public amount for a withdrawal. `0` for a transfer
    /// (nothing leaves).
...
pub struct RequestCreateViewingKeyAccountRequest {
    pub owner: Address,
    /// Reserved for a versioned owner-authentication flow. Non-empty recovery
    /// enrollment currently fails closed because `owner` is a proof identity,
    /// not a verifiable signing public key.
    pub recovery_keys: Vec<P256Pubkey>,
    /// Reserved for a versioned flow that binds an owner public key and signed
    /// message to `owner`. Presence alone is not treated as authorization.
    pub owner_signature: Option<[u8; 64]>,
    /// `OWNER_KIND_KEYPAIR` (P256 rail) or `OWNER_KIND_SMART_ACCOUNT`
    /// (signatureless vault rail).
    pub owner_kind: u8,
}
...
pub struct RequestTransactRequest {
    ...
    /// (signatureless) rail and `Some` selects the P256 keypair rail. The backend
    ...
    /// spend proof reconstructs the input owner as
    /// `zolana_hasher::primitives::hash_bytes(vault)`, and the viewing key account
    /// stores only that hash (its `owner`), not the vault itself, so the client
    /// supplies the preimage. `None` on the P256 rail.
...
pub enum TransactionType {
    ...
    /// Public exit. The spent input's `asset_id` selects the SOL or SPL rail,
    /// and the pool interface and vault derive from it. The destination is a
    /// system account for SOL or an SPL token account for SPL.
    Withdraw {
        public_amount: u64,
