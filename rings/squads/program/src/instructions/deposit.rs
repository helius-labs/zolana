//! `deposit` (tag 1): move funds into the shielded pool through the zone. A
//! fully public deposit with no proof and no co-signer. Settles through the
//! SPP's proofless `zone_deposit` in the same transaction.
...
pub fn process_deposit_ix(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    ...
    // The deposited UTXO's owner hash matches the zone spend circuit's
    ...
    let owner_utxo_hash = poseidon2(&owner, &ix.blinding, SquadsZoneError::ProofHashingFailed)?;
    ...
    // In SPP's `zone_deposit` order `ring_auth` is SPP's `ZoneConfig`. SPP
    ...
    spp_zone_deposit(accs.spp_program, &cpi_accounts, &spp_data, ring_auth_bump)
}
