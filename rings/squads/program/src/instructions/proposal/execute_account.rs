//! `execute_proposal` account layout.
//!
//! The recipient viewing key account is consumed only for a transfer, so the
//! withdrawal and transfer layouts line up with the builder without a
//! placeholder slot. `proposal` and `rent_recipient` stay mutable for the
//! closing refund.
