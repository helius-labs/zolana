...
use zolana_squads_interface::{
    ...
    instruction::instruction_data::CreateRingConfigIxData, state::ring_config::SquadsRingConfig,
    RING_CONFIG_PDA_SEED,
};

use crate::shared::pda::{verify_pda, CreatePdaAccount};

/// `create_ring_config` (tag 3): initialize the singleton ring config PDA.
///
/// Accounts: `[creator (signer, writable, fee payer), ring_config (writable, the
/// new PDA), system_program, program, program_data]`. The config is created at
/// the canonical `[b"ring_config"]` PDA, sized exactly for its serialized form,
/// and stamped with the discriminator via [`SquadsRingConfig::new`]. On an
/// upgradeable loader-v3 deployment, `creator` must be the program's deploy
/// upgrade authority.
#[inline(never)]
pub fn process_create_ring_config_ix(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    if accounts.len() < 5 {
    ...
    let ix = CreateRingConfigIxData::deserialize(data)
    ...
    let bump = verify_pda(ring_config.address(), &[RING_CONFIG_PDA_SEED], &crate::ID)?;
    ...
    let space = SquadsRingConfig::account_size(ix.auditor_keys.len(), ix.merge_authorities.len());

    CreatePdaAccount {
        fee_payer: creator,
        new_account: &mut *ring_config,
        space,
        owner: &crate::ID,
        signer_seeds: [RING_CONFIG_PDA_SEED],
        bump,
    }
    .execute()
...
fn check_initialization_authority(
    ...
    if program.address() != &crate::ID {
        return Err(SquadsRingError::InvalidInitializationAuthority.into());
    }
    ...
    match upgrade_authority_address.map(|key| key.to_bytes()) {
        ...
        _ => Err(SquadsRingError::InvalidInitializationAuthority.into()),
    }
}
...
#[inline(never)]
pub(super) fn write_ring_config(
    account: &mut AccountView,
    config: &SquadsRingConfig,
) -> ProgramResult {
    let bytes = config
        .serialize()
