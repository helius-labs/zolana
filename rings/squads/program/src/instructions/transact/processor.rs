...
/// `transact` (tag 0): ring-proof-gated synchronous transfer/withdrawal settled
/// through the SPP.
///
...
/// `recipient_vka`). The ring proof is verified here, then the settlement is
/// forwarded to the SPP via the ring-auth-signed CPI.
#[inline(never)]
pub fn process_transact_ix(accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    let ix =
        TransactIxData::deserialize(data).map_err(|_| SquadsRingError::InvalidInstructionData)?;
    ...
    let recipient = ring_recipient(encrypted_utxos, recipient_vka.as_ref())?;
    ...
    // `private_tx_hash` already covers the outputs (spec Ring Proof table).
    RingProof {
