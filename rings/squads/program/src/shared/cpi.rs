//! Ring-auth CPI signer and SPP cross-program invocations. The Squads ring
...
/// Build the `[b"ring_auth", bump]` seeds for the ring-auth CPI signer. The
...
/// three (`ring_transact` `[payer, tree, ring_config]`, `merge_ring` `[tree,
/// ring_config, payer]`) plus a trailing SPP program account: SPP settles then
/// emits its `GeneralEvent` via a self-CPI (`invoke(crate::ID, [])`), which can
/// only resolve if SPP's own program account is present in the context the ring
/// forwarded, so every settlement CPI forwards it. Hence `3 + 1`.
const SPP_CPI_MAX_ACCOUNTS: usize = 4;

/// Maximum CPI accounts the proofless deposit forwards to SPP's `ring_deposit`.
...
/// depositor, ring_config, user_token, vault, registry, token_program,
/// program]`.
const SPP_DEPOSIT_CPI_MAX_ACCOUNTS: usize = 8;

/// Maximum CPI accounts a withdrawal forwards to SPP's `ring_transact`: the
/// transfer's `[payer, tree, ring_config]`, the settlement tail, and the
...
/// SPP `ring_transact` CPI: settle a synchronous transfer through the
/// shielded pool, signed by the ring-auth PDA. `accounts` must be exactly
/// `[payer, tree, ring_auth, spp_program]` (the transfer leg only -- the
/// withdrawal leg has a settlement tail and uses [`spp_ring_withdraw`]). The
...
pub fn spp_transact(
    ...
    invoke_ring_signed::<SPP_CPI_MAX_ACCOUNTS>(accounts, instruction_data, ring_auth_bump)
}

/// SPP `merge_ring` CPI: settle a merge proof through the shielded pool,
/// signed by the ring-auth PDA. `accounts` must be exactly `[tree, ring_auth,
/// merge_authority, spp_program]` -- SPP's `merge_ring` reads `ring_auth` (its
/// `ring_config`) as one signer and a second, independent signer it calls
/// `payer` (the ring forwards its own `merge_authority`, already a real
...
/// SPP `ring_transact` CPI for a withdrawal: like [`spp_transact`] but forwards
...
pub fn spp_ring_withdraw(
    ...
    validate_spp_program(spp_program)?;
    invoke_ring_signed::<SPP_WITHDRAWAL_CPI_MAX_ACCOUNTS>(
        accounts,
        instruction_data,
        ring_auth_bump,
    )
...
/// SPP `ring_deposit` CPI: settle a proofless deposit through the shielded
/// pool, signed by the ring-auth PDA. Unlike `ring_transact`/`merge_ring`, SPP's
...
/// spp_program]`. The depositor is already a real signer. The ring-auth PDA is
/// flipped to signer by `invoke_ring_signed`.
#[inline(never)]
pub fn spp_ring_deposit(
    spp_program: &AccountView,
    accounts: &[&AccountView],
    instruction_data: &[u8],
    ring_auth_bump: u8,
    ...
    validate_spp_program(spp_program)?;
    invoke_ring_signed::<SPP_DEPOSIT_CPI_MAX_ACCOUNTS>(accounts, instruction_data, ring_auth_bump)
}
...
/// `invoke_signed`.
#[inline(always)]
pub(crate) fn invoke_ring_signed<const MAX_ACCOUNTS: usize>(
    accounts: &[&AccountView],
    instruction_data: &[u8],
    ring_auth_bump: u8,
) -> ProgramResult {
    ...
    .map_err(|_| SquadsRingError::SppCpiFailed)?;
    Ok(())
}

/// Every CPI into SPP must run this first. Without the executable and id
/// check, the ring-auth signature could be handed to an arbitrary program.
#[inline(always)]
pub(crate) fn validate_spp_program(spp_program: &AccountView) -> Result<(), ProgramError> {
    ...
    {
        return Err(SquadsRingError::InvalidSppProgram.into());
    }
    Ok(())
}
...
fn derive_ring_auth_address(bump: &[u8; 1]) -> Result<Address, ProgramError> {
    Address::create_program_address(&[RING_AUTH_PDA_SEED, bump.as_slice()], &crate::ID)
        .map_err(|_| SquadsRingError::InvalidRingAuth.into())
}
