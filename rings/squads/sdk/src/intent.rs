...
pub struct Recipient {
    ...
    /// Recipient's shared viewing public key.
...
pub struct OutputUtxo {
    ...
    pub asset: [u8; 32],
    /// Output owner. `None` marks the sender's own change output (its blinding
    /// is derived, not chosen). `Some` carries a transfer recipient.
    pub recipient: Option<Recipient>,
    /// Output blinding. For a recipient output the wallet chooses it (31-byte
    /// field element, right-aligned). For the sender change it is derived by
    /// the KDF chain and may be left zero here.
    pub blinding: [u8; 32],
}
