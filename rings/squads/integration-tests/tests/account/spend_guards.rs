...
fn seed_ring_config(test: &mut SquadsRingTest, co_signer: &Pubkey) -> Pubkey {
...
fn seed_vka(test: &mut SquadsRingTest, owner: [u8; 32], state: u8) -> Pubkey {
...
/// The ring config, the sender and recipient accounts, and the co-signer every
/// spend path reads.
struct Fixture {
    co_signer: Keypair,
    ring_config: Pubkey,
...
fn fixture(test: &mut SquadsRingTest, sender_state: u8, recipient_state: u8) -> Fixture {
    let co_signer = Keypair::new();
    test.airdrop(&co_signer.pubkey(), 1_000_000_000)
        .expect("fund co-signer");
    let ring_config = seed_ring_config(test, &co_signer.pubkey());
...
fn transact_rejects_a_missing_payer_signature() {
    ...
    assert_eq!(
        custom_code(&err),
        SquadsRingError::MissingAuthoritySignature as u32
    );
}
...
fn transact_rejects_an_expired_transaction() {
    ...
    assert_eq!(
        custom_code(&err),
        SquadsRingError::TransactionExpired as u32
    );
}

/// The transfer operation names the `(2, 2)` shape. Instruction data that names
/// another would let the SPP proof describe a spend the ring proof never
/// covered.
#[test]
fn transact_rejects_a_shape_the_operation_does_not_name() {
    ...
    assert_eq!(
        custom_code(&err),
        SquadsRingError::ProofShapeMismatch as u32
    );
}
...
fn fold_transact_rejects_an_unsupported_leg_count() {
    ...
    assert_eq!(
        custom_code(&err),
        SquadsRingError::InvalidInstructionData as u32
    );
}
...
fn seed_proposal(test: &mut SquadsRingTest, owner: [u8; 32], expiry: i64) -> (Pubkey, Pubkey) {
    let address = Keypair::new().pubkey();
