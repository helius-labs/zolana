//! Boots a `zolana test-validator` with the SPP and the squads zone program
//! loaded, initializes SPP's protocol config, creates the zone's own
//! `zone_config`, and runs `init_spp_zone_config` as a real CPI into a real
//! SPP.
//!
//! The composed-localnet counterpart to `init_spp_zone_config_e2e.rs`. It
...
//! smart-account, squads zone) is missing. Requires the `zolana` CLI binary
...
use zolana_interface::{
    ...
    pda,
    state::{
        discriminator::RING_CONFIG as SPP_ZONE_CONFIG_DISCRIMINATOR, RingConfig as SppZoneConfig,
    },
...
fn init_spp_zone_config_against_real_validator() {
    ...
    let ring_auth = ring_auth_pda(&squads_zone_program_id());
    ...
    // SPP's zone_config account lives at the zone's own ring_auth PDA and is
    // owned by SPP.
    let account = rpc
        .get_account(Address::new_from_array(ring_auth.to_bytes()))
        .expect("rpc get_account")
        .expect("SPP zone_config exists at the ring_auth address");
    ...
    let (_, ring_auth_bump) =
        Pubkey::find_program_address(&[RING_AUTH_PDA_SEED], &squads_zone_program_id());
