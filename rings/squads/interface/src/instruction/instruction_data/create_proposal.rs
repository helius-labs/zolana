...
/// proposal's `discriminator`, `owner` (= the viewing-key account's owner
/// encoding), and `rent_payer` (= the authenticated raw owner/vault).
#[derive(Clone, Copy, Debug, PartialEq, Eq, SchemaRead, SchemaWrite)]
pub struct CreateProposalIxData {
    /// Recipient owner field for a transfer. For a withdrawal this is the exact
    /// SOL/SPL destination account.
