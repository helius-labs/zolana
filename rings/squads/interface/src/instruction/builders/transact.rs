...
pub enum TransactWithdrawal {
    ...
    /// SPL: SPP moves tokens from the per-mint `spl_interface` to
    /// `user_token_account`, signed by `cpi_authority`.
    Spl {
    ...
    pub fn push_account_metas(&self, accounts: &mut Vec<AccountMeta>) {
        match *self {
            ...
            TransactWithdrawal::Spl {
                ...
                accounts.push(AccountMeta::new(spl_interface, false));
...
///
/// For a smart-account sender, `payer` must be the sender vault and the
/// instruction must be invoked through the smart-account program so the vault is
...
impl Transact {
    pub fn instruction(&self) -> Instruction {
        ...
        accounts.push(AccountMeta::new_readonly(self.ring_auth, false));
