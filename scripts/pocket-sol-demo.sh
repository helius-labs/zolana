...
A_P256_WALLET="${WORKDIR}/wallet-a.p256.json"
B_P256_WALLET="${WORKDIR}/wallet-b.p256.json"
...
B_UNSHIELD_LAMPORTS="${B_UNSHIELD_LAMPORTS:-400000000}"
...
echo "Local solana-test-validator demo only. Do not use these defaults or the SHIELDED_POOL_PROGRAM_ID override against production."
...
P256_TRANSFER_JSON="$("$POCKET" transfer \
    ...
    --state "$A_STATE" \
    --owner-p256-wallet "$A_P256_WALLET" \
    --recipient-p256-wallet "$B_P256_WALLET" \
    --recipient-state "$B_STATE" \
    --tree "$TREE" \
    --prover-bin "$PROVER" \
    --keys-file "$KEYS_FILE" \
