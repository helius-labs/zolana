...
import (
	...
	"circuits/auditor_key_encryption"
	"circuits/witness"
)
...
func compileCircuit(id int) (constraint.ConstraintSystem, error) {
	...
	switch id {
	case CircuitAuditorKeyEncryption:
		circuit = &auditor_key_encryption.Circuit{}
		// The emulated P-256 arithmetic emits one BSB22 commitment; the higher
		// compress threshold keeps the committed wire count in the range
		// groth16-solana's single-commitment verifier supports.
