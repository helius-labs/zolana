//! Trusted-setup binary: writes `pk.bin` / `vk.bin` for a circuit and emits the
//! matching Rust verifying-key source.

use std::path::PathBuf;

use custom_ring_prover::{setup, CircuitId};
...
fn main() {
    ...
    let circuit = match circuit_arg.to_lowercase().as_str() {
        "auditor_key_encryption" => CircuitId::AuditorKeyEncryption,
    ...
    let rust_vk_path = rust_vk_path
        .unwrap_or_else(|| build_dir.join(format!("{}_verifying_key.rs", circuit.name())));
...
fn usage_and_exit(msg: &str) -> ! {
    eprintln!("error: {msg}");
    eprintln!("usage: custom-ring-prover-setup <circuit> <build-dir> [--rust-vk <path>]");
    eprintln!("  circuit: auditor_key_encryption");
