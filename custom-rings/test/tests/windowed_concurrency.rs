...
fn two_members_land_windowed_transfers_proven_against_the_same_roots() -> Result<()> {
    ...
    let entries_tree_id = tree_id(rpc, env.tree)?;
    ...
    for (member, &tag) in members.iter().zip(&tags) {
        ...
        .with_output_tree_id(entries_tree_id)?;
    ...
    let proofs = transfers
        ...
        .collect::<Result<Vec<_>, _>>()?;
    assert_eq!(proofs[0].state_root_index, proofs[1].state_root_index);
    assert_eq!(
        proofs[0].nullifier_root_index,
        proofs[1].nullifier_root_index
    );
