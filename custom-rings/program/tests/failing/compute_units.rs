...
use crate::common::{
    auditor_pubkey, authority, cosigner, create_config_fixture, create_head_map_root_fixture,
    create_key_registry_root_fixture, create_policy_fixture, delegate, ed25519_reader,
    grant_read_access_fixture, init_spp_ring_config_fixture, initialized_config_account,
    p256_reader, revoke_read_access_fixture, set_authority_fixture, set_cosigner_data,
    set_cosigner_fixture, set_delegate_data, set_delegate_fixture, set_paused_fixture,
    set_spend_window_data, set_spend_window_fixture, setup_mollusk, Fixture, USDC,
};
use custom_ring_interface::{
    CoSignScope, CREATE_CONFIG_COMPUTE_UNIT_LIMIT, CREATE_HEAD_MAP_ROOT_COMPUTE_UNIT_LIMIT,
...
fn operator_instructions_fit_their_published_budgets() {
    ...
    assert!(
        consumed(create_head_map_root_fixture(None))
            <= u64::from(CREATE_HEAD_MAP_ROOT_COMPUTE_UNIT_LIMIT)
    );
    assert!(
