//! Ring RPC wire contract and signed read requests.
...
use crate::reader::{ReaderKey, ReaderKeyError};
...
const AUDIT_CURSOR_LIMIT: usize = 256;
pub const AUDIT_PAGE_LIMIT: u64 = 100;
const DEPOSITS_PAGE_LIMIT: u32 = 50;
const MAX_DEPOSITS_PAGE_LIMIT: u32 = 200;

/// The one cursor rule, shared by the request builder, the page options and
/// the indexer response check.
pub fn cursor_in_bounds(cursor: &[u8]) -> bool {
    !cursor.is_empty() && cursor.len() <= AUDIT_CURSOR_LIMIT
}

pub fn limit_in_bounds(limit: &Limit) -> bool {
...
#[derive(Debug, Error)]
#[error("deposits cursor is not a signature")]
pub struct InvalidDepositCursor;
...
impl RingDepositsRequest {
    ...
    pub fn before(&self) -> Result<Option<Signature>, InvalidDepositCursor> {
        self.cursor
            .as_ref()
            .map(|cursor| {
                <[u8; 64]>::try_from(cursor.0.as_slice())
                    .map(Signature::from)
                    .map_err(|_| InvalidDepositCursor)
            })
            .transpose()
    }
}
