...
- Branch: `ts-sdk-port`
- Review HEAD: `c541ba75` at `2026-07-25 14:49 UTC`
...
- Progress: `31 done / 118 total`; `59 needs_fix`; `28 needs_re_review`; `0 todo`, counted at `2026-07-25 14:49 UTC`. Fix workers land commits while this block is read, so recount the Status column of the queue tables before you cite a number
...
- Active reviews: `C01-C22` reviewed `2026-07-25`; the client package gates stay unchecked while its fixed rows wait on re-review. `C21` closed the last `todo` row and is `DIVERGENT` on legacy-message account order
- Wallet package: `W01-W09` reviewed `2026-07-25`; the nine adverse verdicts are now fixed and `needs_re_review`, so the wallet package gates stay unchecked until an independent reviewer confirms. `W07` (sync-delegate viewing key), `W08` (missing shared-tag families), and `W04` (P256 rail source) were the blocking correctness findings
