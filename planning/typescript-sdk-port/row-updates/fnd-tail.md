...
| F106 | Consolidate duplicated byte/crypto primitives | PARTIAL | Example merge moved `test-kit` `base58`/`native` primitives into production packages; `smart-account-client/src/sha256.ts` is deleted in favour of `@noble/hashes`, and its hand-rolled base58 is now `bs58`. Cross-package hand-rolled `base58`/`concat` copies remain in `interface`, `client`, `wallet`, and `test-kit` `user-registry`. | Follow-up issue: shared internal util surface; large mechanical refactor. |
