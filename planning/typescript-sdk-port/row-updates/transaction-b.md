...
Branch: `port/transaction-b`, seven commits on the integration tip.
...
encrypted transactions), `zoneAuthority` (9 cases), and
`fromUtxos.mergeIntoUtxos` (4 cases).
...
Count: `10` rows reach evidence-backed `PARITY` this pass (T01, T04, T09, T18,
T19, T22, T24, T25, T27, T29). `5` are advanced by new executed evidence but
remain adverse (T06, T07, T08, T11, T28). The rest were not reached.
...
- **T10, T17, T26, T30, T31** are aggregate export rows. They inherit the rows
  ...
  here. `./transact` carries the same four values and types, which closes part
  of T26's named omission list.
