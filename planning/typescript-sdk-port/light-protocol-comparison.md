...
| F5 | Zolana's error model is richer than Light's in every way that matters, and loses the underlying failure on the way. Three specific hazards, all local fixes. | Cheap now |
...
| F11 | Zolana's prover client is ahead of Light's on every axis except circuit coverage. | Permanent, stop treating as a gap |
...
Its surface is `record`, `outputs`, `byViewTag`, and `transaction` — none of the
methods `ZolanaIndexer` exposes — so it cannot be substituted for the real
...
`throw new Error(...)` — twenty-three of them in `rpc.ts` alone — and
...
there underneath — `LightSystemProgram.transfer` at line 85 — so a caller who
