...
Do not cite a figure from this table without running the gate; it is refreshed
by hand as workers commit and it goes stale within the hour.

```bash
node sdk-libs/ts/config/pkp-entry-gate.mjs
```

Last update: 2026-07-26 01:40.
...
| Continuous integration | Three checks failing, one cause. `main` deleted `CreateTree.owner`, which this branch still carries, so `cargo check (workspace)` does not compile and the Rust test jobs inherit that |
